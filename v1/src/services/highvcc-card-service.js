...
export function createHighvccCardService({ pool, encryptionKey, panHmacKey, fetchImpl = fetch, addressState = 'OR' } = {}) {
  ...
  const provider = createHighvccCardProvider({ getAccessToken, fetchImpl });
  ...
  async function openCard({ vid, amount, confirmation, requestedBy = 'admin', firstName, lastName } = {}) {
    ...
    try {
      ...
      const address = await addressSource.load(`highvcc:${cardUuid}`);
    ...
    if (!/^\d{12,19}$/.test(pan)) {
      throw new PublicApiError('opened card did not return a usable card number', { code: 'HIGHVCC_OPEN_NO_PAN', status: 502 });
    }
    const balanceDollars = Number.isFinite(Number(card.balance)) ? Number(card.balance) / 100 : a;
    ...
    try {
      ...
      if (conflict.length) {
        // Opened successfully on the platform (money already spent) but this exact card
        // already exists in inventory — never insert a duplicate; surface for manual look.
        throw new PublicApiError('opened card already exists in inventory; check for a duplicate open', {
    ...
    return {
      ...
      balance: balanceDollars.toFixed(2), vid: v, amountRequested: a,
      feeDetail: opened.feeInfo?.feeDetail ?? null, requestedBy,
    };
  }

  return { tokenStatus, setToken, quote, openCard };
}
