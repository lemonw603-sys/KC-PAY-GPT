...
function fixturePool({ count = 0, expiresAt = '2026-08-24T03:00:00.000Z', risk = 0 } = {}) {
  const queries = [];
  const responses = [
    [[{
      ...
      session_repair_expires_at: new Date(expiresAt)
    }], []],
    [[{ count: risk }], []],
    [{ affectedRows: 1 }, []],
    [{ affectedRows: 1 }, []],
...
test('replaces Session on the same order and records metadata without retaining old Session', async () => {
  ...
  assert.match(queries[3].sql, /status = 'CARD_READY'/);
  assert.match(queries[4].sql, /task_type IN \('PREPARE_RECHARGE','SUBMIT_RECHARGE'\)/);
