...
test('Browser manual refresh actually awaits all three loaders',async()=>{
 let handler;const calls=[],notices=[];
 const ctx={document:{querySelector(){return{addEventListener(_,fn){handler=fn}}}},state:{view:'browser'},elements:{syncTime:{}},hideNotice(){},showNotice(x){notices.push(x)},Promise};
 for(const n of ['loadBrowserDispatchJobs','loadBrowserRuns','loadBillingAddressSettings'])ctx[n]=async()=>{await Promise.resolve();calls.push(n)};
 ...
 assert.equal(calls.length,3);assert(notices.includes('刷新完成。'));
});
