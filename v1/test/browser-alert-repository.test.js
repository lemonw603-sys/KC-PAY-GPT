...
test('browser alerts are one row per type and order, reopened on repeat, with fixed severities', async () => {
  ...
  assert.match(queries[0].sql, /INSERT INTO operator_alerts/);
  assert.match(queries[0].sql, /status = IF\(status = 'RESOLVED', 'OPEN', status\)/);
  assert.deepEqual(queries[0].values, ['BROWSER_PAYMENT_UNKNOWN', 'browser-browser_payment_unknown:order-1', 'order-1', 'critical', 't', 'm']);
