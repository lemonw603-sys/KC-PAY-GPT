...
后台生成请求必须带幂等键；断网后用同一键重试会返回原批次，不会新增卡密。新 CDK 在 `cdks` 表只保存带独立服务端密钥的 HMAC-SHA-256，历史 SHA-256 CDK 继续兼容查询和已兑换订单。为了处理响应丢失，新批次的明文集合使用另一枚独立密钥做 AES-256-GCM 加密，仅已认证且完成短时密码复核的自有后台可重新下载；批次没有未使用 CDK 后自动销毁恢复密文。旧批次没有明文副本，不能恢复。
