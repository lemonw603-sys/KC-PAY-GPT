# AI充值业务项目约定

> AI/工程 Agent 角色：本项目的当前接班执行者。上一模型退出、失败或上下文中断后，新模型必须从事实源和工作区恢复进度，继续设计、编码、测试、验证和交付，而不是重开项目或只做审查。新模型先读 `AGENTS.md`，按它的四份阅读顺序进入；不要从根目录 legacy 文件或 `docs/archive/` 开始漫游。

## 项目定位

这是 GPT Plus 自动充值与内部运营系统。客户使用 CDK 提交完整 ChatGPT Session；系统通过卡台 Open API 获取或分配一张专属虚拟卡，再由执行器完成 Plus 购买。未来主执行链路是 Browser Worker 直接操作 ChatGPT 官方购买和订阅管理页面。

第一版目标是建立可运营、可追踪、可人工兜底的最小闭环，不追求一次性覆盖所有充值方式。

## 单一事实源

- 接班一屏（现在的状态与下一步，收尾时覆盖重写）：`docs/HANDOFF_NOW.md`（新窗口先读）
- 目标、里程碑、唯一执行顺序：`docs/PROJECT_MAP.md`（一页）
- 生产事实表（**唯一**，其他文件只指向它）：`docs/CURRENT_STATE.md`；现场比对脚本 `browser-mvp/scripts/state-check.sh`
- 日常运维手册（自检/来单/演练/收口/发布/回滚，每步一条命令）：`docs/RUNBOOK.md`；维护窗口与迁移：`docs/PRODUCTION_PREP_RUNBOOK.md`
- 改造基线（用户确认）：`docs/PRODUCT_SIMPLIFICATION_DISCUSSION.md` 末尾「接班实施基线」；整改矩阵：`docs/SYSTEM_REMEDIATION_MATRIX_2026-09-06.md`
- 决策账本：`docs/DECISIONS.md`；过程记录：`docs/HANDOFF_LOG.md`
- 卡台来源与对账工作线：`docs/CARD_SOURCE_AND_RECONCILIATION_WORKSTREAM.md`（冻结稿 `docs/CARD_SOURCE_AND_RECONCILIATION_FROZEN_SPEC.md`）
- 全链路与状态机总册：`docs/PROJECT_OPERATING_MODEL.md`；产品与技术规格：`docs/V1_SPEC.md`；生产运行手册：`docs/PRODUCTION_PREP_RUNBOOK.md`
- 外部接口合同：`docs/contracts/`；Browser 研究：`docs/browser-research/`
- 2026-08 与 2026-09 的历史报告、审查、交接：`docs/archive/`（索引 `docs/archive/INDEX.md`），只作追溯，不作为当前事实

方向或范围发生变化时，先更新上述文档，再改实现。

## 第一版硬约束

- 产品是 Plus、5X、20X 三个（D-244/D-245，2026-09-17 Lemon 定）。Plus 两条路线都在生产跑；5X/20X 目标路线是 Browser（现由 Lemon 人工用比特浏览器充，路线 305/306 在生产开着），自动化按 V2 排期接入；API 路线能否充 Pro 未知。**ChatGPT 已可从 Free 直接升级到 20X，「先 Plus 后升级」两阶段方案退休（D-245）**，Pro 单与 Plus 同型：一次付款、一次确认、一次取消续费，只是 Checkout 选的套餐不同；页面行为仍须先经非付款 PoC 冻结到 `docs/contracts/` 才能进执行器。
- 一张卡同一时刻最多绑定一个活动订单；完成一单并释放活动分配后，可在**按产品的每卡成功充值上限**内顺序服务后续订单（**Plus 3 单、5X 与 20X 各 1 单，API 与 Browser 两路线一致**；2026-09-14 Lemon 定，D-221。2026-09-24 D-361 已按产品落地：唯一口径 `card-inventory-eligibility.maxPaymentsSql`，Plus 沿用全局键 3、5X/20X 各自的键 = 1，跑过 Pro 的卡不再分配）。容量以消费账本为权威；失败或付款状态不明确时保留占用，绝不释放或换卡重付。
- 卡台开卡写请求必须使用稳定的 `X-Idempotency-Key`；超时和 502/503 只能用原 Key 重试。
- **Browser 自动化代码脆弱，改动受限（D-254，2026-09-17 Lemon 定）**：付款前三件 `browser-mvp/src/billing-address-fill.js`、`live-chatgpt-payment-adapter.js`、`payment-executor.js` 的 submit 段**任何任务不许改**，除非 Lemon 当次单独批；其他 browser-mvp 改动只能在任务书列出的文件白名单内，越界即停下来问；browser-mvp 一动就跑全量测试 + 一次 rehearsal 演练都绿才算改完；常驻 worker 重启前问 Lemon。
- 任何付款执行器在提交结果不明确时都进入 `SUBMIT_UNKNOWN`，禁止自动重试、换卡或换执行器；Browser 点击付款后崩溃必须先对账，不能重新点击。
- 停止新订单与追踪已有订单是两个独立开关；停单不能停止状态轮询和退款同步。
- 客户侧不提供退款查询；退款、交易和余额提取仅在内部后台处理。
- 普通日志不得出现完整卡号、CVV、API Key、accessToken 或 sessionToken。例外（D-380，2026-09-26 Lemon 定「不限制不能存的东西」）：本机导航失败证据包（`~/Library/Application Support/pojia-browser-live/evidence/`，含 Playwright trace）可以含 Cookie、accessToken、sessionToken、请求头与返回内容、邮箱；只存本机、700 权限、14 天自动删、不进 git、不上传、不贴进聊天。卡号 / CVV 仍不得进证据：录制只覆盖进结账页之前。
- 金额使用十进制定点值或最小货币单位，不使用 JavaScript 浮点数直接结算。

## 外部系统与执行目标

- 卡台：`https://card.hnskj.vip/api/open/v1`
- Browser 目标：ChatGPT 官方购买和订阅管理页面；实际域名、页面结构和支付/3DS依赖必须先通过非付款 PoC 冻结到 `docs/contracts/`。
- Browser 主链路跑通优先于保持现有 Provider/路线表/许可命名和后台页面不变；允许最小调整。不得调整掉的只有防重复扣款、付款未知锁定和端到端审计。
- “提链”当前是候选而非已验收结论：必须区分 hosted 长链与依赖账号 Session 的内部短链；与原诺汇盛上号器先做同账号、同菲律宾 sticky 出口的只读配对，会创建 Checkout 的比较使用隔离账号 cohort。
- Browser PoC 采用隔离赛道，不把所有兼容逻辑堆入一个执行器；赛道表示架构，Runtime/Cookie/网络表示公共实验轴。失败赛道留证后归档，最终只接入 champion 和最多一个可在 Checkout 创建前路由的预验证 fallback。
- 没有菲律宾出口可以继续 OFFLINE/NON_PH 筛选；普通菲律宾 VPN 只作预筛，最终结论需要接近未来生产的菲律宾 sticky 出口。
- 旧直充系统：ZZSHU 只保留历史兼容，不参与 Browser 新链路设计和实施。
- 开源基线：`https://github.com/KC-CatK/KC-PAY-GPT`

业务层不得直接拼接外部 URL 或解析供应商/页面私有字段。HNSKJ 调用经过 `HnskjCardProvider`；ChatGPT 页面行为经过版本化 Browser Executor 和页面适配层。

## Fork 策略

- 保留一个未改动的上游基线分支，便于比较和同步。
- v1 继续保留现有 CDK、订单、后台、MySQL、任务、维护开关、通知和历史适配器；Browser Worker 只有在设计、非付款 PoC、仿真和单独上线确认完成后才允许注册和启动。
- 浏览器池、Stripe、hCaptcha、代理、地址、录屏截图等旧能力可以保留在 Git 历史或隔离代码中，但默认不得注册路由、启动 worker 或要求生产密钥。
- 不为“以后可能使用”而保留存在高危漏洞的运行时依赖；恢复旧能力时按当时版本重新审查并接入。

## 开发纪律

- **回答事实前的强制三道闸门**：凡要向用户回答“现在是什么状态、为什么、能不能用、下一步会怎样”，必须在当轮依次完成：①读取当前原始证据（代码；涉及生产再读 release/服务/数据库/请求日志）；②读取决定该事实的业务规则或覆盖项；③把新证据与 `PROJECT_MAP.md`、`CURRENT_STATE.md` 和用户已确认事实做冲突检查。任一项未完成只能回答“尚未核实”，禁止直接给结论或据此规划。
- **核实要留痕、不靠自称（2026-09-15 立，D-234）**：报任何现状类结论（状态/数量/能不能/为什么/两路线走向）时，必须随附**查到的证据**——查询语句或代码位置 + 关键的原始输出片段；拿不出证据的一律视为“尚未核实”，不得当结论或规划依据。这条把三道闸门从“AI 自称查过”升级成“一眼能验”：查没查、查得对不对，由随附的原始证据回答，不由 AI 的措辞回答。样板＝V2 §2.8 供卡链路核查（`provider_accounts`/`orders` 实查、frozen 与实际分配交叉验证、中途两次推断被数据推翻即改）。
- **三条惯犯规则（2026-09-11 立，D-172；违反过多次，优先级高于其他表达习惯）**：
  1. **当场重查再汇报**。向用户报任何状态（成功/失败/完成/在跑/数量）之前重新查一次，不得使用本轮早些时候读到的值；轮询时中间态不是终态；"合格/可分配/已完成"这类派生结论必须跑真实规则或既有查询得出，不得自拼 SQL 或凭理解推断。
  2. **不给观察补原因**。数字与预期不符时只能写"差多少、原因未知、待查"；没有证据的原因一个字都不写。
  3. **外部字段先验真响应**。外部系统的字段名、状态值、响应结构，必须对着一次真实响应核实后才能依赖；自己写的测试夹具不构成对外部合同的证明（曾把 `cardNo` 同时写进代码与夹具，真实字段是 `lastFour`，测试全绿而功能恒不生效）。统计必须写明分母与排除项。
  4. **先想边界再动手，别拿反馈当设计**（2026-09-12 立，D-186；当天犯了五次）。做工具或写查询前先问
     「什么算合法输入、什么算不合法」，不要先做一个能跑的再靠报错修补。已犯：SQL 提取器四次误报
     （嵌套模板、驱动语法、注释里的 SQL、条件片段），以及 D-184 写完 SQL 靠测试绿当验证——
     而测试里的数据库是假的。反馈快的场合代价小，客户页与资金路径上代价不是一个量级。
  5. **方案按业务流程检验**：定稿前过一遍"客户任意时间自助操作、运营不在场"，需要人在场的步骤必须显式标出并说明代价。
- **观察与结论分开**：原始字段只能写成观察，例如“余额 `$0.01`”；“可补余额”“可分配”“永久不可用”等属于业务结论，必须有对应规则和完整条件支持。不得把相关性、局部字段或默认状态补写成业务含义。
- **冲突即停止下结论**：现场值和已确认事实不一致时，先明确存在冲突并查根因；不得选择其中一个顺眼的证据继续推演，也不得用最新一次局部查询覆盖用户已确认规则。
- **先定位再行动**：先确认具体项目、页面、模块、路由和责任边界；未定位清楚时不得创建任务、改代码或安排部署。
- **结论必须现场核验**：事实结论至少核对当前代码；涉及生产时还要核对当前 release、服务/进程环境、数据库状态、请求或日志。不得凭记忆、对话摘要、旧报告或文案直接断言。
- **所有具体事实必须核对证据**：凡涉及页面/导航/按钮/入口、接口路径、开关状态、流程顺序、生产能力、数据数量、部署版本或“已经完成”的回答，必须先查当前代码和相关运行证据；涉及生产再核对当前 release、服务/进程、数据库或请求/日志。不能把功能名称、接口名称、历史布局、旧报告或对话摘要推断成当前事实。若只完成代码核对，必须明确标注“代码显示”；尚未现场验证的内容不得说成已完成。
- **分层表达**：明确标记“已验证事实 / 代码已验证但生产未验证 / 尚未验证 / 建议”；发现证据冲突先停在最早不确定点，不用旧结论覆盖现场行为。
- **业务状态不得由单字段推断**：卡片/库存结论不能只看 `cards.status`、`inventory_status` 或余额；必须同时核对 `card_operational_overrides`（`RETIRED`/`PRODUCT_ONLY`）、活动 assignment、消费账本和当前 Provider 证据。余额低不等于“可补余额”，Provider 显示 active 也不等于业务可分配。若原始字段与已确认运营规则冲突，先按冲突处理并查清，不得用局部查询覆盖已确认规则。
- **执行必须闭环**：修改后运行针对性测试和必要的生产只读复验；发现问题要修正事实源和实现，不能只在聊天或报告里记录。
- **收尾不靠用户开口，也不靠 AI 自觉，靠脚本**（2026-09-12 Lemon 提出「聊久了怕留尾巴，但我想不起来要求，让你自动做又怕做不好」）：
  1. **落盘与收尾每轮自动做**，不等用户说。触发条件是客观的：生产状态变了、用户做了决定、一个可交付完成了——三者任一发生就当轮落盘。
  2. **用户验证只需一行**：`scripts/wrapup-check.sh`。八项全绿就是没留尾巴；有 `[失败]` 是硬伤，有 `[提醒]` 要逐条看过。**做没做、做得对不对，由脚本回答，不由 AI 自称。**
  3. **对齐要 AI 主动给**：用户连续做完一批决定后，不等他要，主动列一份「我理解的决定」清单让他核对。机器查不了「我理解的和他的本意是否一致」，这一项只能人看。
  4. **发现新的漏法就往脚本里加一条**，不要写进记忆或注释了事。已经这样加过的：设计稿硬规则断言、前后端校验清单比对、客户链路 SQL 对生产实跑、文档悬挂词扫描、本机遗留调试服务检查。
- **当轮落盘**：重要业务决策、代码提交、部署 release、运行配置和生产状态变化必须在同一轮更新对应事实源；生产事实只改 `CURRENT_STATE.md`（唯一），`PROJECT_MAP.md` 只保留里程碑与执行顺序，历史过程进入 `HANDOFF_LOG.md`/专项证据；窗口收尾覆盖重写 `HANDOFF_NOW.md`。
- **生产写操作必须独立核实**：任何写库/改开关/部署之后，用一次**新的**独立查询（新连接、新命令）核实结果，不得拿同一批命令混杂的输出当确认（2026-09-09 门槛误报教训）。
- **写库只走正式路径**：生产库写操作只通过正式连接池的脚本/服务/后台接口（可 `--dry-run`、带审计、复用仓库函数）；`prod-query.sh` 一类只读工具不得用于写。
- **不替用户下结论**：用户描述的界面、现象、操作，不凭措辞推断成事实；能核实就核实，不能核实就用可观察特征向用户确认（2026-09-09 "结账页"误判教训）。
- **时间一律带时区**（UTC 或 UTC+8 标明），落盘与汇报同。
- **统筹负责原始证据**：协作窗口的结论不得靠转述或任务状态猜测；统筹必须读取其原始回复/提交/报告，并与当前工作区和生产事实交叉核对后再向用户汇报。
- **本项目已关闭工具审批弹窗**（2026-09-11 19:47 UTC，Lemon 同意，仅本项目 `.claude/settings.json` 生效，不影响其他项目）。**这意味着机器不再拦你，唯一的闸门就是下面这条规则本身**——花钱与生产动作必须先开口问，不得因为"技术上能直接执行"就直接执行。
  仍保留弹窗的只剩三类**真金白银出去且不可逆**的：`go-live.sh`（拉起真付款；2026-09-26 起已退役，一运行就停下，D-385）、`highvcc-card.mjs open`（真开卡）、`card-funding`（真补余额）。
  2026-09-12 去掉了 `deploy-release.sh switch` 的弹窗：发布不花钱、可回滚（switch 自带 ROLLBACK 命令），而我每次发布前本来就在对话里问过，同一个决定让 Lemon 点两遍纯属噪音（当天密集修 bug 发布了 4 次）。**去掉的是机器的第二道确认，不是「先开口问」这条规矩**——发布照样先问。
- **资金和生产动作先确认**：开卡、补余额、充值、付款、退款、提现、切换生产执行权限或其他可能产生资金/客户影响的动作，执行前必须说明影响、停止点和回滚/收敛方式，并取得当次确认。
- 面对真实故障，先以实际请求、响应、数据库状态和供应商运行行为定位，再修改代码。
- 外部 API 响应必须做运行时 Schema 校验；未知状态进入人工处理，不能默认映射为成功或失败。
- 数据库状态迁移必须显式、可审计；任务处理使用租约和有界重试，不能让单个失败订单阻塞队列。
- 未经操作时确认，不创建 API Key、不真实开卡、不提交真实充值、不提取余额或销卡。
- Browser 项目的重大方向、运行事实、接口冻结、PoC/灰度/容量结论必须同步回写 Browser 基线、`DECISIONS.md`、合同和交接文档，不能只保留在聊天上下文中。
