...
- 日期：2026-08-17
- 性质：fixture 合同测试 + 2026-08-21 一次真实直充失败响应记录
...
- 真实生产响应：HTTP 400、业务码 `40030`，消息为“仅支持免费账号提交，当前账号套餐为 plus”。这证明目标账号当前为 Plus 时上游拒绝；API 路径不增加独立套餐预检，需把该明确拒绝映射为原订单 Session 可恢复状态并保留资金审计，不重试、不换卡。
