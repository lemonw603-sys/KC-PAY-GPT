...
- 存储：MySQL 只保存带独立服务端密钥的 HMAC-SHA-256，不保存 CDK 明文
...
运行前由部署密钥存储注入 `NODE_ENV`、`DATABASE_URL`、TLS 字段和 `CDK_HASH_KEY_V1_BASE64`，不要把带密码的 URL 或密钥写进 shell 历史。生产远程数据库缺少 `DATABASE_TLS=true` 时，工具会在连接前失败关闭。

- UTF-8 文本，每行一枚 CDK；忽略空行和首个 BOM。
- 最多 1000 个非空行，文件最大 2 MiB。
- 只接受系统高熵格式：`PJ-` 加 20 位去除易混淆字符的随机串；不再接受短码或自定义低熵格式。
