...
当前精确停止点：API 常驻最小充值权限为 `true`，接单与派发仍为 true，健康端点 ready；Browser 当前 main 只读 smoke 与隔离 dry-run 已通过。下一步可按客户式 CDK+Session 测试订单临时切换 Browser，付款前停止。Claude 单列三步客户充值页已部署至生产，并完成公网 CSP、桌面/移动、教程和真实历史订单查询复验。
