...
本项目所有窗口必须遵守根级 `CLAUDE.md` 的“开发纪律”，尤其是：先定位模块；事实结论核对当前代码与生产证据；事实、未验证和建议分层；冲突先停；执行后复验；重要决策、提交、部署和生产状态当轮落盘。统筹窗口必须读取并核对协作窗口的原始回复、提交或报告，不能仅凭转述或任务完成状态汇报。任何资金或生产动作执行前必须说明影响、停止点和回滚/状态收敛方式，并取得当次确认。

地图只保存最新有效状态；历史过程和被推翻的旧状态不得继续出现在“当前事实”区域，而应进入 `HANDOFF_LOG.md`、`DECISIONS.md` 或专项证据报告。

| 文件 | 只负责什么 | 何时更新 |
|---|---|---|
| `PROJECT_MAP.md` | 目标、进度、当前生产摘要、唯一顺序 | 决策/阶段/生产/优先级变化 |
| `CURRENT_STATE.md` | 最新生产运行事实 | release、服务、env、DB gate、readiness 变化 |
| `PROJECT_OPERATING_MODEL.md` | 全链路、状态机、控制矩阵、验收总册 | 合同/验收口径变化 |
| `DECISIONS.md` | 用户确认的决策历史及理由 | 新确认或旧决定被推翻；历史状态栏不得覆盖 `CURRENT_STATE.md` |
| `HANDOFF_LOG.md` | 时间线与执行证据 | 每次部署、验证、修复、清理 |
| `ROADMAP.md` | 历史阶段明细 | 仅保留历史，不决定下一步 |

维护纪律：事实、决策、建议必须标明；旧报告不自动升级为当前事实；一次重大变化必须在同一提交更新地图、状态和相应账本。新窗口只需先读地图、当前状态、本总册，再按链接追证据。
