...
开卡接口返回卡片 ID 不等于最终成功。只有卡状态为可用、卡余额达到开卡金额、卡号/CVV/有效期完整时才能进入 `CARD_READY`。完整卡资料只在服务端加密暂存，直充创建成功后立即清除；不得进入 Provider 日志、后台列表或客户页面。等待超过限定次数时进入 `RECONCILIATION_REQUIRED`，不得自动重开卡。
