...
1. 客户提交 CDK。
2. 服务端验证 CDK 可用，并建立短期兑换会话。
3. 客户提交完整 Session JSON。
4. 服务端校验必要字段、JWT 结构及有效期。
5. 在一个数据库事务中占用 CDK并创建订单。
6. worker 查询卡台余额和指定卡段可用性。
7. worker 使用订单的稳定幂等键开一张卡。
8. 保存卡台卡 ID和订单独占绑定，再读取卡资料。
9. 将订单写为 `SUBMITTING` 后，调用直充创建接口。
10. 明确获得 `201 + code=0 + order_no + card_key` 后保存外部标识。
11. 定时查询直充状态，直到成功、失败或需要人工处理。
12. 成功后进入退款观察；取消续费状态异步补查，不阻塞充值成功。
