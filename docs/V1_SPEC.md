...
6. 保存卡台卡 ID 和订单独占绑定，创建独立的卡片就绪检查任务。
7. 检查任务异步确认卡状态、余额和完整卡资料；未就绪则延迟复查，明确失败只终止当前订单。
8. 完整卡资料加密暂存后，将订单写为 `SUBMITTING` 并调用直充创建接口。
9. 明确获得 `201 + code=0 + order_no + card_key` 后保存外部标识并清除暂存卡资料。
10. 定时查询直充状态，直到成功、失败或需要人工处理。
11. 成功后进入退款观察；取消续费状态异步补查，不阻塞充值成功。
