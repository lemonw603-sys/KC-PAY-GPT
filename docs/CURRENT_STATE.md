...
- 第二单 API 灰度已完成；临时充值写权限已关闭，单笔 Permit 已撤销。卡台已读到 `PURCHASE 15.76 USD`，05:59 UTC 余额由 `$16.00` 降为 `$0.24`，资金扣减与消费金额一致；06:09 UTC 复查交易状态仍为 `PROCESSING`，最终状态待后续只读补证，绝不重付。
