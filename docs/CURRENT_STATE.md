...
|---|---|---|---|
| 生产 release | `/opt/pojia/releases/20260911-resolve-unknown-ui-3d4936d`（commit `3d4936d`；B1：`RESOLVE_UNKNOWN_PAYMENT` 三个 bug 修复并接后台按钮——F-44 renewalCancelled 严格布尔、F-45 Pro 单 CHARGED 落 20X 交接态不记整单成功且 COMPLETE_20X 可接续、F-46 续费已关同步写订单/run 取消字段；两分支补关 dispatch/leases/artifacts；admin.js v=45；无新迁移；回滚点 `20260911-highvcc-snapshot-sync-275f6e7`） | 2026-09-11 04:42 UTC | `deploy-release.sh prepare`（981 文件 manifest OK、备份 OK）→ `switch`；服务器本机独立 curl：served admin.js 含 RESOLVE_UNKNOWN_PAYMENT×8 / 确认核实结果×3 / 严格布尔发送×1，service 文件含 strictBoolean 与 MANUAL_VERIFICATION_RESOLVED 证据检查，控制路由未登录 401，live/ready 200，web 无错误日志 |
...
| 已上线（本次 release） | `20260911-resolve-unknown-ui-3d4936d`：付款结果不明/已转人工的 Browser run 有了正式收口入口「确认核实结果」（run 面板 + 订单抽屉；RECONCILE_ONLY/HUMAN_REQUIRED × PAYMENT_UNKNOWN/PAYMENT_CONFIRMED，20X 交接态除外）。语义：已扣款+Plus 单→RECHARGE_SUCCESS（续费已关）或 CANCELLATION_REVIEW_REQUIRED；已扣款+Pro 单→只记 Plus 阶段，等人工升级后点「确认 20X 已升级」；未扣款→关单、释放卡、CDK 退回。真库集成 8/8 三轮稳定；**生产上尚未被真实点击过** | 2026-09-11 04:42 UTC | 服务器本机 curl 复验（见 release 行） |
