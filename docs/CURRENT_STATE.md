...
|---|---|---|---|
| 生产 release | `/opt/pojia/releases/20260911-alerts-342ad5f`（commit `342ad5f`；D-175 通知改造与排队超时告警）。回滚 `20260911-sync-skip-msg-56c5290` | 2026-09-11 15:18 UTC | `deploy-release.sh prepare`（981 文件 manifest OK、备份 OK）→ `switch`；服务器本机独立 curl：served admin.js 含 RESOLVE_UNKNOWN_PAYMENT×8 / 确认核实结果×3 / 严格布尔发送×1，service 文件含 strictBoolean 与 MANUAL_VERIFICATION_RESOLVED 证据检查，控制路由未登录 401，live/ready 200，web 无错误日志 |
