...
| 可分配卡（资格 SQL，Plus 门槛 16） | **1 张：3118（$60.00，2026-09-11 10:52 UTC 新开，BIN 53211304）**。非终态订单 0。9839/7428/2911 已从卡台消失 → `source_present=0`、`MISSING_FROM_SNAPSHOT`、HELD_FOR_REVIEW（9839 由 Lemon 本人注销，$50 已退回钱包并充入新卡 3118，已确认无资金问题，D-163） | 2026-09-11 10:58 UTC | 资格 SQL 直接查 = 9839 |
...
| 最近真实单 | `PJV1-CkFQM-aahxjZdJZX8vkY`（10:30 提交，mengx612，Pilot，卡 9839）：**走到点付款一次后被发卡方拒付**，页面红字「Tinanggihan ang iyong kard.」；未扣款（卡台余额未变）；已用 `resolve-unknown-payment.mjs` 记 NOT_CHARGED 收口 → CLOSED，卡与 **CDK 均自动退回**（F-48 修复生效）。详见 `docs/E2E_CHAIN_TEST_SAMPLE.md` 第 3 次 | 2026-09-11 10:58 UTC | 生产主机 dry-run + 真跑输出；隧道新连接独立 SELECT（orders/tasks/cdks/cards/card_assignment_history） |
