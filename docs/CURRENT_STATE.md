...
| 已知未修 | ①**结账页 403→500 根因（强证据假设，未做对照实验）**：用户手动路径 = 6 号窗口 + 上号器登录 + 同一菲律宾出口 + 同样页面点击，成功；差异只剩"登录态怎么进浏览器"。客户提交的 session 是 `/api/auth/session` JSON（只有 sessionToken），`session-bootstrap.js` 把它按 `{url}` 注入 → **host-only cookie `chatgpt.com`**；网站随后自己下发 `.chatgpt.com` 域的同名 session-token（Lane4 现场同时存在两个 `__Secure-next-auth.session-token`：`chatgpt.com`/无过期/len 3901 = 我们注入的，`.chatgpt.com`/90 天/len 3921 = 网站下发的）。今天是**第一次**真正走"替换常驻登录态→注入客户 token"路径（WAL：此前所有到过结账页的任务都是 `existingSessionPreserved`，用的是上号器装好的常驻登录态），主站接口容忍双 cookie（身份核对通过），结账页 SSR 不容忍（403/500）。次要差异：注入路径没有 auth.openai.com 层（上号器窗口有 usc_/unified_session_manifest/oai-client-auth-info@auth）。**修法候选**：注入改 `domain: .chatgpt.com`，或注入后首屏加载完删掉 host-only 副本，或直接走已有的 `extension-session-runtime.js`（上号器扩展路径）。查清前不上真单；②预检租约默认已改 900s（`run-live-pool.sh`），只在 09-09 第 4–5 次预检验证过"不再超时"，未在成功路径验证；③`browser_run_events` 同一任务多次重试只落第 1 次（job_id+sequence 唯一键），后续尝试只在本机 WAL；④本机绕过连接池直连写入（09-07/08）遗留问题已清；⑤后台控制事务并发可能 `ER_LOCK_DEADLOCK`；⑥手动卡付款后无独立卡侧扣款证据 | 2026-09-09 15:27 UTC | 本表 + HANDOFF_LOG |
