...
| 可分配卡（正式资格 SQL） | **1 张：`5371`（卡段 `54317796`，库内余额 **$34.28**（13:45 UTC 核，`last_transaction_synced_at` 为空即从未同步，卡台实时值待 highvcc token 刷新后核对），账本用量 0/3，Lemon 09:02 一键开卡开出。**今日该卡跑了 12 单全败（同一客户同一 Session，D-203/D-205），随后 Lemon 于 13:29–13:34 UTC 手动付款一次（D-206，**已确认 Plus，CDK 已锁 `REDEEMED`，13:58 UTC 独立核实**）——**账本行存在但状态错标 `RELEASED`，用量因此显示 0、实际 1；3159 同样错标一笔，见 D-207**）。`3159`（`53211304`）已用尽：余额剩 $2.84（低于门槛 16），`DEPLETED`。**注意 3159 的账本计数停在 2，实际用了 3 次（原因是那一笔状态错标为 `RELEASED`，不是缺行，D-207）**（07:31、09:17 系统付款，09:47 运营手动付款；最后一次收口时 `card_consumption_ledger` 无对应行，见 D-198 续）——**补钱前必须人工核对账本**。**别被库里那 5 张幽灵卡骗**：9839 显示 $50、2911/3241/7428/9354 各显示 $1，但 `source_present=0`、`MISSING_FROM_SNAPSHOT`，系统按 `HELD_FOR_REVIEW` 扣下，不要手动放出来。hnskj 账号 11 张共 $0.40，卡段从未成功过。非终态订单 0（13:45 UTC `state-check.sh` 核） |
