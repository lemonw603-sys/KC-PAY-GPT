...
| 可分配卡（正式资格 SQL） | **1 张：last4 `3159`，卡段 `53211304`，余额 $34.28**。2026-09-13 07:32 完成第一个真实客户单 `PJV1-XrDZCfpakRwaoRQLrEbS`（实扣 $15.72），07:51 手动触发快照同步后余额回填、重回 `AVAILABLE`。**用量 1/3**（`card_max_successful_payments=3`），按 $15.72/单还能跑 **2 单**。**从 D-195 起不必再等小时级同步**：跑完一单执行器会立刻触发一次正式快照同步（约 2.5 秒）。**别被库里那 5 张幽灵卡骗**：9839 显示 $50、2911/3241/7428/9354 各显示 $1，但 `source_present=0`、`MISSING_FROM_SNAPSHOT`，卡台快照里已无这些卡，系统按 `HELD_FOR_REVIEW` 扣下，不要手动放出来。hnskj 账号 11 张共 $0.40，卡段从未成功过。highvcc 钱包 $271.73。非终态订单 0 |
