...
|---|---|---|---|
| 生产 release | `/opt/pojia/releases/20260911-sync-skip-msg-56c5290`（commit `56c5290`；含 D-168 卡段标注、D-169 卡台降频与默认卡段留空、D-170 字段名修正）。回滚 `20260911-sync-lastfour-11dbf3c` | 2026-09-11 14:39 UTC | `deploy-release.sh prepare`（981 文件 manifest OK、备份 OK）→ `switch`；服务器本机独立 curl：served admin.js 含 RESOLVE_UNKNOWN_PAYMENT×8 / 确认核实结果×3 / 严格布尔发送×1，service 文件含 strictBoolean 与 MANUAL_VERIFICATION_RESOLVED 证据检查，控制路由未登录 401，live/ready 200，web 无错误日志 |
