...
| 可分配卡（资格 SQL，Plus 门槛 16） | **0 张**；非终态订单 1（Dqcnq），7402 持有 ACTIVE 分配。**已知系统缺口**：`card-inventory-eligibility.js:10` 对 `sync_tier=MANUAL_IMPORT` 跳过新鲜度检查直接信任静态 `current_balance`，highvcc 卡入库即标 MANUAL_IMPORT（`highvcc-card-service.js:196`）→ Dqcn 09-10 14:29 UTC 建单时分到的 7402 实际已只剩 $1.08。state-check 在 0 张时误报 1 张（NULL 计数 bug，脚本未修） | 2026-09-11 00:5x UTC | 资格 SQL 直接查 0 行；代码行号见前 |
