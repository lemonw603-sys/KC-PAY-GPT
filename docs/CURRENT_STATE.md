...
| HNSKJ 卡 | `5276` AVAILABLE/active，余额 $16（2026-09-17 01:34 新开，段 23）；旧卡 12 张均 FAILED/invalid/REFUND_WATCH 余额 $0。**5276 此刻不合格**：`last_transaction_synced_at=05:00:05`，超过资格 SQL 的 15 分钟窗口（见 read-sync 行的节奏观察）；来单会先 WAITING_FOR_CARD 再按需同步（代码路径 `workflow-repository.js:216-260`，未实跑）。 | 2026-09-17 05:49 UTC | cards 独立 SELECT + 生产 release 的 eligibleInventoryCardSql |
...
| 可分配卡（正式资格 SQL） | 1 张：**`29bb`（103 manual_excel，$145，用量 0）**，不是 5276（见 HNSKJ 卡行）。Plus 现走 API 冻结 101，29bb 是 103 的卡、API 单用不到；非终态订单 0。**`state-check` 只比"1 张"这个数，分不出是哪张。** | 2026-09-17 05:49 UTC | 生产 release 的 `card-inventory-eligibility.js` 生成 SQL → 隧道新连接查（与 state-check 同法，多列出卡号） |
