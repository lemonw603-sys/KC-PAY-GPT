...
| 可分配卡（正式资格 SQL） | **1 张：`1657`**（backup-a / highvcc，$18.56，Plus 门槛 16）。`1652`（hnskj）**已 DEPLETED，余额 $0.01**——Lemon 09:02 前补进的 $50 跑完一单后，剩余 $34.57 **由 Lemon 手动退回卡台钱包**（09:12–09:17 UTC；此前标"未查清"，2026-09-14 Lemon 确认是他操作，D-218 补记已订正）。**API 路线（冻结卡源 hnskj）当前 0 张可分配，且 hnskj 卡台今日服务器故障、开不了卡**（Lemon 2026-09-14 报），新 Plus 单走 API 会停在 `WAITING_FOR_CARD`；Browser 路线有 `1657` 一张可用。非终态订单 0。5 张幽灵卡仍 `source_present=0`、`HELD_FOR_REVIEW`，不要放出来 | 2026-09-14 15:05 UTC | 资格 SQL 经 prod-query 实跑；34.57 由 Lemon 确认；hnskj 故障由 Lemon 报 |
