...
|---|---|---|---|
| 生产 release | `/opt/pojia/releases/20260911-cdk-return-fix-bfacbe1`（commit `bfacbe1`；F-48/D-156：人工核实未扣款后退回 CDK。前序 `0396bb8` = A2/D-150 预检不点 Upgrade、重试 1）。回滚 `20260911-preflight-noupgrade-0396bb8`；备份见发布日志。`pojia-worker` 已随本次发布重启（06:56 与 08:5x） | 2026-09-11 09:14 UTC | `deploy-release.sh prepare`（981 文件 manifest OK、备份 OK）→ `switch`；服务器本机独立 curl：served admin.js 含 RESOLVE_UNKNOWN_PAYMENT×8 / 确认核实结果×3 / 严格布尔发送×1，service 文件含 strictBoolean 与 MANUAL_VERIFICATION_RESOLVED 证据检查，控制路由未登录 401，live/ready 200，web 无错误日志 |
...
| 最近真实单 | `PJV1-9TN0gGX-I5rRdhXxLrq7`（2026-09-11 07:18 提交，账号 shichuan003@gmail.com，Pilot 窗口，卡 9839）：预检一次过且不建 Checkout；付款点击一次后结账页弹 hCaptcha，未完成、**未扣款**（账号仍 free、卡台余额未变）；08:2x 用 `resolve-unknown-payment.mjs` 记 NOT_CHARGED 收口 → CLOSED/`HUMAN_VERIFIED_NOT_CHARGED`，卡 9839 回池；CDK 经 F-48 修复后已退回 AVAILABLE | 2026-09-11 09:14 UTC | 生产主机 dry-run + 真跑输出；隧道新连接独立 SELECT（orders/tasks/cdks/cards/card_assignment_history） |
...
| 已上线（本次 release） | `20260911-cdk-return-fix-bfacbe1`：人工核实「未扣款」收口后 CDK 会真正退回（此前被提交点击证据静默拦下）。半自动人机验证接力代码已合并（browser-mvp，本机运行，不经服务器发布） | 2026-09-11 09:14 UTC | 服务器本机 curl 复验（见 release 行） |
