...
| 可分配卡（正式资格 SQL） | **1 张：`1657`**（backup-a，$18.56）。`1652`（hnskj）**已 DEPLETED，余额 $0.01**——Lemon 09:02 前补进的 $50 跑完一单后，剩余 $34.57 于 **09:12–09:17 UTC 被退回卡台钱包**（流水 `CARD_BALANCE_RETURN`，系统 10:00:12 读到）。**代码里没有任何发起退余额的写路径**（`grep -rn "BALANCE_RETURN\|returnBalance" v1/src browser-mvp/src` 无结果），worker `PROVIDER_CARD_WRITES_ENABLED=false`，系统只是读到结果——**是谁退的未查清**。**因此 API 路线（冻结卡源 hnskj）当前又没有可分配卡**，新单会停在 `WAITING_FOR_CARD`。非终态订单 0。5 张幽灵卡仍 `source_present=0`、`HELD_FOR_REVIEW`，不要放出来 | 2026-09-14 11:41 UTC | 生产 release 的资格 SQL 经 prod-query 实跑；card_transactions / provider_balance_snapshots 独立核对 |
| HNSKJ 卡台 | 可用。钱包余额 **$106.09**（11:38 快照；今日 $18.25 → 07:02 $121.77 → 08:47 $71.52 → **09:17 $106.09**，最后一跳是 `1652` 退回的 $34.57）。卡台共 14 张卡（13 active），`providerOnlyActiveCount=0`。**卡余额只能靠同步读到，手动补钱后要等一次 `card-read-sync`**（今日实测滞后 5 分 34 秒，由订单的按需同步触发）。今日 06:40、06:43 两次 `card_recharge`（自动补余额）HTTP 400 `DEFINITE_FAILURE`/`DO_NOT_RETRY`，**原因未查** | 2026-09-14 11:41 UTC | 卡台只读原始响应 + provider_calls + provider_balance_snapshots |
