...
### 本地未部署的体验收敛（2026-08-31）

- 工作区新增 Session 粘贴尾部安全整理、提交结果不明防重复、CDK 查询恢复，以及后台 CDK 生成复用已登录会话（生成保留审计/确认，下载等高敏操作仍 step-up）。
- 生产 `/opt/pojia/current` 未改变；线上静态资源 hash 仍为本次改动前基线。必须安装依赖后跑完整 v1 测试，再按独立发布流程部署和做浏览器验收。
