...
> **完整发布候选状态（本地，未部署）**：提交 `8012da8` 新增 `scripts/build-production-release.sh` 与 `scripts/verify-production-release.sh`，强制单一 commit 的 `git archive` 和全量 tracked-file manifest，旧自动开卡 timer 部署单元已删除。正常候选 825 文件校验通过；篡改、缺失、额外文件反例均被 verifier 拒绝。候选位于 `artifacts/release-candidate-20260906-8012da8/`；生产 current、migration 和服务尚未切换。
...
用户确认上一轮完整运营流程。工作线状态表已将 UI-02、IMP-02、PAY-03、REC-01/02 转为已确认方向。对抗审查发现 AR-01～AR-06，核心是当前本地备用卡代码仍写死单源并自动混选、完整快照语义与跨来源实体卡去重/并发锁尚未成立。报告：`docs/2026-09-06_card-source-operating-design-adversarial-review.md`。尚未进入 D2 最终冻结或代码实现，生产不变。
