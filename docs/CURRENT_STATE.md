...
|---|---|---|---|
| 生产 release | `/opt/pojia/releases/20260910-highvcc-bookmarklet-1738627`（commit `1738627`，highvcc token 一键刷新书签脚本（bookmarklet，URL fragment 传递，不经服务器）+ 修掉上一版遗留的 `public-isolation.test.js` 资产隔离违规（裸 `href="https://..."`，部署前漏跑全量测试）；上几版内容叠加在内；**F-16+F-3 的 `RESOLVE_UNKNOWN_PAYMENT` 后台能力已在 `487b51a` 提交但尚未随任何 release 部署**；无新迁移；回滚点 `20260910-highvcc-jumplink-e27df13`） | 2026-09-10 12:04 UTC | `readlink -f /opt/pojia/current`；switch live/ready/admin 均 200；文件内容直接核对已复验；本机 v1 全量 598/596（2 败为既有 F-40） |
| 回滚点 | `/opt/pojia/releases/20260910-highvcc-jumplink-e27df13`（再前 `20260910-highvcc-wallet-refresh-9ddd07a`；本版无迁移，直接切回即可） | 2026-09-10 12:04 UTC | 部署记录（switch 输出 ROLLBACK 命令） |
| 最新数据库备份 | `/var/backups/pojia/pojia-20260910T120413Z.sql.gz.enc`，完整性 OK（release prepare 阶段） | 2026-09-10 12:04 UTC | deploy-release prepare 输出 |
| pojia-web | active（09-10 12:0x UTC 随 release 切换重启） | 2026-09-10 12:04 UTC | systemctl |
