...
| 可分配卡（资格 SQL，Plus 门槛 16） | **0 张——余额门槛 16 之上一张都没有，系统当前接不了新单**。`live-pool-supervisor.sh` 每 60 秒判一次条件，会一直停在「可分配卡 0 张且无人占卡」不拉起执行器；客户此时提单会卡在等卡。现存余额（全部 < 16）：`53211304` 的 3118 = $7.77（充值额 $60，已服务 9 次分配、其中 2026-09-13 两单全自动成功各实扣 $15.72）、`53211304` 的 0601 = $1.27、`51398996` 的 5501 = $1.79、`42882000` 的 7402 = $1.08，其余 DEPLETED 在 $0.01–$0.31。**补货认准卡段 `53211304`**（唯一自动成功过的卡段，2026-09-13 两单连续成功均用它）**，别补 `51398996`**（7 次提交 6 次用它全败，D-185）。按单笔实耗 $15.72 估，给 3118 补到 ≥ $16 即可接 1 单。**卡台账上真实只有 5 张卡（highvcc，`sync-highvcc-snapshot.mjs` 每小时同步），总余额 $11.91**：3118 $7.77／5501 $1.79／0601 $1.27／7402 $1.08／0237 $0。**库里另外 5 张是幽灵卡，别拿它们的余额当钱**：9839 显示 $50、2911/3241/7428/9354 各显示 $1，但它们 `source_present=0`、`source_operational_status=MISSING_FROM_SNAPSHOT`，即卡台快照里已经没有这些卡，余额是它们最后一次出现在快照时的读数，系统按 `HELD_FOR_REVIEW` 扣下不分配（`manual-card-import-service.js:229-233`），这是对的。hnskj 账号 11 张卡总余额 $0.40，卡段 `40024200`/`43612081`/`40041606` 从未成功过。**两个卡台加起来 $12.31，一单实耗 $15.72 —— 连一单都不够。** 非终态订单 0 | 2026-09-13 04:34 UTC | 资格 SQL 现场直查；同一口径下 `ready-check.sh`（COUNT）与 `state-check.sh`（修好的计数）双跑一致 |
