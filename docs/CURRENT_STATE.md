...
| 可分配卡（正式资格 SQL） | **2 张**：`5371`（卡段 `54317796`，$50.00，用量 0/3，**2026-09-13 09:02 Lemon 经后台「一键开卡」开出（他本人确认），卡段与已验证的 `53211304` 不同、暂无战绩**）；`3159`（卡段 `53211304`，$34.28，用量 1/3，已服务第一个真实客户单）。**缺卡告警阈值 `card_stock_low_threshold` 已从 0 改为 1**（D-197）——剩最后一张时提醒，而不是等一张不剩。**别被库里那 5 张幽灵卡骗**：9839 显示 $50、2911/3241/7428/9354 各显示 $1，但 `source_present=0`、`MISSING_FROM_SNAPSHOT`，卡台快照里已无这些卡，系统按 `HELD_FOR_REVIEW` 扣下，不要手动放出来。hnskj 账号 11 张共 $0.40，卡段从未成功过。非终态订单 0 |
