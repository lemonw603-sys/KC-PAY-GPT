...
| 可分配卡（正式资格 SQL） | **1 张：`5371`（卡段 `54317796`，$50.00，用量 0/3，Lemon 09:02 一键开卡开出，**该卡段唯一一次使用以 `CHECKOUT_DRIFT` 失败，样本 1 次不下结论**）。`3159`（`53211304`）已用尽：余额剩 $2.84（低于门槛 16），`DEPLETED`。**注意 3159 的账本计数停在 2，实际用了 3 次**（07:31、09:17 系统付款，09:47 运营手动付款；最后一次收口时 `card_consumption_ledger` 无对应行，见 D-198 续）——**补钱前必须人工核对账本**。**别被库里那 5 张幽灵卡骗**：9839 显示 $50、2911/3241/7428/9354 各显示 $1，但 `source_present=0`、`MISSING_FROM_SNAPSHOT`，系统按 `HELD_FOR_REVIEW` 扣下，不要手动放出来。hnskj 账号 11 张共 $0.40，卡段从未成功过。非终态订单以 `state-check.sh` 现场为准 |
