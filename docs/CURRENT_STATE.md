...
| 可分配卡（正式资格 SQL） | **0 张——唯一那张卡 3159 刚服务完一单，`DEPLETED` 待余额同步**。2026-09-13 07:32 UTC 完成第一个真实客户单 `PJV1-XrDZCfpakRwaoRQLrEbS`，实扣 $15.72，卡上应剩 **$34.28**（`current_balance` 现为 NULL，等 `sync-highvcc-snapshot.mjs` 小时级快照回填，回填后自动重回可分配）。**用量 1/3**（`card_max_successful_payments=3`），按 $15.72/单还能跑 **2 单**。**别被库里那 5 张幽灵卡骗**：9839 显示 $50、2911/3241/7428/9354 各显示 $1，但 `source_present=0`、`MISSING_FROM_SNAPSHOT`，卡台快照里已无这些卡，系统按 `HELD_FOR_REVIEW` 扣下，不要手动放出来。hnskj 账号 11 张共 $0.40，卡段从未成功过。highvcc 钱包 $271.73。非终态订单 0 |
