...
| 备用卡（manual_excel） | 7402 $49、ASSIGNED、MANUAL_IMPORT、source_present=1，ACTIVE 分配 1，绑定 Dqcnq 订单；不能分配给另一单。其余卡运营覆盖项本轮未逐张重验 | 2026-09-10 23:23 UTC | cards + card_assignment_history + card_operational_overrides；完整资格 COUNT(*)=0 |
| 可分配卡（资格 SQL，Plus 门槛 16） | **0 张**；非终态订单 1（Dqcnq），7402 持有 ACTIVE 分配。state-check 当前错误显示一张：GROUP_CONCAT 空集返回 NULL，被 awk 算成一项；不得以该脚本代替资格 SQL | 2026-09-10 23:23 UTC | 复用当前 eligibleInventoryCardSql(c,16) 的 COUNT(*)=0；cards + card_assignment_history 独立查询 |
...
| 订单总况 | RECHARGE_SUCCESS 6 / RECHARGE_FAILED 15 / CLOSED 14 / CARD_READY 1；未逐一复核历史订单取消续费状态 | 2026-09-10 23:23 UTC | SELECT status,COUNT(*) FROM orders GROUP BY status |
...
| 最近真实单 | `PJV1-DqcnqHF0tPlxDhygTtAA`：CARD_READY；预检 DEAD，attempts=5/max_attempts=5，CHECKOUT_NAVIGATION_FAILED；ASSIGN_CARD/PREPARE_RECHARGE COMPLETED，SUBMIT_RECHARGE PENDING/0 次；7402 $49 持卡未收口。该单 recharge_attempts 无记录、关联 browser_runs=0。WAL 最后仍为身份核对通过后导航失败；Sentinel 根因未证实。未发客户页面请求、未重跑 | 2026-09-10 23:23 UTC | 生产独立只读 SQL + 本机 lane-4.wal；详见接班核对记录 |
