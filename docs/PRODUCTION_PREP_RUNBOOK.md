# 生产前安全准备运行手册

> 目的：在不执行开卡、直充、退款或余额提取的前提下，完成迁移、备份、Bark 和只读体检。
> 本手册不授权任何 Provider 写调用；出现未知状态时立即停止并保留证据。

## 0. 操作前快照

先验证候选来自一个确定提交且没有旧 release 叠加文件：

```bash
scripts/verify-production-release.sh \
  /opt/pojia/releases/<candidate> \
  /opt/pojia/release-bundles/<candidate>/source-manifest.sha256
```

必须返回 `release_manifest=OK`；出现任意 missing、changed 或 extra 都停止部署。候选只能由 `scripts/build-production-release.sh` 通过 `git archive` 构建，禁止复制 current 后局部覆盖。

以 root 执行并保存完整输出：

```bash
date -u
readlink -f /opt/pojia/current
systemctl is-active pojia-web.service pojia-worker.service \\
  pojia-card-read-sync.timer pojia-highvcc-snapshot-sync.timer \\
  pojia-card-catalog-sync.timer pojia-bark-notifications.service
pojia-ops status
```

同时保存：当前发布清单哈希、`schema_migrations` 版本、应用设置、活动任务/租约、Permit、资金风险尝试和 Bark `DEAD` 数量。

在外部网络执行网络边界检查：

```bash
nc -vz -w 5 ops.vibebridge.top 3306
```

预期结果必须是连接失败或被防火墙拒绝。若公网 3306 可以建立 TCP 连接，立即停止本手册后续步骤，先关闭公网暴露；不得尝试 MySQL 登录。生产机本地的 `127.0.0.1:3306` 才允许可达。

## 1. 进入维护窗口

迁移和恢复演练前，停止可能领取资金任务的进程和定时器：

```bash
systemctl stop pojia-card-stock-runner.service
systemctl stop pojia-worker.service
```

不要停止只读同步，除非供应商限流或数据库维护另有要求。停止后必须确认：

- 活动开卡任务为 0；
- 活动充值 Permit 为 0；
- 活动资金风险尝试为 0；
- 过期租约为 0；
- 不确定 Provider 调用为 0。

如果任一条件不满足，不得继续迁移或启用 Bark，先人工处理并保存证据。

## 2. 备份和隔离恢复

```bash
pojia-ops backup
pojia-ops restore-test
pojia-ops verify
```

恢复测试必须使用无网络临时 MySQL，不得连接生产库，不得启动 Worker。确认备份文件、校验文件和恢复密钥均不进入项目目录；另行确认已有异地加密副本和分离保存的恢复密钥。

### 恢复成功的判据（D-324）

`verify`只证明备份文件校验和、解密与gzip完整；**只有`restore-test`全部功能检查通过才输出`restore_test=OK`**。它不是生产数据库恢复命令。

新版`restore-test`沿用现有临时容器，在`--network none --skip-networking`下导入，调用同版本`v1/scripts/verify-restored-backup.mjs`：

- 读取订单与CDK关联；取最新最多3条Session和3批CDK密文，用`/etc/pojia/runtime.env`中的两把原密钥解密并校验结构。只读白名单键、不执行环境文件、不输出密钥/密文/明文。文件权限接受0600/0640，不接受组可写或其他用户可访问。
- 057之后必须存在准确的告警轮次触发器、两个轮次字段与迁移记录；规则不符、半迁移、未知执行身份、额外存储过程/事件/视图一律停止，不能当作旧版本跳过。
- 仅在本次临时容器中，若缺少已确认的`pojia_migrator@172.17.0.1`，创建**锁定登录**的同名账号，仅授临时库`operator_alerts`表的SELECT/UPDATE/TRIGGER。不恢复原密码、不授全局权限。不改已有账号权限；不足则失败。
- 新建合成告警，关闭再重开，验证轮次1→2，并回滚验证行；不会改恢复出来的真实告警。检查结束无论成功或失败均删除本次临时容器。
- 057之前的备份明确显示`NOT_APPLICABLE_PRE057`，仍须通过业务读取和解密；没有解密样本则显示未验证错误，不假报密钥正确。可用`verify`单独证明文件完整。

若显示`restore_verifier_missing`，说明运维入口和release不配套；不能退回旧的“只数表”检查。正式安装时，从同一已验证release安装`deploy/server/pojia-ops.sh`到现有运维入口，并核对文件哈希。也可从该release直接执行此脚本，它会调用同一release里的验证器。安装/运行生产主机上的恢复演练仍需当次批准与资源检查。

### 真正重建数据库时不能漏掉的步骤

1. 从独立保存的配置/秘密恢复原应用密钥与迁移/运行账号配置，不生成新密钥顶替旧密钥，不将整个`mysql.user`或密码复制进业务备份。
2. 导入后查询`information_schema.TRIGGERS`的DEFINER，与已批准迁移身份逐字核对（包括`@`后的host）。按已批准的目标环境账号方案恢复身份及所需权限；缺失或不一致先停止。**临时演练里的锁定账号不是生产迁移账号配置，不能照抄去锁生产迁移账号。**
3. 在无网络副本上跑上述读取/解密/告警探针。目标生产库上线前再核对账号、权限和服务只读健康，执行器保持停用，恢复营业另行确认。
4. 异地备份可获取性、独立密钥可获取性、应用配置与真实订单链路，仍需分别验收；本脚本的代表性检查不能替代它们。

本机复验：`node scripts/restore-functional-rehearsal.mjs`。使用合成备份，不需要生产凭据；证据在`docs/reviews/2026-09-21-restore-fix/`。

## 3. 执行迁移

只使用迁移专用账号：

```bash
cd /opt/pojia/current/v1
npm run migrate
npm run migrate
```

第二次执行必须只报告 `already applied`。迁移完成后锁定或撤销迁移账号，并保存迁移版本和命令输出。

## 4. 恢复只读服务

迁移确认成功后，先启动 Web 和只读能力；资金 Worker 和卡库存付费执行器保持停止：

```bash
systemctl start pojia-web.service
systemctl start pojia-card-read-sync.timer pojia-card-catalog-sync.timer pojia-highvcc-snapshot-sync.timer
```

`pojia-highvcc-snapshot-sync.timer`（2026-09-11 起）：每 10 分钟把备用卡台 A（highvcc）全部卡片余额走「导入备用卡」正式路径同步进 `cards`，数据未变时按文件哈希重放不写。单元文件在 `deploy/server/`，发布脚本不会安装它；新机器或单元变更时：复制两个文件到 `/etc/systemd/system/` → `systemctl daemon-reload` → `systemctl enable --now pojia-highvcc-snapshot-sync.timer`。需要立刻刷新时 `systemctl start pojia-highvcc-snapshot-sync.service`；失败（多为 highvcc token 过期）只在 `journalctl -u pojia-highvcc-snapshot-sync.service` 可见，后台首页 token 状态会显示过期。

客户接单开关保持关闭，数据库中的 `dispatch_new_recharges` 保持关闭。

## 5. Bark 验证

配置 `/etc/pojia/bark.env` 后：

```bash
systemctl enable --now pojia-bark-notifications.service
```

使用普通内部告警验证：

- 手机收到消息；
- 正文不包含 Session、PAN、CVV、API Key 或完整 CDK；
- 两个 runner 并发时只有一个领取；
- `SENDING` 超时后能够重领；
- `DEAD` 恢复需要显式确认词并留下审计记录。

## 6. 只读上线体检

```bash
PROVIDER_WRITES_ENABLED=false \\
PROVIDER_CARD_WRITES_ENABLED=false \\
PROVIDER_RECHARGE_WRITES_ENABLED=false \\
npm run preflight:readiness
```

然后执行 Provider 只读检查。若 HNSKJ 仍返回已知的 401，立即停止，不重复尝试密钥，也不转入任何写调用。

## 7. 退出维护窗口

仅在迁移、恢复、Bark 和只读体检全部通过后恢复服务。第一笔真实订单前，继续保持：

- `pojia-worker.service` 停止或无可领取资金任务；
- `pojia-card-stock-runner.timer` 停止；
- `card_auto_replenishment_enabled=false`；旧 timer 不得随发布恢复；
- 所有 Provider 写开关关闭；
- 没有活动 Permit、UNKNOWN 或资金风险尝试。

真实订单按 [RUNBOOK.md](./RUNBOOK.md)「1. 来单」执行（单笔、Lane4 单窗口、付款开关当次开当次关）。早期小批量稿已归档：[archive/undated/SMALL_BATCH_RUNBOOK.md](./archive/undated/SMALL_BATCH_RUNBOOK.md)（历史，只作追溯）。
