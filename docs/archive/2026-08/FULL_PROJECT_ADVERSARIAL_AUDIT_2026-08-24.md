...
## 补充：2026-08-24 用户确认与更正

审查落盘后就报告中需用户拍板的四项做了确认，结果与更正如下（更正原报告对应结论，原发现作为"审查当时判断"保留）。

1. **生产状态（更正第一/五/六节的"时间线冲突"）**：用户澄清——**我方生产服务器正常**；阻塞在**卡台（HNSKJ）正在升级服务器**，当前开不了新卡、也无法往新卡充值。原报告的"08-22 生产正常 vs 08-24 服务器未修好时间线冲突"**不成立**：两者不矛盾，"服务器尚未修好"是指卡台侧写能力不可用。**修正结论**：我方 Web/Worker/MySQL 视为可用，可做生产只读体检；真实开卡/卡余额充值/Plus 充值阻塞在卡台写能力恢复，而非我方故障。P0-1 从"判断生产是否可用"降级为"核实我方 release/开关现场值 + 跟踪卡台升级完成"。

2. **决策账本 ID 重复（更正第六节问题1 与 P1-1 的处理方式）**：用户授权重编号。但落实时发现：`git` 对比显示 **D-089、D-109 的重复条目位于其他窗口未提交改动（D-090~D-133 那批 +59 行）中**，仅 D-069 是已提交老问题。按硬规则"不覆盖/不提交其他窗口修改"，**本窗口不编辑 `DECISIONS.md`**。重编号映射建议（待拥有该未提交改动的窗口提交后统一执行，或由用户指定窗口执行）：
   - 行 96「API 充值路径不增加独立账号预检」原误标 D-069 → 建议 **D-134**（保留行 75 的 Browser dispatch 为 D-069）。
   - 行 97「Plus 卡台路线人工切换入口」原误标 D-089 → 建议 **D-135**（保留行 95 的"节点后对抗审查"为 D-089；注意此条本身是其他窗口新增）。
   - 行 153「可生成内部测试 CDK」原误标 D-109 → 建议 **D-136**（保留行 121 的 repository 网络故障为 D-109；两条均为其他窗口新增）。
   - 仅改编号、不动语义，并在"依据"列注「原误标 D-0xx，2026-08-24 重编号」。

3. **接单/派发联动（第六节问题3）**：用户回复**待议**。本窗口不改动，保留为待确认项：当前"开始接单"联动打开派发、"停止接单"不关派发；如需"只接单不自动派发"，暂用 `recharge_dispatch_mode=MANUAL` 隔离。

4. **后台导航收敛（更正第九/十二节 P2-1）**：用户明确——**不采纳** `ADMIN_CONSOLE_SIMPLIFICATION_2026-08-24`（该文为另一窗口的优化意见），要求由本执行者按实际情况独立评估，不受其方案干扰。**修正**：撤销"按前序方案推进"的表述；后台结构改动改为"由接班执行者基于独立代码/数据/运行核查另行提出"，且在此之前不做任何后台导航/字段删除合并。第十节"必须保留的安全和追溯能力"仍然有效，与本条不冲突。

其余 P0/P1/P2 结论不变。本补充节不新增任何 P/D 级生产证据。
