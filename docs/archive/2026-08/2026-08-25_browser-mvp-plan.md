...
由统筹窗口评审 `docs/archive/2026-08/2026-08-25_browser-shared-contract-compat.md` 中的未决合同；在 `recharge_attempts`、资金 permit 和审计关联正式冻结前，Browser 线保持只读 projection，不接真实 Session、Checkout、卡片、付款、Provider 写入或生产 release；不 cherry-pick 混合检查点，不使用 `git add -A`，不清理 `.playwright-cli/`/`artifacts/`。
