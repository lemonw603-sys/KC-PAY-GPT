...
审查后的第一步已实现于本地：自动开卡任务在分卡发现无合格卡时按订单需求幂等排队；自动调度增加 `WAITING_FOR_CARD` 需求门槛，库存定时器模板从 10 秒降为 60 秒。订单驱动补余额、完成后即时唤醒和生产部署尚未完成。

随后已补上订单关联的余额补充触发：分卡发现存在合格但余额不足的候选卡时，按订单创建唯一 `card_funding_attempts(PREPARED)`，并让资金 runner 可处理这类显式订单任务，即使旧的全局低余额扫描开关仍关闭。资金 runner/定时器的生产启用和补余额后的即时唤醒仍待部署前验证。
