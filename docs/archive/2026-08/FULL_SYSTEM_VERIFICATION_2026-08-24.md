...
- 历史早期尝试读取到 279 字节非 JSON 文本；该轮不能证明真实 Session 内容，后续 7430/7409 字节带 19 字符尾部的读取结果才是有效 Session 诊断证据。
...
## Session/开关风险审查（非用户新决策）

- 技术安全判断：Session 属于敏感凭证，报告应只记录脱敏元数据和错误，不记录原文；这不是用户新增业务规则。
- 测试建议：接单开启但派发关闭可用于验证订单创建；是否执行需单独确认。
- 测试建议：派发开启但 Provider 写入关闭仍可能产生任务、租约、卡片分配或等待状态。
- 技术审查提示：真实资金风险不能只看接单/派发开关，还需核对 Provider/卡台写入和资金栅栏。
