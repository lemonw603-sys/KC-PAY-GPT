...
## 客户状态阶段性对抗式审查（2026-08-23）

### 结论

- 没有发现客户侧把库存/卡台内部原因错误暴露为客户错误的代码路径；`WAITING_FOR_CARD` 明确映射为处理中。
- Plus 已有账号的 Provider 40030 已能在客户状态查询中映射为可行动的 `ACCOUNT_ALREADY_PLUS`。
- Session 无效/过期进入可更换 Session 的动作状态，替换次数由服务端限制；未知和对账状态不会映射为成功。
- 当前生产接单、派发、Provider 写入和卡余额充值均关闭；Web/Worker、只读同步、只读对账和健康检查正常。

### 仍需保留的边界

- `WAITING_FOR_CARD`、`WAITING_FOR_SESSION`、`UNKNOWN` 等没有当前生产活动样本，不能宣称真实生产链路已验收。
- 生产仍有一个内部测试 `CREATED` 订单和一个历史对账问题，不能用当前总览的“处理中/复核中”数字推导正常客户业务成功率。
- 浏览器逐页视觉验收仍未完成；本轮是 API、数据库、代码和测试核对。
...
- 后台订单详情保留原始失败字段，并对历史缺失字段提供只读 Provider 证据回退；客户状态接口提供可行动的 Plus/Session 原因。
- 未映射的 Provider 代码仍保留原始代码，不被静默改写。
- 当前没有发现 CDK、订单、卡片、Provider 调用、卡片交易和失败/退款记录之间新的断链证据。
- 本阶段完成的是 API、数据库、代码和测试核对；没有生产写入，也没有宣称视觉逐页验收或成功充值验收。

### 本轮最终回归结果（2026-08-23）

- 在当前工作树重新执行 `npm test --silent`：407 tests，373 passed，0 failed，34 skipped。
- 跳过项均为未配置 `TEST_DATABASE_URL` 的隔离 MySQL 集成测试；因此不能把本轮结果表述为生产数据库集成验收。
- `git diff --check` 通过。
