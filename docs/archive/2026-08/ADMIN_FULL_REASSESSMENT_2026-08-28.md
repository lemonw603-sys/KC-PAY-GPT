...
## 2026-08-29｜“后台有卡仍提示缺卡”修正

- 生产核验确认新卡 `1628/6185` 真实存在：Provider active、卡段 17、余额 `$5`、卡资料完整。
- 原因一：系统曾把默认开卡卡段 16 错当成唯一合法卡段，导致卡段 17 被标记 `CARD_TYPE_MISMATCH`。现改为接受卡台当前公布的全部合法卡段；默认卡段仅保留为未来开卡偏好。
- 原因二：后台把“立即可分配卡为 0”和“没有卡”混为一谈。现区分“可直接分配”和“已有卡、补余额后可用”；人工库存模式下不显示低库存告警。
- 原因三：只读卡同步曾用开卡金额代替 Plus 最低余额判断，可能把 `$5` 卡标成 AVAILABLE。现统一使用后台配置的 Plus 最低余额，`6185` 正确显示为“余额不足，充值后可用”。
- 卡余额充值调度与资金栅栏现在可以识别 `DEPLETED/PROVISIONING` 的可补余额卡；仍不会在写开关关闭时执行真实卡充值。
- 订单分配不再要求卡片卡段等于订单快照里的默认开卡卡段；仍保留 Provider 账户、余额、资料、新鲜交易同步、历史消费和运营覆盖等资金安全条件。
- 生产最终核验：`6185` 已接管，本地状态 `DEPLETED`，待补余额卡 1，开放低库存/等待卡片告警 0；接单、派发、Provider 写入、Browser 付款、自动开卡和卡余额充值均保持关闭。
