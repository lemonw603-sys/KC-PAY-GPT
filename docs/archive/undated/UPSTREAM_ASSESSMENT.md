...
根目录默认启动权已经收回：`npm start` 进入 v1；旧 Node 服务只能通过 `npm run start:legacy` 启动，旧 Docker 文件使用 `.legacy` 文件名，不再被默认 Docker 命令发现。
...
2. ~~建立 v1 数据表、状态枚举、任务租约和事件日志。~~ 已在 MySQL 8.4.11 完成迁移与集成验证。
3. ~~编写两个 Provider 及纯 fixture 合同测试。~~ 已完成，真实响应仍待验证。
4. 接入 Provider 调用记录、CDK与客户提交页面。
5. 接入内部订单、异常和退款页面。
