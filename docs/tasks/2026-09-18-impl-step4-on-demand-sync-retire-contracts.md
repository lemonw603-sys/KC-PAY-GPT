...
- 第③步已把水位/切换校验改成不看新鲜度的库存口径（D-259）；本块动的是分卡那一侧，**资格规则 `eligibleInventoryCardSql` 本身不改（Lemon 2026-09-18 选 a，D-266）**：新鲜度条件保留，靠当场同步让卡进窗口；不做 b。
...
- 最短存活期：按 D-228 补充二先配 6 小时、可调，每次实际销成功的时间记下来（Lemon 2026-09-18 确认沿用，不再问）。
