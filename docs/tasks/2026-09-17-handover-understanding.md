...
2. **可分配卡不是 5276**：用生产 release 那份 `eligibleInventoryCardSql`（门槛 16）跑出的合格卡是 **29bb（manual_excel，$145，用量 0，无 override）**；5276 因 `last_transaction_synced_at=05:00:05` 超过 15 分钟窗口此刻不合格。CURRENT_STATE 写"1 张：5276"；`state-check` 只比"1 张"这个数所以报一致。**含义**：Plus 现在走 API、冻结 101，29bb 是 103 的卡，API 单用不到；来一单会先 WAITING_FOR_CARD → 排按需同步 → 5276 刷新后 5s 重试分配（代码路径 `workflow-repository.js:216-260`，**未实跑过**）。
