...
## 6. Lemon 的回答（2026-09-17）与方案修订

**八答**：①单量少、卡台资金少，每台 2 张 Plus，Pro 近期不预留，问有没有更好办法；②Plus $50，5X $100，20X $150，Pro 不想让资金躺卡里；③卡台上限是每天在线 100 张、销后可再开，单量大可提；④hnskj 钱包不得低于 $30、highvcc 不得低于 $20，基线「250」是错的；低于 $50 只告警；⑤hnskj 故障时倾向自动开 highvcc 顶上；⑥不过期的 key 拿不到（两个 AI 试过）；⑦hnskj 12 张空卡已被卡台作废（换服务器）；highvcc 5 张是 Lemon 注销的、余额已提回钱包；⑧认可水位驱动。

**对①的回答——更好的办法不是少保几张，是让钱躺得短**：
- 钱占用 = 卡上金额 × 卡存活时间。每台 2 张 Plus = $200 是稳定占用，不大；真正浪费是「用满了还挂着」。所以资金效率的杠杆在 **4.5 销卡待办要做实**：用满 3 单立刻进待销清单，到卡台最短存活期就提醒你删，钱当天回钱包再开。这样 $200 循环着用，不是越保越多。
- Pro：**水位 0、不预留**，保留订单驱动兜底——Pro 单来了缺卡，调度器当场按产品金额开一张（5X $100 / 20X $150），客户等一次开卡（几十秒到几分钟，卡台正常时）；客户页要给「准备中，预计 X 分钟」。Pro 卡一单一卡、用完即进待销，钱回得也快。
- 水位数和是否预留都放设置页按台按产品可调，单量涨了改数字不改代码。

**修订**：
- 4.1 水位：Plus 每台 2；Pro 每台 0（订单驱动开）；开卡金额 Plus 50 / 5X 100 / 20X 150（替代 A3 的「20X 按单价」）。
- 4.1 日限：改为「每台每日开卡上限」当资金保险丝，建议每台 20 张/天起步（可调）；卡台的 100 张在线上限另计（销卡回笼后不占）。
- 4.1 钱包预检：开卡前 `钱包余额 − 开卡金额 − 手续费 ≥ 卡台硬底线`（hnskj 30 / highvcc 20）才开，否则不开并推手机；另设告警线 $50 只告警不停。基线「HNSKJ 开卡要求账户余额 250 以上」**作废**。
- 4.2 故障切换：本台连续开卡失败 N 次（或卡台返回维护/5xx）→ 标本台「故障」→ 该产品的补给需求自动转另一台。**前置**：Browser 路线现在就成立（两台卡都能走）；API 路线要等 C2 验过 highvcc 卡走 ZZSHU 后才成立，之前 hnskj 故障时 API 只能告警。
- 4.2 token：接受人工点。token 过期 → highvcc 开卡/同步失败 → 推手机「贴 token」，不静音。
- 4.5 两批旧卡：落实时用正式脚本把 hnskj 12 张标「卡台已作废」终态、highvcc 5 张标「已注销、余额已回钱包」终态，不再 HELD_FOR_REVIEW；两批都不用再处理资金。
- 4.6 不变。

**我理解的这一面的决定（请核对）**：
- 供卡改为水位驱动 + 订单兜底；Plus 每台保 2 张，Pro 不预留来单即开；开卡 50/100/150。
- 三条开卡线合一：hnskj runner 挂定时、highvcc 接调度、worker 死线删、930 条残留归档。
- 两台都有库存眼睛：告警按台按产品；highvcc 钱包入库；余额变化通知。
- 钱包底线 hnskj 30 / highvcc 20 是硬预检，$50 是告警线；每台每日开卡上限起步 20。
- hnskj 故障自动转 highvcc（Browser 即刻、API 等 C2）。
- 分卡时当场同步过期卡，去掉每小时 15 分钟合格窗口。
- 销卡待办做实（用满即进、到期提醒、手动删），是资金效率的主杠杆。
- 删补余额整条线；两批旧卡标终态。
