...
**最新对齐（22:51 UTC+8）**：用户认可D-338合并结果，尚未授权发布。下一项成功率建议待确认：默认近7个北京时间日历日提交且已完结的订单，明确演练排除，进行中不算失败，普通未成功关闭仍沿现口径计入；显示成功数/完结样本数，零样本“—”，历史保留可查，不混同自动完成率。14:51:28 UTC再次SELECT orders按status分组仍20/37/21；created_at在09-14 16:00～09-21 16:00 UTC的5单为1成功/1失败/3关闭。此前detail.sql已证3关闭均明确演练，排除后仅2样本，不把50%当稳定性证明。当前仅建议，未改代码。
