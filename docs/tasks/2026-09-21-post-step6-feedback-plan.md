...
| FB-08 | 订单是否有大量脏数据、是否清理 | 78条已完成第一轮分类；没有支持“大量结构性垃圾”的证据。其中7笔续费待核是待履行义务，不是脏数据；2条RECONCILIATION和16个旧CDK归属仍待核 | 给可复核分类清单；真实交易/资金链不删；统计过滤与物理清理分开；每批有具体预览和用户确认，前后数量/关联/审计验证 | 部分已验证；不获准删除，剩余三类继续收口 |
...
**D-343简化历史测试数据**：Lemon确认16个旧CDK和2条待对账单都是自己测试，且CDK尚未正式对客户开放。撤销三分类归属方案，改为固定名单一次性收口：订单/事件保留，16个CDK作废不恢复可售，2条未扣款RECONCILIATION释放。不做新页面/长期规则；下一步是dry-run预览与审批，未执行生产写入。

1. 固定16个订单/CDK与2条ledger名单，执行前重新校验状态、绑定关系及无PURCHASE证据。
2. 先给dry-run预览；批准后备份目标行，用单事务将16个CDK改REVOKED、2条ledger改RELEASED并写审计，再用新连接独立复核。
