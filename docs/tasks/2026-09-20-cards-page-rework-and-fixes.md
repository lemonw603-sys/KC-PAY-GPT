...
> **2026-09-20 晚更新（交新窗口前的事实修正，不改下方原文）**
>
> 1. **A 部分（A1~A4）已经全部做完了**，提交 `9a7c3fa`（三处错误 + 删掉重复造的第四处）与 `f2134a4`（今日花费口径）。代码实证：`admin-read-service.js:601` 有 `SUM(o.status='WAITING_FOR_CARD')`、`card-inventory-eligibility.js:242` 有按产品的 `product_used`、`admin-read-service.js:742` 开卡费按 `first_seen_at` 且含 `chargeback`。**新窗口不要再动 A 部分**，下方 A1~A4 只作背景阅读。
> 2. **B1 的硬前置已经满足**：设置页两项都有了——最低余额 `index.html:198` 的 `#minimum-balance-form`（按产品三行），每卡单数 `admin.js:1719` 已改为可编辑（提交 `3c435ea`）。所以「删掉卡片页块 2」不再有阻塞，可以直接做。
> 3. **全量测试基线已变**：不是 976 / 908 pass，是 **985 / 917 pass / 1 fail**（唯一 fail 仍是既有 F-65，不许为全绿改它）。另注：那条 F-65 测试目前实际红在第一条断言（版本号写死 `admin.js?v=61` 而实际已 `v=70`），后三条守门断言没执行——**改 admin.js 后记得同步 bump `index.html` 里的 `?v=`**，这是本仓库惯例。
