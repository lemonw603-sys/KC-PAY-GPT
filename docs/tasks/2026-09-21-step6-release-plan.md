# 第⑥步统一发布方案（D-328已批准执行）

> **执行完成：2026-09-21 07:52 UTC独立复核，第⑥步已发布为20260921-step6-9b9f181。** 055～058、日对账真实持久化、生产备份隔离恢复、新ops安装通过；原接单与9timer恢复，临时SUPER撤回。实际结果见`../reviews/2026-09-21-step6-release-v2/report.md`。下文保留方案/历史停止记录，不可当作尚未执行或再执行授权。

> **D-328最新**：用户已批准容量修复、验收后继续发布。固定候选`9b9f1812a709ca54b8f1b49a48a2e4c8a2669ad8`，release名`20260921-step6-9b9f181`，包含058及已证实旧摘要格式兼容；真实MySQL10项、默认981/0/69通过。迁移范围**055～058**，保留只存摘要逻辑。先用旧备份做候选恢复检查；迁移后Bark仍停时跑一次原日对账，独立读回摘要/心跳/告警，再备份恢复并switch。以下66bfe98/8395fe3均为历史候选，不复用目录。

> **2026-09-21 D-327执行结果覆盖**：已批准推进并完成66bfe98的推送/prepare，进入维护后发现日对账报告无法写入VARCHAR(255)，在迁移前中止并恢复原⑤b营业。055～057、临时SUPER、switch和新ops安装均未执行。下面保留已批准原方案；**修复容量并重新确定新增迁移范围前，不继续发布该准备包**。执行证据见`../reviews/2026-09-21-step6-release/report.md`。

更新时间：2026-09-21，UTC+8。范围＝工作台/CDK/卡片/设置、已确认的诊断收敛及其后端修复，不只CDK。**本文是拟执行方案，不是已发布记录，也不构成执行授权。**

## 候选与依据

- 固定代码候选：`66bfe98fdd35c84e4d60e081febcef31b6d6ff35`（包含D-324恢复检查与D-326诊断真实接线；旧5e7e717不含诊断修正）。
- 拟release名：`20260921-step6-66bfe98`；prepare前检查同名目录不得已有不一致内容。
- 决策D-313～D-326；既有三份1440契约及新诊断9项契约/变异；联合核查、修复、迁移/恢复演练证据见docs/reviews下2026-09-21相关报告。D-323付款开关保持原样。
- 默认测试979 pass/0 fail/69 skipped；此前28迁移场景、6类订单恢复场景、14恢复检查场景，加本次24项隔离检查（含4条诊断正式收口和4次旧接口409不变性）通过。不是所有生产资金动作/全项目MySQL套件全绿。
...
- 诊断新增资源admin.js?v=84、diagnostics.js/css?v=1须与manifest一致，诊断只读报告/卡台名称/案例列表可读取。**不用真实case发关闭请求测试409，不改历史OPEN案例**；后端保护用已验隔离证据和生产源文件hash核对，避免缺陷本身让测试误写生产。
