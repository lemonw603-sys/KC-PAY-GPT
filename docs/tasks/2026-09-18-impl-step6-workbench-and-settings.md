...
> **✅ 工作台已选定 C「看板优先·精修」实施中（2026-09-18，D-283）**：三版比稿 `docs/design/prototypes/step6-workbench-compare.html`；删两路线耗时、订单整宽、**留今日订单表**、待处理在数字墙下方。导航＝6 个一级页（保留订单页）。主体复用现有 overview 取数，3 个新看板数（自动完成率/今日花费按台/按台按产品卡用量）待新写聚合，自动完成率口径待 Lemon 定。
