/* =========================================================================
   app.js — 重设计原型的交互逻辑（视图状态机 + 客户契约消费）。
   关键不变量：
   ...
   本原型通过 window.MockBackend 调用假后端；生产改为对应 fetch 即可。
   ========================================================================= */
(function () {
  'use strict';

  const api = window.MockBackend;

  // ---- 客户可见 7 状态的展示文案（语义对齐 v1 现有 STATUS，措辞打磨） ----
  const STATUS = {
    QUEUED:          { label: '已排队',   title: '订单已创建',           desc: '系统已收到订单，正在排队等待处理。', terminal: false, poll: 2500 },
    PROCESSING:      { label: '处理中',   title: '正在为你开通 Plus',    desc: '订单已进入处理流程，请不要重复提交，稍候即可。', terminal: false, poll: 2500 },
    REVIEWING:       { label: '复核中',   title: '订单正在复核',         desc: '系统需要进一步确认结果，请保留查询码稍后查看。', terminal: false, poll: 8000 },
    ACTION_REQUIRED: { label: '待更换账号', title: '需要更换账号 Session', desc: '当前账号不符合开通条件，请在下方更换一个免费账号的 Session。', terminal: false, poll: 15000 },
    FINALIZING:      { label: '收尾中',   title: '正在确认取消自动续费', desc: '付款已确认，系统正在关闭自动续费，马上完成。', terminal: false, poll: 3000 },
  ...
  const el = {
    ...
    inputForm: $('input-form'), cdk: $('cdk'), session: $('session'),
  ...
  function errText(err) {
    if (err && err.code) return ERRORS[String(err.code).toLowerCase()] || '操作未完成，请稍后重试。';
    if (err && err.message === 'network_error') return '无法连接服务，请检查网络后重试。';
    return ERRORS[err && err.message] || '操作未完成，请稍后重试。';
  }
  ...
  function setStepper(step, done) {
    ...
    const pct = done ? 100 : ({ 1: 16, 2: 58, 3: 100 }[step] || 16);
  ...
  el.replaceForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      pollStart = Date.now();
      renderStatus(order, { scroll: true });
  ...
  const SAMPLE_SESSION = JSON.stringify({
    user: { id: 'user-8f3a2b', email: 'lin.zhao@gmail.com', name: 'Lin Zhao' },
    account: { id: 'acct-2c91e0' },
    accessToken: 'SAMPLE.ACCESS.TOKEN',
    sessionToken: 'sample.session.token.jwe.value',
    expires: '2099-01-01T00:00:00.000Z'
  }, null, 2);

  function demoPrefill() {
    el.cdk.value = 'PLUS-8KQ2-D7M4-2F19';
    el.session.value = SAMPLE_SESSION;
    fieldError(el.fieldCdk, ''); fieldError(el.fieldSession, '');
    showView('input', { step: 1 });
  }
  function demoConfirm() { demoPrefill(); el.inputForm.requestSubmit(); }
  function demoStatus(cs) {
    stopPoll();
    const order = api.demoForceStatus(cs);
    successShownFor = null; pollStart = Date.now();
    showView('tracking', { step: 3 });
    renderStatus(order);
  }
  async function demoAutoplay() {
    stopPoll(); demoPrefill();
    const { order } = await api.createOrder({ cdk: 'PLUS-AUTO-' + Date.now().toString(36).slice(-4), session: JSON.parse(SAMPLE_SESSION) });
    el.session.value = ''; pollStart = Date.now(); successShownFor = null;
    showView('tracking', { step: 3 });
    renderStatus({ publicNo: order.publicNo, status: 'QUEUED', updatedAt: new Date().toISOString(), timeline: [] });
    toast('演示：订单将自动推进到成功。', 'info');
  }

  el.demoOpen.addEventListener('click', () => { el.demo.hidden = false; });
  el.demoClose.addEventListener('click', () => { el.demo.hidden = true; });
  el.demo.addEventListener('click', (e) => {
    const b = e.target.closest('button'); if (!b) return;
    if (b.dataset.theme) return setTheme(b.dataset.theme);
    const d = b.dataset.demo; if (!d) return;
    if (d === 'input') { stopPoll(); showView('input', { step: 1 }); }
    else if (d === 'prefill') demoPrefill();
    else if (d === 'confirm') demoConfirm();
    else if (d === 'query') openQuery();
    else if (d === 'autoplay') demoAutoplay();
    else demoStatus(d);
  });

  // ---------------- 仅原型：自动化/截图编程钩子（生产不含） ----------------
  // 供 Playwright 截图脚本或手动控制台调用；不参与任何真实业务。
  window.__proto = {
    view: (name, o) => showView(name, o),
    status: (cs) => demoStatus(cs),
    prefill: demoPrefill,
    confirm: demoConfirm,
