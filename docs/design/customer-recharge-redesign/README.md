...
| 6 | **成功** | tracking / SUCCESS | 就地打勾勋章（一次性描边动画）；成功摘要（充值账号 + 完成时间）；成功摘要下方附「去 ChatGPT 官网确认订阅」官方外链；查询码；完整时间线；**不弹窗** | `*-04-success`（含 `-dark`） |
...
| 9 | **Session 获取教程** | 弹层（覆盖任意视图） | 输入页与更换处「如何获取 Session？」入口触发；分步引导 + 两个官方外链（登录、Token 页面）+ 脱敏 JSON 示例 + 安全提示 | `*-08-guide` |
...
└── screenshots/       桌面/移动 × 8 状态 + 2 张深色成功（共 18 张）
```

---

## 9. 增补：Session 获取教程与订阅确认（2026-09-01）

### 9.1 Session 获取教程

获取 Session 是客户完成充值的最大障碍，现有页面对此零引导。本次新增图文教程，直接服务「减少理解成本」：

- **入口**：输入页「账号 Session」标签右侧，以及更换 Session 表单内，各一个「如何获取 Session？」。不占首屏。
- **形态**：原生 `<dialog>` 模态弹层，可 ESC / 点背景 / 「我知道了」关闭，无障碍友好。
- **内容**：一句话说明 + 两个官方直达按钮（`https://chatgpt.com/` 登录、`https://chatgpt.com/api/auth/session` 打开 Token 页面）+ 4 步图文（登录 → 打开 Token 页 → 全选复制 → 粘贴）+ 脱敏 JSON 示例 + 安全提示（Token 含账号权限，勿转发）。
- **边界**：纯前端静态内容，不新增任何后端；外链 `target="_blank" rel="noopener noreferrer"`，全部指向 chatgpt.com 官方域名。若运营有独立帮助中心，可把外链改为站内地址。

### 9.2 订阅确认（轻量版）

- **位置**：仅成功状态，成功摘要下方一个「去 ChatGPT 官网确认订阅状态」外链，跳官方设置页。
- **理由**：满足客户「我的号真的变 Plus 了吗」的确认需求，同时零后端、不碰 session、不违反「客户侧不做资金 / 退款查询」边界（订阅状态由官方展示，我们不经手）。
- **明确否定**自建订阅查询后端（DECISIONS C11）：会动后端、扩大敏感数据面、有稳定性 / 封号风险；经核查，同类参考站的「订阅查询」本身也只是一个跳转 `chatgpt.com` 的外链，并无自建查询能力。
- **放成功页而非首屏**：避免与「订单查询」混淆——订单查询 = 查我们的充值进度；订阅确认 = 去官网看账号状态，两回事。
