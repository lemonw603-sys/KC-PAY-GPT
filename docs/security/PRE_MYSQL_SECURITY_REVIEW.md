...
真实客户数据的 MySQL 接入仍暂缓。代码层阻塞项已经修复，剩余阻塞项是生产数据库接入合同：

1. 已完成：Provider 错误消息在写入 `tasks.last_error_message` 前统一脱敏。
2. 待完成：生产数据库接入必须明确私网或 TLS、应用最小权限账号、独立迁移账号和备份加密方案。
...
- `v1`：82 个测试，73 通过，9 个 MySQL 集成测试因本轮未提供测试库而跳过。
...
2. 建立 v1 生产部署合同：HTTPS、应用端口隔离、MySQL 私网/TLS、应用/迁移账号分离、密钥注入和备份。
