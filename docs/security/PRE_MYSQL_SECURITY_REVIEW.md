...
真实客户数据的 MySQL 接入仍暂缓。代码层阻塞项已经修复，剩余阻塞项是生产数据库接入合同的服务器侧验收：

1. 已完成：Provider 错误消息在写入 `tasks.last_error_message` 前统一脱敏。
2. 待完成：在实际服务器验证 3306 网络边界、TLS 主机名校验、应用最小权限账号、独立迁移账号和备份恢复。
...
### F-01 中：Provider 错误消息可能未经脱敏写入任务表（已修复）

**修复状态（2026-08-17）**

- 任务错误在持久化前统一经过敏感文本清洗，外部业务代码也限制为安全格式。
- 新增恶意错误 fixture，覆盖 `accessToken`、`sessionToken`、PAN、CVV 与 API Key。
- 修复后定向测试和完整测试均通过。
...
- Web、worker 和 CDK 工具统一使用受校验的运行数据库配置。
- 迁移脚本改用独立 `MIGRATION_DATABASE_URL`，不再要求 Session 或后台密钥。
- 生产环境连接非回环 MySQL 时强制 TLS，并固定校验证书身份；支持公共 CA 和私有 CA。
- 已建立 `docs/contracts/MYSQL_PRODUCTION_SECURITY.md`，规定最小权限、网络、密钥、备份与恢复验收。
- 尚未取得实际服务器、账号权限、TLS 和备份恢复证据，因此生产接入阻塞仍未关闭。
...
### F-04 低：`card_key` 被不必要地重复持久化（已修复）

**修复状态（2026-08-17）**

- 提交成功事件只保留外部订单号，不再复制 `card_key`。
- 状态查询审计改用本地稳定标识 `recharge-status:<orderId>`，不再把 `card_key` 写入 `provider_calls.request_key`。
- `orders.recharge_card_key` 仍作为轮询所需的单一业务副本保存。
...
- `v1`：86 个测试，77 通过，9 个 MySQL 集成测试因本轮未提供测试库而跳过。
...
1. 已完成：修复 F-01 与 F-04，并增加回归测试。
2. 代码与文档已完成：生产 MySQL 私网/TLS、应用/迁移账号分离、密钥注入和备份合同。
3. 接入空的测试 MySQL，执行迁移和全部九个集成测试。
4. 对真实 schema 做权限核验、TLS 核验、备份恢复和敏感字段抽样检查。
5. 将限流器改为有界清理版本，并在公开上线前核对反向代理拓扑。
