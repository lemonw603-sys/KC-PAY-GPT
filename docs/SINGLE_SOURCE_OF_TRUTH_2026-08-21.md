...
1. API 路径不增加独立账号套餐预检；由充值 API 的明确拒绝（如 `40030`）作为权威资格结果，回退到原订单 Session 可恢复状态。
2. 继续核验 `40030` 在生产后台的详细字段展示；当前代码已写入订单/任务/Provider 调用/资金尝试审计。
3. 继续核查每次真实 `create_direct` 与充值尝试账本的事务边界；阶段三已完成历史孤立明确失败记录的受限补齐和 readiness 检查。
4. Browser 路径的同 Context 必要只读账号步骤单独通过非付款 PoC 验证，不回写为 API 路径独立预检。
