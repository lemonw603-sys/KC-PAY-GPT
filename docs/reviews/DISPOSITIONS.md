...
## F-65 / F-67 后半 结案（2026-09-20，D-306）

这两条一直记作同一个洞的两头：「自动开卡总开关在界面上没有」。**Lemon 2026-09-20 裁定：不做这个控件。** 据此结案，不再算缺口。

**裁定之前查了两件事，都带证据**：

1. **「不接单」代替不了它** —— `grep accept_new_orders v1/src/services/card-supply-scheduler-service.js` → **0 次**。调度器只读 `card_auto_replenishment_enabled` 和策略表；开卡由 `demand = max(水位, 等卡单数)` 驱动。生产实查：两台 plus 水位 2、等卡单数 0 —— 零订单时只要可分配 < 2 就会开卡。
2. **水位就是那个开关** —— 水位 0 → `demand = max(0, 等卡数)`，没人等卡就不开；而水位在设置页已经能改，按台 × 按产品。生产 6 行策略里只有两台的 plus 两行需要动。

**比总开关好在两点**：能只停一台（总开关是全局）；不碰 `setSupplyAutomation` 那个一次写两键的毛病（补余额已弃 D-218，生产刻意把两键设成不同值，调一次就抹平）。

**随之做掉的**：前端 `data-supply-toggle` 孤儿处理器删除（全项目无渲染处，留着会误导下一个人）；后端端点保留并就地注明「为什么没有界面 + 接它之前必须先拆成单键」；设置页水位那栏写明「水位填 0 ＝ 这台这个产品不再自动开卡」。

**那条「有意留红」的测试，真相在这里记一笔**：它从 `d33544c` 起就红在**第一行写死的 `admin.js?v=61`**，后面三条真正守 F-65 的断言**一次都没执行过**。也就是说这半个月里「留着它当缺口信号」这个说法本身是不成立的——它守的是一个过期的版本号。把版本断言改成「带版本 + 只增不减」后真断言才第一次跑起来，立刻红在「供给三态文案必须存在」，而那个控件在工作台改乙-3 时就拿掉了。既然裁定不做，该断言随决定作废，测试改成钉住本决定（旧谎报文案不许回来 / 前端不许出现那个控件与端点 / 设置页必须写着水位那句话），**三个变异全被抓**。

**教训**：「有意留红」这种安排，必须定期确认它红的**是不是那个原因**。一条永远红的测试和一条没有的测试，提供的信息量是一样的。

**全量 1004 / 937 pass / 0 fail**。
