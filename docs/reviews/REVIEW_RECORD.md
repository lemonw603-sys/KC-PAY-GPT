...
## 批次 STEP5｜截至 2026-09-18 09:51:49 UTC｜commit 31b5639｜release 20260918-step5-740bc1d｜板块 B/E/F（V2第⑤块）

完整的协议字段、原始证据、反例、修改建议和范围见 [STEP5_REVIEW_2026-09-18.md](STEP5_REVIEW_2026-09-18.md)。仅审查，不改业务、不部署，不与执行者直接对话。

- **F-47 / P1**：押金预检修复只在本地，生产仍执行旧公式；已知问题现场复核。
- **F-48 / P1**：所有额外扣款统一列待登记，重复扣款也可不算差异。
- **F-49 / P1**：首次导入余额被当累计入金，混算全历史流水，金额判据缺可靠起点。
- **F-50 / P1**：首跑后的只读报告立即给persistent=true，生产dry-run已复现。
- **F-51 / P1**：没有有效同步水位/一致快照门槛，旧数据可重复升级异常。
- **F-52 / P1**：DAILY_DIGEST只有静音，没有余额变化汇总发送。
- **F-53 / P1**：负余额被取绝对值，20美元差额可算成MATCHED（离线复现）。
- **F-54 / P2**：跨天恢复只resolve今天key，昨天日报仍OPEN。
- **F-55 / P2**：发送次数COUNT复用通知行，重复发送覆盖历史、漏计。

### 未能核实的事项
真实金额差根因、新故障端到端推送、拒付扣款所在账户、token时段联动、真实数据库集成与新真单。
### 与事实源冲突但无法判断谁对
D-274将金额差归为非资金问题缺独立证据；D-272手动消费豁免是否涵盖所有未知扣款需明确。HANDOFF仍沿用已撤回押金规则，属于可确认过期，由执行者订正。
### 本次未覆盖范围
第⑥—⑧块、Browser/付款核心、全项目历史审计；未改代码、未部署、未生产写入。

## 评估补充 STEP5→678｜2026-09-18 10:36 UTC｜commit 7eab0b8｜板块 B/D/E/F 规划衔接

新增 [V2_STEP5_TO_STEP678_OPTIMIZATION_ASSESSMENT_2026-09-18.md](V2_STEP5_TO_STEP678_OPTIMIZATION_ASSESSMENT_2026-09-18.md)，供 Lemon 与 Claude 讨论是否优化，**不是已批准整改任务**。不新增缺陷编号、不修改 F-47～F-55 历史。

主要补充：区分经营日报与付款核实；⑥可继续无依赖部分，精确通知次数/余额日报延后需明确确认；手动用卡登记不能只记备注；⑦保留产品容量和目标套餐核实；⑧核实仍在使用的reconciliation_cases/browser_interventions后再决定删或归档。含10:25–10:26 UTC生产只读证据、代码路径、39项测试及候选取舍代价。未改业务、未部署、未写生产。

### 未能核实的事项
历史金额差根因、系统外手动消费登记最佳模型、Pro真实交付、最终UI与完整清理动态覆盖。
### 与事实源冲突但无法判断谁对
手动消费豁免边界、对账表归档含义待双方明确；人工接管表仍有活动代码依赖，未见替代实现。
### 本次未覆盖范围
完整全项目复审、真实付款、浏览器操作和全部优化实施；报告只用于评估。

## 批次 STEP5B｜截至 2026-09-18 11:51:26 UTC｜commit 2458f34（实现7266ec5）｜release 20260918-step5-740bc1d｜板块 B/E/F

复审依据D-275/D-277收窄后的要求。详见 [STEP5B_REVIEW_2026-09-18.md](STEP5B_REVIEW_2026-09-18.md)，含逐项旧发现状态、原始输出、影响条件、反证及本机最小重放。

- **F-56 / P1**：日报改固定key，持续OPEN时次日不再入队；critical升级也不推。实际MySQL临时表+正式仓库函数复现。
- **F-57 / P1**：手动用卡登记复用已销卡确认，写RETIRED终态和完成事件，尚待销的卡退出due。
- **F-58 / P1**：F-51仍未闭合，旧数据跨日正式timer可升级；新“同步失败”用例仅测persist:false。
- **F-59 / P2**：固定新key未结清生产已存在的旧日期key日报。
- **F-60 / P2**：D-277要求删的CHARGE_BACK/DISPUTE在两处活动代码中仍保留。

已认可：金额降级、未知扣款分离、GET不推进、删除空汇总选项、告警实例统计改名、负余额原反例修复。本地899/833通过/0失败/66跳过；新代码生产只读输出与执行者一致。⑤b与F-47仍未发布，不宣称生产修好。

### 未能核实的事项
新版生产实际投递、外部真实销卡、有效同步水位完整性、历史批准记录。
### 与事实源冲突但无法判断谁对
RETIRED override候选与inventory终态排除不可混同；若手动登记要求先外部销卡，需要明确该前提。历史金额差不重新定因。
### 本次未覆盖范围
⑥⑦⑧实施与全项目安全复审；未改业务、未生产写入、未部署或花钱。

## 批次 STEP6｜2026-09-19 01:46 UTC｜commit 8cd6d7e（已回滚），HEAD db10c9b｜板块 B/E

详见 [STEP6_REVIEW_2026-09-19.md](STEP6_REVIEW_2026-09-19.md)。未提交三份前端修改视为在途，不覆盖。生产现场已回⑤b。
- F-61/P1：“解决”只关case，不写账本或收口订单；交接声称不符。
- F-62/P1：unverifiableAmountCount误读成unverifiableCount，浏览器复现30→0。
- F-63/P1：请求失败降成空数据，显示没有待办。
- F-64/P1：待销来源缺失，差异“看逐张”没有对应报告落点。
- F-65/P1：已发布版复用旧供给组合开关，会一起打开补余额；在途版已移走控件，仍需留意设置页。

### 未能核实的事项
完整发布交互批准记录、831测试独立复跑、生产实际资金按钮点击。
### 与事实源冲突但无法判断谁对
D-283/sidebar暂留旧皮与回滚理由冲突，不能一概指责擅自改设计；HANDOFF顶部回滚而正文仍已上线。
### 本次未覆盖范围
在途重做最终质量、⑥其余三页全量、Pro与全项目安全；未改业务、未部署、未写生产。
