...
默认1050 tests / 981 pass / 0 fail / 69 skipped。临时库清理后剩余0，恢复容器/临时密钥清理。没有修改生产数据或权限来做这批测试。

### 旧备份摘要格式兼容

维护前首次生产备份恢复在新版摘要结构检查失败，生产未停单。只读确认旧值188字符、JSON合法，仅generatedAt与discrepancyFingerprints，时间2026-09-18T08:29:39.596Z；是旧版本格式而非损坏。验证器现在仅在058以前允许这一个已证实格式，并明确输出LEGACY_FORMAT；058以后仍要求日期与persistentFingerprints完整，不把兼容当新版写入已通过。单测覆盖新/旧/坏JSON/坏日期/错误数组，真实MySQL10项重跑通过。
