...
> **2026-09-21 D-328发布后覆盖**：第⑥步及诊断已上线，055～058、临时权限撤回、真实生产摘要保存/读回、迁移前后生产备份隔离恢复、进程/HTTP/源文件哈希均已验证。容量缺陷闭合；实际存摘要544字符，旧31348描述是展示报告。旧备份格式有严格版本兼容，不当新版持久化证明。**仍未验**：各产品真实客户付款、异地副本和分离密钥取得；也不是生产所有写按钮已实点。证据`reviews/2026-09-21-step6-release-v2/report.md`。

> **2026-09-21 D-327发布中新缺陷**：生产日对账04:02任务ER_DATA_TOO_LONG；app_settings.setting_value为VARCHAR(255)，候选只读报告31348字符。**此前persist:false/小样本检查未覆盖真实规模的完整报告落库**。第⑥步发布在迁移前中止并恢复旧版营业，容量修复/真实写入与读回/恢复验收待确认实施。不要把“对账页面能读”当“每天报告已成功保存/通知”。证据`reviews/2026-09-21-step6-release/report.md`。

> **2026-09-21 D-326最新覆盖**：诊断旧通用关闭入口已在本地前后端拦付款不明，4条从诊断出发的真实MySQL/UI收口及直接HTTP409不变性验证通过；逐卡明细接线、错误状态/重试、原型几何及变异通过。此前“只覆盖工作台、诊断未验”和欠账11的本地缺口由此闭合，**生产未发布、历史case未清理**。默认979通过/0失败/69跳过，不算全MySQL全绿。证据 `reviews/2026-09-21-diagnostics-implementation/report.md`。

> **2026-09-21 D-324覆盖**：正式restore-test已接业务关联读取、代表性解密、057结构/DEFINER/合成告警轮次验证；14项真实隔离验收通过，生产未安装。缺身份自动准备仅限准确无网络临时容器、锁定账号、告警表必要权限；未知/权限不足拒绝。D-323付款范围文字明确不改，不再待修。此前D-322“正式恢复脚本未增强”由本条覆盖；生产备份、独立密钥/异地副本、真实恢复后链路仍未验。证据 `reviews/2026-09-21-restore-fix/report.md`。

> **2026-09-21 D-322补验覆盖**：付款不明API/Browser×已扣/未扣的工作台→订单详情→正式服务→真实隔离MySQL收口四例已通过，卡资格/CDK重提及幂等均核对；不再称“本地页面收口全未验”。合成备份在无网络新MySQL中恢复、Session/CDK解密/getOverview已通过，缺DEFINER报1449的负例及补账号权限后轮次递增已实证。**未闭合**：付款按钮范围提示、正式恢复脚本的DEFINER/功能验收；生产真实未知付款样本、异地备份/分离密钥可用性仍未验。证据 `reviews/2026-09-21-step6-safety/report.md`。
...
**D-321最新覆盖**：此前迁移中断/无卡恢复两缺口已本地修复，原7+新15断点均可续做，28迁移场景与6类队列恢复场景实库符合预期。无卡实际分到卡后才恢复准备/提交，UNKNOWN资金与有效租约拒绝不变。生产057创建权限、DEFINER、备份/维护/发布仍未落实，不以本地修复替代。见演练报告末尾和fixed-evidence.json。
