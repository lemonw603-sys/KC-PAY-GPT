...
> - **2026-09-18 第④步（代码在 main 未发布，D-267）做了但没生产证明的**：分卡当场同步只有单测 + 集成 + 「候选 SQL 在生产窗口外选中 5276」的只读实证，**没有一单真实分卡走过新路径**（发布后用一张窗口外 hnskj 卡 + 一单演练验）；API 付款不明两路证据（历史 0 次，无样本）；Browser 崩溃进补核（历史 0 次）；highvcc 卡真证据一路要等 D-268 第 1 条批了才在生产生效；待销清单端点未在生产调过（口径 SQL 已在生产只读实跑）。
