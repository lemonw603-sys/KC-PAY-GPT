...
- Pro 5x/20x 页面路径、全新账号（非 Rejoin）的弹窗：未跑。20X 第二阶段改为「已订阅账号 → Confirm plan changes 弹窗」（D-133）：导航器与只读演练脚本 `poc-plan-change-dialog-readonly.mjs` 已有（`1e8449c`），只在本地 Chromium 夹具上通过，尚未在真实已 Plus 账号上跑；付款后会话恢复阶梯（D-134）同样只有单测。
