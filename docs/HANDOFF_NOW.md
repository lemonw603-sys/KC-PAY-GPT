...
1. 上面四个问题：**D-363 定先做⑤，再①～④一批**（①先给原型）。⑤ 已发布。**①～④ 已发布**（03:05 UTC，卡片页原型已重冻、四份可跑契约一致）。**D-365 已做、未发布**：欠账 16（逐个候选 + 一致排序）、17（只替等卡单转台）、同一台故障只推一条（推送领取时覆盖）；待 Lemon 说发布。字体 Lemon 选 B（D-366），与三件一起发。本机隔离库 `pojia_ui_d363` 已删。
...
- （已定 D-366）「可离开」第一条改为：第一张真实客户 Plus 单＝真钱验收，此后连续 10 单真实客户单无人介入。
