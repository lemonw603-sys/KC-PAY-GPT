...
更新：2026-09-14 15:10 UTC（下方 banner 为 2026-09-16 阶段2收尾增补）。写者：大脑窗口。**本文只由大脑窗口写，且每次重要事实变化后整篇复核，不止改暂停原因那一行。**
...
> **2026-09-16 10:46 UTC 单笔排障更新**：h9RKl 的历史403已由一号窗口 Resource Timing 坐实（10:17:11Z /api/auth/session=403，套餐接口200）；当前正式probe比对生产订单身份成功、套餐FREE。代码只对200空身份等待，对403直接失败退码。详见 `docs/incidents/2026-09-16-h9RKl-report.md` 及4份脱敏证据。上游403诱因与付款后段仍未知；建议有限只读复核+诊断，尚未实施/重启/重兑。已有 executor.js/payment-executor.js 改动保留。state-check 已知 line61变量错误未修。

> **2026-09-16 10:56 UTC 诊断补丁**：第二项获批，仅将身份核验HTTP状态/阶段/CF头存在性写入已有失败事件。本地34/34测试通过；2026-09-16 11:04:25 UTC 经用户确认重启，PID67720，11:05 UTC心跳复验通过；新诊断尚无真实失败入库样本。第一项有限重试仍在讨论，未实施，不能承诺消除403。详见HANDOFF_LOG末节。
...
- **多窗口分工见 `PLAN_2026-09-14.md` §6，Lemon 待定（三天盒强度高，建议开）。** 开之前四份事实源仍只由大脑写。
...
## 现在状态（2026-09-14 15:05 UTC 当场核实，`state-check.sh` 全一致）
...
- **可分配卡 1 张：`1657`（backup-a / highvcc，$18.56）。今日 hnskj 卡台服务器故障、开不了卡（Lemon 报，D-223），
  API 路线（冻结卡源 hnskj）0 张可分配 → 新 Plus 单走 API 停在 `WAITING_FOR_CARD`；Browser 路线有 `1657` 可用。**
  `1652`（hnskj）已 `DEPLETED` $0.01：Lemon 补的 $50 跑完一单后，剩余 **$34.57 由 Lemon 手动退回卡台钱包**（此前标"未查清"，已确认，D-218 补记订正）。
- **A3（账本推算口径 `card-inventory-eligibility.js`）未发布**（生产 md5 `7efec865` ≠ 工作区 `525b7e1b`）。
  D-218 已选 3（放弃补钱只开新卡），A3 可发，**前提**：Lemon 先真正关闭补余额开关（D-223，现仍 true）+ 手动开卡入库记真实金额（同一发布）。列为 Day 09-15 T1。
...
| 14:50 | 每卡单数按产品：Plus 3、5X/20X 1，两路线一致 | D-221 |
| 15:05 | Lemon 给三天时间盒 + **推翻用时间当验收**；PLAN 重写 v3、验收改机器可验 DoD | D-222 |
| 15:05 | 补余额开关审计显示 08-31 起从未关成（UI 两开关合一）；今日 4 次 400 由它触发 | D-223 |
...
**14:40 UTC 追加**：D-218 选 3（放弃补钱只开新卡）；hnskj 卡由 Lemon 手动开；switch 同时重启 worker（已改 `b9f0837`）；每卡单数按产品：Plus 3/5X/20X 1（D-221）——`default_open_card_amount=16` 与之不符且无后台入口，自动开卡前必须先改。
**15:05 UTC 追加**：Lemon 给三天时间盒（09-15 冲刺 / 09-16 V2.0 / 09-17 V2.1 / 09-18 起 V2.2）并**推翻"用时间当验收"**（D-222，PLAN 已重写 v3，验收改为机器可验 DoD：测试+现场脚本+只读复验，稳定性不作交付门槛）；hnskj 卡台今日故障开不了卡；补余额开关审计显示从未关成（D-223，待 Lemon 真关）；34.57 确认是 Lemon 手动退回。开卡有 $0.5/张手续费。
...
4. **同一 Session 连败没有熔断**——V2 已纳入 B1（缝13/缝E：同一 CDK 连败 N 次熔断、SUBMIT_UNKNOWN 不计失败），阶段3落实。
...
8. **补余额开关 `card_balance_recharge_enabled` 仍 true**——Lemon 以为关了，审计显示 08-31 起未变更（D-223）；今日 4 次 hnskj 补钱 400 由它触发。待 Lemon 真关。

## 下一可执行项

0. **今天（09-15）= 三天盒 Day 1**，任务书 `docs/tasks/2026-09-15-day1-browser-mainline.md`：T1 A3+funded_amount 发布（前提：Lemon 先关补余额开关 D-223）· T2 诊断存库 · T3 切换校验+解耦卡台 · T4 免费试用入口。主线 Browser（API 卡台故障，`1657` 一张）。
1. **三天时间盒**（D-222，PLAN v3）：09-16 V2.0 · 09-17 V2.1 · 09-18 起 V2.2。验收按新口径（测试+现场脚本+只读复验，**不用时间**）。后两天任务书在前一天收尾时建。**待 Lemon**：多窗口开不开、V2 六个面是否全认。
...
```text
暂停原因：无阻断。进入三天时间盒（D-222），今天 = Day 1（09-15），任务书已建。hnskj 卡台今日故障开不了卡，主线走 Browser（`1657` 一张）。
         **待 Lemon**：①去后台真正关补余额开关（D-223）②多窗口开不开（决定 V2.0 那天能否装下控制面 4 页）③V2 六个面是否全认。
...
禁止操作：不实施人机验证绕过；开卡/补余额/换卡/提现等资金动作仍需 Lemon 当次确认；**A3 发布前提是补余额开关已关**；**worker 在跑时不碰 Pilot 窗口**
恢复第一步：读本文 → 读 D-222（三天盒+验收口径）/ D-219（V2 六个面）/ D-211（六类惯犯）→ `ready-check.sh pay` → `state-check.sh` → 读 `docs/tasks/2026-09-15-day1-browser-mainline.md`
下一步：Lemon 关补余额开关 → 发 T1 → T2/T3/T4
```
