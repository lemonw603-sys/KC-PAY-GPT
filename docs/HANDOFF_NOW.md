...
更新：2026-09-24 02:5x（UTC+8）。当前执行顺序以 **D-352** 为准（覆盖 D-329 门槛与 PROJECT_MAP 旧八步⑥之后的排序）。
...
1. **块 5 发布**（等 Lemon 确认）：`deploy-release.sh prepare` → 复核 → switch → 只读复验（列表查询对 80 单实跑；`admin.js?v=92` / `admin.css?v=30` / `orders.css?v=3` / `orders.js?v=3`）。回滚点 `20260923-d355-8f9ddc1`。
...
- 订单页 critique 待定五项（`reviews/2026-09-24-orders-v3-critique.md`）：**P1 等 Session 单的「取消并放卡」列表有、抽屉规则永远不给（D-355 放卡后规则冲突，先查取消服务那份规则）**；P2 桶内付款不明用危险色排首行；P3 等待态抽屉先放状态句；P4 时间标 UTC+8；P5 进度空时给当前阶段。两个挑衅性问题（默认落「需要我处理」桶？取消收进抽屉只显示剩余小时？）。
