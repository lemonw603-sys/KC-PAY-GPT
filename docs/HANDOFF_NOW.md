...
- **D-398：ZovoCard 接进本系统、执行规则全听它的**（本系统只管入口 / 记录 / 通知 / 总开关，入口分流、不进本方卡层）。待 Lemon 核对：成功口径（它 `completed` 即成功？）、下单口（让它选卡）。前提：Lemon 的 ZovoCard 账号余额 $0 未充值，直充接口需开通；字段须对真实响应核实后才写代码。材料在 Lemon 本机 `~/Downloads`（`spacexcard-openapi (1).md`、`卡网.zip`），未入库。
