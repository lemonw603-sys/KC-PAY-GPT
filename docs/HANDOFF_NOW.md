...
更新：2026-09-24 01:1x（UTC+8）。当前执行顺序以 **D-352** 为准（覆盖 D-329 门槛与 PROJECT_MAP 旧八步⑥之后的排序）。
...
- **块 5 订单页 v3 代码完成、本地全绿、未发布**（`reviews/2026-09-24-orders-v3-impl.md`：真实页 30/30、parity 一致、v1 1032/0）。线上仍是 D-350 版（`admin.js?v=89`）。真钱只留 Plus 一单（块 4）+ 20X 一单（块 6），其余用 rehearsal（D-352 补记）。

## 下一可执行项（按 D-352 块序）

1. **块 5 发布**（等 Lemon 确认）：`deploy-release.sh prepare` → 复核 → switch → 只读复验（列表查询对 80 单实跑；`admin.js?v=91`）。回滚点 `20260923-d355-8f9ddc1`。
