...
更新：2026-09-26 01:0x（UTC+8）＝ 09-25 17:0x UTC。执行顺序以 **D-352** 为准，「可离开」第一条以 **D-366** 为准；本窗口 D-376～D-381。
...
- 分支 **`block6-pro5x`（`103f7d2`，已推送，未合 main、未发布）**：在 `b3f1d37` 上加了「升级按钮是灰的就等它变可点」（`56fa9e7`）与导航失败证据包（`f640f9c`、`103f7d2`）。
...
- **导航失败证据包已做完（分支 `f640f9c`+`103f7d2`，D-379/D-380/D-381）**：导航失败时本机存 `~/Library/Application Support/pojia-browser-live/evidence/<时间>-<运行号>/`（完整 trace + 控制台 + 网络含出错返回内容 + 页面 + 截图），成功即丢弃录制；失败事件多 `evidenceRef`。11 条测试、8 处变异全抓到；分支全量 334：325/0/9。未合 main、生产未用。
- **对照实验 A 做了（D-381）**：新免费号 + 块 6「一可点就立刻点」→ 41 秒进标准结账页，无报错。「点太快」推测不成立；09-25 两次失败原因仍未知（候选：那个号、执行器完整路径、当时 ChatGPT 侧临时问题）。成功基准：`POST /backend-api/payments/checkout` 200。证据包首用暴露网络清单超时 bug，已修（`103f7d2`）。
- **下一步（待 Lemon 定时间）**：带证据包重跑块 6 Plus 回归演练（要停付款池、Lemon 客户页建单，最好再用一个没用过的免费号）。失败就有完整现场；通过就进「合 main → 发布 → 池切固定目录 → 重开 305」。3 号窗口还开着实验 A 的未付结账页，演练前关掉。
