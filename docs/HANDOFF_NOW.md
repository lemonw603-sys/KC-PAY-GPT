...
更新：2026-09-26 01:3x（UTC+8）＝ 09-25 17:3x UTC。执行顺序以 **D-352** 为准，「可离开」第一条以 **D-366** 为准；本窗口 D-376～D-379。
...
- 分支 **`block6-pro5x`（`f640f9c`，已推送，未合 main、未发布）**：在 `b3f1d37` 上加了「升级按钮是灰的就等它变可点」（`56fa9e7`）与导航失败证据包（`f640f9c`）。
...
- **导航失败证据包已做完（分支 `f640f9c`，D-379/D-380）**：导航失败时本机存 `~/Library/Application Support/pojia-browser-live/evidence/<时间>-<运行号>/`（完整 trace + 控制台 + 网络含出错返回内容 + 页面 + 截图），成功即丢弃录制；失败事件多 `evidenceRef`。11 条测试、8 处变异全抓到；分支全量 334：325/0/9。未合 main、生产未用。
- **下一步＝对照实验**（不付款）：立刻点 vs 可点后等 5 秒，带证据包。方案待 Lemon 选：他在 3 号窗口手动登录一个没用过的普通免费号（不用建单、不停池），我用脚本跑。线索：Lemon 手动点进的是标准结账页 `/checkout/…`，自动点是网址不变的页内报错。
