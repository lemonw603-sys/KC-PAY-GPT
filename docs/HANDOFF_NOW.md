...
更新：2026-09-24 02:1x（UTC+8）。当前执行顺序以 **D-352** 为准（覆盖 D-329 门槛与 PROJECT_MAP 旧八步⑥之后的排序）。
...
1. **块 5 发布**（等 Lemon 确认）：`deploy-release.sh prepare` → 复核 → switch → 只读复验（列表查询对 80 单实跑；`admin.js?v=91` / `admin.css?v=30` / `orders.css?v=3` / `orders.js?v=2`）。回滚点 `20260923-d355-8f9ddc1`。
...
- 全局令牌对比度：`--wb-t3` 50%→44%、`--wb-warn` 40%→36%（四页共用，impeccable 检出 3.6:1 / 3.9:1）。
