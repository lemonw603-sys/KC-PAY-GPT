...
0. **三个决定在 Lemon 手上，都影响下一单**：
   ① **D-218 三选一**（补钱时抬 `funded_amount` / 放宽 A3 / 放弃补钱只开新卡）——定了 A3 才能发；
   ② **API 路线现在没卡**——开一张 hnskj 卡（真开卡，需当次确认）/ 切回 Browser（`1657` 可用，但 Browser 死点未修）/ 矛盾五实测（ZZSHU 用 highvcc 卡，一单就知道）；
   ③ **`deploy-release.sh switch` 要不要同时重启 worker**（D-220）。
