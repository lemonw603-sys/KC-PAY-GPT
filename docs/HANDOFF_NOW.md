...
- **待讨论（Lemon 提出，未定）**：新卡台 ZovoCard（zovocard.com，开卡 + GPT/Claude/Grok 直充 + 卡密发放/兑换 + 取消续费 + 回调，全套开放接口）接进来还是独立用。材料只在 Lemon 本机 `~/Downloads`（`spacexcard-openapi (1).md`、`卡网.zip`），未入库。现场：Lemon 的 ZovoCard 账号余额 $0、未充值（09-26 08:4x UTC 页面横幅）；文档写直充接口要「已有入账充值 + 管理员开通或 SVIP」。分析与建议见 HANDOFF_LOG 同日章节。
