...
更新：2026-09-24 03:2x（UTC+8）。当前执行顺序以 **D-352** 为准（覆盖 D-329 门槛与 PROJECT_MAP 旧八步⑥之后的排序）。
...
- **块 5 订单页 v3 代码完成、本地全绿、未发布**（第八轮 D-358 后：真实页 32/32、parity @1920 一致、v1 1032/0；`reviews/2026-09-24-orders-v3-impl.md`）。全站内容区封顶 1280（四页宽屏一起收窄）。项目根新增 `DESIGN.md`。线上仍是 D-350 版（`admin.js?v=89`）。真钱只留 Plus 一单（块 4）+ 20X 一单（块 6），其余用 rehearsal（D-352 补记）。

## 下一可执行项（按 D-352 块序）

1. **块 5 发布**（等 Lemon 确认）：`deploy-release.sh prepare` → 复核 → switch → 只读复验（列表查询对 80 单实跑；`admin.js?v=92` / `admin.css?v=30` / `workbench.css?v=20` / `orders.css?v=3` / `orders.js?v=3`）。回滚点 `20260923-d355-8f9ddc1`。
2. **块 4**：Plus Browser 真钱一单（Lemon 定何时、自费或客户单）；付款后半段在当前 release 仍零样本。
...
- 块 5 是否现在发布（prepare 前先问）。
- `DESIGN.md`「概览」一节的措辞核对（令牌与规则都有代码出处，只有创意语言是沿用候光）。
- critique 已裁定（D-359）：1 做（已修）、2 不做、3 不改、4 同意（块 5 后加「还剩 N 小时」，欠账）、5 同意（已改）。
