...
更新：2026-09-24 03:5x（UTC+8）。当前执行顺序以 **D-352** 为准（覆盖 D-329 门槛与 PROJECT_MAP 旧八步⑥之后的排序）。

## 现在的状态

- 生产 release **`20260924-orders-v3-822cb7d`**（09-23 18:0x UTC；块 5 订单页 v3 全部 + 内容封顶 1280 + 令牌调深；回滚点 `20260923-d355-8f9ddc1`）。默认路线 BROWSER（D-353）；API 路线代码标未验证。生产数值只看 `CURRENT_STATE.md`。
...
1. 块 5 已发布、Lemon 看过生产页「没问题」（04:0x UTC+8）。欠账：等 Session 行芯片加「还剩 N 小时」（D-359 第 4 条）。
