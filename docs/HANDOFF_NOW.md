...
更新：2026-09-13 07:40 UTC。写者：大脑窗口。**本文只由大脑窗口写，且每次重要事实变化后整篇复核，不止改暂停原因那一行。**
...
- 生产 release **`20260913-steady-ring-5825d1c`**；web / worker 均 active；迁移 `052_cards_bin`。回滚点 `20260913-customer-ux-4ccb8c7`。
- 付款开关 **true**、接单开关 **true**；非终态订单 **0**、active_runs **0**。
- **可分配卡 0 张——卡 3159 刚服务完一单，`DEPLETED` 待余额同步**。实扣 $15.72，
  应剩 **$34.28**，等 `sync-highvcc-snapshot.mjs` 小时级快照回填后自动重回可分配。
  **用量 1/3**，还能跑 **2 单**。急着接下一单可以手动跑一次快照同步。
