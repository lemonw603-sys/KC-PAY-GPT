...
3. 客户充值页改造（进行中，D-178）：**方案「候光」已定稿并完整固化**（D-179）：原型 `docs/design/prototypes/houguang-v2-full-flow.html`，规格与六条硬规则、踩坑清单见 `docs/design/README.md`。待再出 1–2 版对比；若无更好的即按此实施（拆四步、CDK 先验、九阶段、订阅查询、去掉时间承诺）。
