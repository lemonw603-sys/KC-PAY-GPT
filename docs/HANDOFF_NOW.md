...
更新：2026-09-24 09:0x（UTC+8）。当前执行顺序以 **D-352** 为准（覆盖 D-329 门槛与 PROJECT_MAP 旧八步⑥之后的排序）。
...
3. 欠账 3 每卡单数按产品：已发布并复验（D-361）。欠账 15 工作台「剩 N 张 · 能充 N 单」：**代码完成、本地全绿、未发布**（D-362），发布先问。
