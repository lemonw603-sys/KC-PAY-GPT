...
1. 上面四个问题：**D-363 定先做⑤，再①～④一批**（①先给原型）。⑤ 已发布。**①～④ 已发布**（03:05 UTC，卡片页原型已重冻、四份可跑契约一致）。**D-365 已做、未发布**：欠账 16（逐个候选 + 一致排序）、17（只替等卡单转台）、同一台故障只推一条（推送领取时覆盖）；待 Lemon 说发布。「剩 N 张」字体出几版给 Lemon 挑。本机隔离库 `pojia_ui_d363` 留作字体比稿用，定稿后删。
