...
0. **全链路审计两轮已出**（`docs/reviews/FULL_CHAIN_AUDIT_2026-09-10.md`）：3 个 P0——F-5 客户页永远不让重贴 Session（一行 bug，线上同源）、F-1 预检 DEAD 无重开入口无告警、F-10 等待期 session 失效→打回→撞 F-5；第二轮新增 P1：F-16 付款不明/升级人工后无正式收口（只能手工 SQL）、F-18 核实 lane 仍重注入旧 token（96ac467 只删了付款路径）。修复顺序见报告 §五末尾；未经用户点头不改。
