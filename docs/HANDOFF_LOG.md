...
## 2026-09-21｜给用户实际操作的隔离CDK演示服务

08:44 UTC+8核对。用户指出截图无法判断操作是否顺手，明确同意恢复可点击演示并保留试用。新增scripts/cdk-demo.mjs，复用真实createApp/CDK服务与订单只读服务，单独创建本机step6_cdk_demo并迁移至056，不注册客户下单、开卡、付款及外部卡台操作接口，不启动worker。由于当前无preview_start工具，已告知并使用独立Node进程，端口8804/PID30087/exec会话29657；只绑定127.0.0.1。

在应用内浏览器实际登录→点CDK→生成1张，看到“已生成1张并复制”及自动批次筛选，随后恢复全部列表供用户试用。独立HTTP /health/ready=200、cdks/liability=200（pending14/reserve5/done1/attention1）；被禁用的下单/开卡路径404。23张初始样例+1张交互测试码，均属隔离数据。脚本node --check及git diff --check通过。未修改生产实现、未连接生产、未执行state-check/wrapup，未推送；本轮不需重跑业务实现全套测试。

本服务和库按用户要求保留，不按自动测试收尾删除；待用户试用结束后再停止自己的进程并清理隔离库。原8803/8899及常驻Browser池均未操作。页面标签已标记为交付，用户可继续点击。已按delivery-commitment-check核对：恢复环境/登录/实际生成/交付链接完成，用户试用与后续联合验收仍待进行。

## 2026-09-21｜CDK 用户实际试用确认（D-319）

UTC+8。用户在隔离页回复「同意，没问题」，记为CDK当前页面与操作获确认。已更新DECISIONS与HANDOFF_NOW；不重开已定方案，不把确认扩展成发布授权或既有回归失败豁免。本轮仅文档更新及diff检查，不停演示、不连接生产、不运行迁移/发布。下一步四页联合验收和发布前检查。

## 2026-09-21｜四页联合验收与12条旧失败归因

UTC+8，基线f8a8f60。用户同意继续本地四页联合验收与旧失败影响核查；不接生产、不改业务代码。按dogfood流程实际检查8805独立隔离服务的工作台/设置/卡片/CDK，保留8804用户试用页。设置水位2→0保存+审计通过，工作台和卡片页同时更新；工作台生成码在CDK页显示，SQL核验NORMAL与30天。

临时git archive副本仅改测试夹具/退休预期：冻结卡台、初始入卡金额、assigned_card_id、通知白名单类型、VERIFY_CARD、Session无限次/无卡恢复状态。在全新step6_joint_qa3库最后一跑43 tests/41 pass/1 fail/1 skipped。正式测试与业务源码完全未改；诊断补丁保存在报告旁。唯一剩余失败Bark重开，经精确alert_id对照：扫描没看到RESOLVED则重开后仍SENT；看到了则PENDING。中途复用污染库/通用claimNext探针结果已弃用，未作为结论。

报告 `reviews/2026-09-21-step6-joint/report.md`：J-01告警重开漏推（High）、J-02今日订单请求失败被渲染成无订单（High，浏览器loadingFailed inspector事件+DOM实证）、J-03设置页残留72小时门槛（Medium）、J-04CDK无备注批次日期显示半截时间（Low）。前两项建议发布前修；未擅自实施。其余未覆盖与已知欠账如实列出。截图在本轮会话保留，未虚构本地截图文件。

本轮8805/验收标签及step6_joint_qa/qa2/qa3/ui四个专用隔离库清理；临时副本/文本证据可重放，原8803/8804/8899与常驻worker不动。不连接生产、不执行state-check/wrapup、不推送。下一步让用户确认修复J-01/J-02（可同批纠正J-03/J-04），不能凭CDK用户验收直接放行上线。

## 2026-09-21｜D-320修复J-01～J-04

UTC+8。用户同意修复后新增057告警轮次，两列与状态转OPEN触发器，不依赖会变化/被清空的时间字段；通知按轮次领取/确认，dispatcher传递incidentVersion，旧发送回调不覆盖新一轮。修复今日订单__error丢失并加局部重试，移除设置72小时退休展示，批次日期按Asia/Shanghai完整日历格式。没有扩展新业务模块或改布局，admin.js/cdks.js资源版本同步升。

新增回归先红后绿；实际MySQL测试覆盖重开漏扫、并发扫描/领取、白名单、普通内容更新、重试终止、迟到回调、upsert触发器。告警专项及原Bark测试2 pass；默认1023/954 pass/0 fail/69 skipped。真实8805页面阻断请求后出现“今日订单读取失败。重试”，恢复点重试才显示“今天还没有订单”；设置页72小时已无，CDK下拉及行内日期完整。卡片/CDK/营业条三份1440契约均通过，CSS/文案/diff检查通过。

证据追加到reviews/2026-09-21-step6-joint/report.md，旧报告保留。当前四项本地修复不等于生产放行；需要另外核对057权限/备份/中断恢复、bark换代码，不能只部署UI。历史迁移前漏告警无自动回补保证；无卡Session恢复排程仍未专验。未连接生产、不推送、不跑含生产连接的state-check/wrapup，8804试用服务不动；本轮8805和两个专用隔离库验完清理。

## 2026-09-21｜第⑥步整体发布前生产只读核对

01:55～02:01 UTC（09:55～10:01 UTC+8），用户同意继续只读核对，随后问清范围；已明确这轮针对整个⑥工作台/CDK/卡片/设置共同上线依赖，不是只CDK，也不是四页所有场景全部重验。

生产current仍20260918-step5b-b0a36d4；web/worker/bark均active且同release，PID分别3053/2035/3105，NRestarts2/0/9；ready200；最新迁移054；终态20成功/37失败/21关闭，无非终态单、活动Browser账号槽0。state-check通过，但其未检查PID/重启/备份日期，CURRENT_STATE这些行已据原始SSH证据订正。最近服务器启动2026-09-21 01:08:52 UTC，原因未查，不借历史解释。
...
## 2026-09-21｜D-321两处恢复缺口修复与实库复验

UTC+8。用户同意最小修复。migrate.js加入数据库级单执行者锁、整批055～057权限与结构预检，step6-migration-guard绑定已审SQL的SHA256并逐项续做；原SQL未改，不加迁移业务表、不提权、不改全局变量。同名异结构拒绝，已记账却缺结构也拒绝。权限不足先停，不再留下半迁移。

Session事务在资金检查后锁任务：有效租约/重复分卡任务拒绝，无卡恢复/补建唯一ASSIGN_CARD，下游暂以现有DEAD+专项原因标记暂停；实际assignAvailableCard成功后只恢复该原因且规范去重键的准备/提交任务。已绑卡保留原路径。未改Browser付款代码或CDK业务方案。
...
## 2026-09-21｜D-323付款开关不改，先解释恢复方案

用户明确知道Browser付款门范围即可，不改开关。重新核对pojia-ops恢复仅验表数、备份仅应用库、057自动告警轮次规则和隔离1449证据，解释问题针对全新数据库缺DEFINER身份/权限，不是数据丢失。拟只补恢复必需身份/权限检查和隔离业务验收，不改CDK/订单/付款规则、不加页面。仅更新决定与交接，未实施修复、未改生产或凭据；恢复方案等待讨论确认。

## 2026-09-21｜D-324恢复检查修正完成本地验收

用户确认解释后批准最小修正。pojia-ops restore-test接同release的verify-restored-backup.mjs，不再表数>0就报成功。网络/容器标签/名字/端口隔离闸、057身份结构及半迁移检查、代表性Session/CDK解密、订单CDK关联、合成告警轮次1→2与回滚都进入正式路径。只在隔离容器prepare已知DEFINER：锁定登录、仅operator_alerts表SELECT/UPDATE/TRIGGER；已有账号权限不足与未知身份报错，不擅加权。修复清理trap登记时点，创建失败不删除同名既有容器。
...
## 2026-09-21｜D-330跨板块影响、核心支付与钱包跳转补充

用户要求整改必须考虑其他板块、支付能力不能退化，允许为优化计划性暂停；并提出钱包文字直达卡片高级highvcc开卡区钱包刷新，也接受更方便稳定路径。只读代码确认顶部data-rig-wallet与高级loadHighvccWallet都用GET /api/v1/admin/backup-cards/highvcc/wallet；前者结果位于会重绘的台账，后者在dataset.loaded保护的高级区域，两条均需验证重复刷新/失败反馈，不能仅凭位置认定更稳定。未点钱包/同步/开卡，没有生产操作。

统一规划增加G0影响面表、每批核心回归、D-254敏感代码边界、维护停止/恢复条件；钱包两条候选按准确定位、少步骤、失败不假报成功、无新增资金调用评估，不复制后台接口。A～E及本批处理完再用一天进⑦保持。当前仍是规划补充，不将用户维护许可当立即停业或改资金规则授权。

## 2026-09-21｜D-331确认开工，A第一批入口与失败反馈本地验证

用户确认按规划推进。钱包文字改为直达现有高级刷新按钮；token待办展开祖先折叠并聚焦原输入框，读页失败仍可修复登录，切页离开不抢焦点；余额读取显式返回成功与否，去掉失败仍提示成功及“不影响开卡”的无依据文案。只改admin静态资源与测试，入口版本v85，后端/Browser付款/资格SQL无变化，未发布。
...
## 2026-09-21｜用户要求可交互Demo，8805隔离演示已打开

用户希望实际点按，内容数据可用测试。新增scripts/feedback-demo.mjs，仅127.0.0.1:8805绑定且校验Host；复用实际admin.js/css/HTML，内存合成API，无DB/Provider/worker/真实凭据。四场景：正常、token失效、保存成功但读取失败、方式切换结果未确认。只演示本批工作台/卡片，其他模块导航禁用，未注册API统一403；付款开关POST实测返回DEMO_ONLY_此操作未接通。外部登录/书签禁止，输入token忽略不存不发。

19:00～19:03 UTC+8在应用内浏览器实点：token待办展开高级表单；钱包失败；填demo-token保存后刷新显示41.49测试余额；保存成功但读取失败场景返回“token 已保存，但状态读取失败”；未保存卡台时点Browser提示先保存/改回。原生点击工具曾返回错误但实际已跳转，读取DOM确认后改用浏览器定位器；场景切换reload中连续动作超时后按新DOM继续，不把中间态当失败结论。合成category由AVAILABLE改READY以符合前端合同，byProduct字段修正后两页样本一致。node --check通过、GET200。

Demo是用户试用产物，服务及标签保留，默认恢复token失效工作台。它不证明后端真实业务或生产已发布；场景中的凭据恢复为人工设定样本，不用于验定时告警周期。没有修改正式应用文件，本轮不需重新跑全套付款回归。原8803/8804/8899/常驻Browser池保持。

## 2026-09-21｜Demo体验后调整需求，先对齐不叠加改动

用户要求钱包原位显金额/刷新/登录，重新探索简便稳定的续期；认为卡源改动更难理解倾向不改，另提出去掉充值方式原生confirm、右下角反馈不明显；“4没看懂”指代待确认。代码核查只使用现有token没有续期实现，不能把旧滑块登录裁定冒充不存在续期端点的证据。11:38:57 UTC只读查两台钱包历史：HNSKJ今天11:37:58、highvcc09-19 00:53:18，旧金额需带时间不能冒充实时。新请求已入原规划/交接，未变更应用或Demo，撤回范围与下一原型待对齐，不做生产凭据/资金操作。

## 2026-09-21｜D-332明确撤回范围，第三批交互本地实现
...
## 2026-09-21｜D-336余额继续逐笔，Bark第一批本地降噪与简文案
...
## 2026-09-21｜钱包覆盖获认可，准备对齐成功率口径

用户认可D-338结果，未把该认可视为发布许可。22:51 UTC+8核对getOverview算法仍历史完结口径，14:51:28 UTC生产只读订单分布20/37/21、近7日5单1成功1失败3关闭。建议近期窗口/已证实演练排除/显示样本数/零样本“—”，普通未成功关闭沿现口径，待共同确认，未改算法。重写HANDOFF_NOW为当前快照，清除仍等待登录、仍待关2case等过时说法，保留23:20单次复测/身份指纹入口、已完成维护、未发布改动及剩余项，历史证据仍在原报告中。

## 2026-09-21｜D-339近7天成功率本地闭环，23:20复测因工具通道受限停止

用户确认近7个北京自然日、已结束分母、明确演练排除、普通失败/关闭保留、成功/样本展示与零样本“—”。聚合和`RECENT_FINISHED`列表共用predicate；数字墙点击清空旧搜索/日期，历史全部/已完成保留。无迁移/新服务/数据删改，D-254文件0未动。
...
## 2026-09-22｜D-344本批不修5X/20X关路提示

Lemon明确本次不操作任何修复：5X/20X待整套系统稳定后重点做，现在为关闭状态单独修提示没有意义。该裁定覆盖上一节“待批准的最小修正”：不改客户页、验码/建单服务或文案，不部署、不切路线、不动生产数据。

关路产品先收Session再在建单拒绝仍作为真实已知边界保留，但不再是第⑥步阻塞项；未来启动5X/20X时，将页面、验码、路线、真实开通与产品验收一起解决，不堆当前临时兼容。Plus审查结论不变，现有建单路线保护继续阻止造单、占码和付款。下一步转订单页需求讨论。

## 2026-09-22｜D-345订单页需求与换模型后业务校准

Lemon明确日常浏览最近客户充值、邮箱为主，关注时间/结果/卡及卡台/API或Browser，详情要对应CDK；合并与否未定。用户同意先恢复业务背景再讨论，未授权实施。核对基线、最新决策、列表/详情、状态投影、CDK恢复与建单规则：现列表订单号主行邮箱次行；详情CDK只有关联元数据无明文；051保留同码多订单历史，Session重贴则沿原单。状态细分和资金/运行信息并列是展示复杂的代码证据，不等于后台状态机需要重写。

已记录需求并纠正地图仍把C写成未开始的进度滞后。代码核对已完成；生产订单样本/当前UI未重新实测，原型、方案与实现未完成。后续先复述业务理解、继续共同裁定，不将日常浏览需求擅自解释成合并页面或合并数据。仅文档变更，无业务代码/部署/资金操作。

## 2026-09-22｜D-346历史失败展开查看

Lemon选择同一码最新充值主行、展开后看之前失败。仅记录展示需求，不删除历史、不按邮箱合并不同CDK、不改变统计口径；页面是否合并尚未决定。更新决定/任务/交接，无业务实现或生产写入。

## 2026-09-22｜D-347双时间与具体进度的优先级

补录Lemon已同意提交/成功时间同列两行、拥挤可移其中一个到详情；本轮明确希望具体步骤，但系统稳定高效第一。核对现有order-stage和orderRow：已有只读阶段及渲染，不需要据此假定新增付款流程；API/Browser细度不能伪造统一。本轮仅需求和代码依据落盘，布局、阶段映射与实现未完成，无部署或生产写入。

## 2026-09-22｜订单/CDK两版可点击演示

用户同意通过原型比较A两页联动/B同页分区。新增独立纯假数据HTML，复用8899；核心四条操作与桌面/手机浏览器检查已执行，独立代码审查无阻断问题，轻微键盘/空态问题保留披露。未选最终版本、未改正式系统、未发生产请求。完整范围与未验证边界见reviews/2026-09-22-orders-cdk-prototype.md。

## 2026-09-22｜D-348否定订单/CDK原型并校准正式CDK

Lemon明确A、B两版都没有真正理解现有CDK系统。经重新核对正式代码：CDK页面包含四格统计、普通/应急/卡网投放用途、生成起默认30天/自定义/不过期、批次备注、明文恢复、单码与批量登记已发出/延期/作废；订单详情包含CDK状态/批次/兑换/补发关系，CDK列表可按邮箱或订单反向跳订单。原型只换入口，未保留这些成熟流程，因此两版否定，不再选版或直接画第三版。更新交接与决策，未改业务代码、数据库、生产或部署。

补充纠正：上一轮把“同CDK重提产生的历史订单”简称成“同CDK多次充值”，表述不准确。正式规则是同一CDK同一时刻最多一个活动订单；只有无付款失败后才可能退回原码重提，同一进行中订单重复提交或Session重贴沿用原单；成功、付款已提交、付款未知或有资金证据时不退回。文档改为“同码重提/历史订单”。

## 2026-09-22｜D-349订单页结构Demo重做并验收

Lemon要求重新了解业务后再给方案并回复“同意”。按D-345～D-349制作结构Demo `step6-orders-structure-v2.html`：订单页作为客户充值运行台，CDK页保持完整管理职责；邮箱优先、同码最新主行/历史就地展开、兑换与成功分开、双时间、路线/卡台/卡尾分开、付款未知禁止重复充值、详情展示CDK并可定位。Playwright真实浏览器验收覆盖首屏、历史展开、付款未知详情和截图；首次发现历史按钮误开详情，已修正后重验通过。仅Demo和文档变化，未接生产、未改正式订单/CDK代码。

## 2026-09-23｜D-350订单页最小正式接线（网络恢复后续做）
...
验证：新增`v1/test/order-summary-contract.test.js`，加上现有admin-read/public/workbench测试，全量`npm --prefix v1 test`通过1014/0/70；CSS漂移检查也通过。网络恢复后重设本地step6_demo临时密码并持久在线程，8803真实登录验收订单摘要/处理中筛选与卡片页highvcc文案；样本无已分配卡，付款卡点击只能以代码/单测覆盖。卡片高级highvcc ranges因无token返回409，未重试或连接外部卡台。未部署。已提交推送`141c0dd`、`97c6420`、`8c3e8f6`。

## 2026-09-23｜D-351 Lemon否定D-350订单页交互，暂停发布

Lemon实际试用本地正式后台后反馈：订单页整体视觉与其他页面割裂；摘要卡片过大、占空间且不能筛选已充值订单；列表点击几乎都跳详情，主操作边界不清；点击付款卡直接打开V1卡片详情，没有先回答订单场景下这张卡与本单的关系和下一步。D-350因此不合格，不发布，不继续局部打补丁。保留代码和验收证据，下一步重新读现有后台视觉体系、业务任务与真实交互，再共同定新方案。

## 2026-09-23｜订单页工作块停止，完成交接收尾

Lemon明确本窗口不再由本执行者继续订单页工作块，只要求落盘和交接。D-345～D-351 继续保留为历史需求与失败现场，但不再推进订单页设计、第三版 Demo、正式实现、发布或局部修补；未来只有 Lemon 新的明确请求才能重新打开。

本窗口完成的可核实工作：

- 生产 `browser-mvp/scripts/state-check.sh`：release、web、worker、付款开关、接单开关、20X门槛、迁移均 `[一致]`。
- 订单页/工作台针对性测试：28 pass / 0 fail；V1 全量：1014 pass / 0 fail / 70 skipped。
- `v1/public/admin/assets/admin.js`、`cdks.js`、`diagnostics.js` `node --check` 通过。
- 只读核对生产订单/CDK/卡片关系：同CDK重提历史、同邮箱多订单、同一卡多订单均有真实样本；未执行生产写入或资金动作。
...
## 2026-09-23｜块 5 需求对齐稿完成（三路只读盘点）

设计侧：四页采用版与视觉硬规则 15 条、订单页需求台账 R-01～R-14、被否做法 X-01～X-07、硬边界 N-01～N-07、可复用模式 10 条。代码侧：订单页 7 列/4 摘要卡/隐藏下拉/整行点击；9 个 /orders 端点与守卫；人工动作 10 项及状态前置；详情返回未显示字段十余项；`=== 1` 布尔比较 bug；列表接口返回解密完整卡号。数据侧（14:26 UTC 只读）：80 单/30 邮箱/55 码，多单邮箱 76%，最多一码 12 次/3 小时；FAILED 37 + CLOSED 22 vs SUCCESS 20，7 单脚本标成功，7 单经 SUBMIT_UNKNOWN；ADMIN 事件 70 条中真正改状态的只有取消关单/手工标成功/付款不明判定三类；备注标签补偿退款 0 行；卡最多服务 17 单。产出 `tasks/2026-09-23-orders-page-three-questions.md`，8 个问题待 Lemon 答。
...
## 2026-09-24｜订单页第八轮（宽屏空洞）+ 内容封顶 1280 + DESIGN.md（01:3x～02:1x UTC+8）
...
Lemon 决定新开窗口；接班方式由他直接跟新窗口说，本窗口不做安排。本机常驻池 6667 / supervisor 61962 是块 3 正式池，保留不动；本窗口起的本地 8803/8899 演示服务已停。
...
## 2026-09-25｜块 6 合并发布 + 常驻池切固定目录（17:30～17:5x UTC）

过程与核实见 D-383。关键时刻（UTC）：17:41 prepare / 17:42:40 switch / 17:43 pool prepare / 17:44:10 关付款 / 17:44:15 停 42676 / 17:44:27 新 supervisor 52141 / 17:44:45 开付款 / 17:45:50 池 52721（固定目录）/ main ff `f748bb63` → `17d1e22`（plist 入库）。
...
## 2026-09-26｜D-391 复核归处理中（00:45～00:56 UTC）

Lemon：复核选 A；何时对外发卡密自己定。改 `STATUS_VIEW.REVIEWING`（tone ok、说明「正在人工确认,完成后这里会更新。」+ 关页不影响）；发现「付款确认比平时久」变体借用复核样式，另行固定为橙色以免被连带改绿。改写 houguang 两条锁定测试，加两条真浏览器测试，两处变异都抓到；截图两尺寸亲看。v1 1124：1059/0/65、ui-copy-check、customer-sql-probe 通过。00:55 prepare（备份 `pojia-20260926T005534Z`）→ switch `20260926-d391-54dd294` → 新连接复验三服务 cwd、线上 v=43 含新文案、web err 0。常驻池不动。

## 2026-09-26｜D-392～D-393 付款前重跑讨论 + 诊断页失败原因统计（01:00～03:10 UTC）

Lemon 问「付款前卡住立刻让客户重兑对不对」→ 查实重兑清空卡密与 Session、失败类型、同号成功间隔（15/21 只有 4 张在一刻钟内）→ 修正方案（重跑 1 次、按错误码）→ Lemon 定先不做；建议 2、3 放着。Lemon 要「失败原因要有记录、方便统计」→ 我先说「原话没进库」→ 查 09-25 的 k3QP 发现 D-375/D-380 已存 `navigationError`、D-239 续已存付款段 `diagnostic`，**更正**；Lemon 选诊断页统计表、付款池不改。
实现：`failure-stats-service.js`（四条只读 SQL，导出供测试 / 探针）、`GET /api/v1/admin/failure-stats`、诊断页分区（原型先改）、中文对照共用。演练判定第一版把 `worker_id` 为空的 5 张真付款单算成演练，生产实跑逐张核对后改规则。本机 `mysqld` 缺库起不了 → 用 `pojia-stage1-mysql` 容器自建库（RUNBOOK §2.8）跑集成测试，顺带补验 D-390 挂住告警真阳性。本地后台验收时登录被限流（15 分钟 5 次，重启本地服务清零后只登一次、存状态复用）。v1 1129：1062/0/67；几何比对两页一致。03:07 switch `20260926-d393-738204c`，复验接口 401、线上脚本、生产只读统计。清理：停两个本地服务、删自建库。

## 2026-09-26｜D-394 放弃并放卡 + 真数据库测试 + 已知未修复核（03:15～04:05 UTC）

Lemon：21 张老码自己作废；做 2、3。① 「放弃并放卡」：`close-rehearsal-order.mjs` 核心搬进 `pre-payment-closeout-service.js`（只读判断 + 加锁执行），脚本与按钮共用；发现成功率（D-339）靠事件里的 `closeRehearsalOrder` 标记排除演练 → 脚本照旧打、按钮不打；两套「演练」判定对比（生产上标记 ⊂ 运行方规则，差 2 张）交 Lemon。隔离库集成 3 项、六处守卫变异全抓；真页面点按钮发现放弃后抽屉仍写「付款可能已经开始」（关单只认 CANCELLED_PRE_SUBMISSION）→ 补上。03:43 switch `20260926-d394-0b5285f`。② 真数据库测试：14 项失败逐条查依据（D-217 / D-246 / D-352 / D-355 / f8c1d300 / 19ee0b8e），全部是测试落后；中途怀疑 hnskj 自动开卡登记卡缺充值额 → 查 `scripts/card-stock.js` 与生产，排除。拆规则的临时工具按 AND 硬切嵌套子查询失效，未采信其结论。新 `mysql-tests.sh`（每文件一库）、发布关卡（拿 HEAD~1 验证会停）。③ 已知未修 7 条：②④⑥⑦关闭，①③⑤仍在。清理：删 `pojia_it_verify` / `pojia_ab_verify`，停本地服务。

## 2026-09-26｜D-395 演练判定统一（04:10～04:30 UTC）

Lemon 选 A。规则移到 `rehearsal-order-sql.js`，成功率样本改用它；找全标记用处（成功率 SQL、两条测试、`test-recent-success-rate.mjs`、脚本 / 模块 / 手册注释）。生产只读对比：近 7 天 0/1 → 0/0（多排除的就是 `pcNK`），样本 0 时前端显示「—」（`admin.js:313`）。顺带比对两种「关单成功」认法，生产 27 张关单都是 0，只记不改。v1 1134 全过；隔离库成功率校验 5 项；发布准备真数据库关卡首次上岗 69/0/1。04:29 switch `20260926-d395-8b61c71`，服务器上用新代码只读读首页指标与失败统计复验。

## 2026-09-26｜D-394 ① 21 张老码作废（04:40～04:52 UTC）

Lemon：「你给我作废吧」。找正式路径（`close-historical-test-cdks.mjs` 是 D-343 另一件一次性收单，不适用）→ `cdk-service.updateCdkCodes`。当场重查 21 张（Plus 19 / Pro 20X 2，均未标已发出，15 张曾兑换失败退回）→ 服务器端按同条件取码并断言数量与构成后作废，21/21 → 新连接独立核实（第一次核实 SQL 我写错 GROUP BY 报错，改写重查）。事实表 5X/20X 行 Pro 20X 卡密改为无可用。

## 2026-09-26｜D-396 清「需要我处理」（05:00～06:12 UTC）

每周自检 34 过 / 2 提醒。「需要我处理」自检 9 vs 列表 11：查实是单位不同（订单页按卡密一行，1UfN/LDPg/UFi8 中两张收在同一卡密行下），非冲突。7 张续费未确认单的卡余额 $1.05～$1.80（续费扣不到），Lemon 确认当时已关续费 → 正式服务登记；3 张 CARD_DECLINED 单 run 停 PAYMENT_UNKNOWN、尝试 CLEARED → 改谓词（生产前后 3→0）；`_xH4` 取消放卡。发布 `20260926-d396-7a7998b`（关卡 69/0/1）。残留 BROWSER_PREFLIGHT 任务：手册 §3 脚本不覆盖（试运行 0 条），放着。

## 2026-09-26｜新窗口接班 + D-397 第一张卡密怎么发（06:25～06:50 UTC）

按 AGENTS 顺序读四份事实源 + D-352 / D-366 / D-391；`state-check.sh` 20 项一致、`wrapup-check.sh` 全过（唯一提醒命中 PROJECT_MAP 章节标题）；池 56510 心跳 06:31:57 vs 06:32:02 UTC。可用卡密接班一屏写 7、现查 8：多的是 D-396 取消 `_xH4` 退回的那张（订单 06:05 CLOSED、卡密 AVAILABLE），已更正（`22de7574`）。Lemon 同意第一张卡密先发给能等的熟人、约在场时间（D-397）。随后只读查第二单的卡：用生产 `walletPreflight` 现算 highvcc `ok:false`（$17.50 < 底线 $20）→ 事实表新行、交 Lemon。RUNBOOK §1 五处过时（①预检已并入 run / RECHARGE_FAILED 手工收口已支持 / Browser 付款不明走「确认核实结果」/ 客户页有更换账号表单 / 9839 已注销）按代码改正；中途我写了一句「CARD_READY 取消路径 09-26 走过」，回读发现 `_xH4` 当时是 WAITING_FOR_SESSION、证据不成立，当场改回「本次未查」。

## 2026-09-26｜D-397 补记：来单时充 highvcc 的方案按流程核对（06:55～07:15 UTC）

Lemon 选「来单时第一时间充」。按「客户任意时间来、运营不一定在」过四问：来单推送 ✓；充完 ≤1 分钟生效（readWallet 现查）✓；钱包不够不会再推（wallet-low 09-18 SENT 后一直 OPEN、只改消息不重推）✗；缺卡推送文案「调度器正在补」在补不了时也发（09-26 00:01:18 UTC 实推）✗。结论与缺口交 Lemon（D-397 补记）。本轮无代码、无生产写入。

## 2026-09-26｜D-397 补记二：开不出卡时叫人（07:20～08:25 UTC）

Lemon：「做」。先说验收再动手。读 D-190 / D-363 / D-365 与调度器、分卡、推送白名单、incident 触发器（057）后发现：只加「开不出卡就推」会与分卡重试每 60 秒关告警打架 → 每分钟响；故改为调度器独管等卡告警、分卡不再关。测试：先写错一条预期（没人等时按策略表顺序 hnskj 排第一、报 BLOCKED，是 D-365 排序），改测试不改代码；变异 8 个首轮 7/8，补「事务里才看到并发 job」用例后 8/8；真数据库新用例用 M2 变异确认确实在跑（34/1）。生产只读三查均 0。提交 `862b6bb6`，发布待批。

## 2026-09-26｜D-397 发布（08:20～08:30 UTC）

prepare（真数据库 70/0/1、清单 1400、备份 `pojia-20260926T082320Z` OK）→ 复核（无新迁移、非终态订单 0、运行中 0、开卡任务 0）→ switch 08:24 UTC。独立核实见 D-397 补记二「发布」。等首轮调度时本机 `sleep` 被拦，改为 until 轮询服务器上 runner 的退出时间。

## 2026-09-26｜ZovoCard：接入还是独立（讨论，未定；08:30～08:50 UTC）

Lemon 给了 ZovoCard 开放接口文档与一份白标卡密兑换页 + 发卡商城对接文档（均在其本机 Downloads，未入库），问接入还是独立，担心本系统过于保守拖慢它。读完：它是一整套「卡密 → Session → 直充」系统（预检 10 分钟一次性令牌、直充 202 异步 + 轮询/回调、拒付自动换卡最多 2 次可关、每卡默认 5 次可调、成交即自动取消续费、付款不明「勿重建订单」、幂等键、资金流水回调）；开放接口直充下单要手动给 `card_id`，卡密兑换走平台自动选卡。Chrome 只读看账号：余额 $0、未充值。结论交 Lemon：慢的是本机单窗口 Browser 与本方卡层，不是卡密 / 订单 / 客户页层；把它当「执行 + 卡」一条路线接入时这些慢处都不经过；只当卡台接给 Browser 用才会被拖慢。建议先花小钱实测再定。无代码、无生产写入。
Lemon 回复：接进本系统，但不受本系统运营后台规则约束 → D-398（原则、边界、待核对两条）。核实入口现状：所有订单一进来就建 ASSIGN_CARD、路线须挂本方卡台（`order-intake-repository.js:110-120,326`），故必须入口分流。
Lemon 追问「是不是更适合独立」→ 讨论（交集只有卡密与客户页且对方自带；接入要动本方管钱的入口；先独立更易回头）→ Lemon：「独立做吧，新建项目」→ D-399 作废 D-398。新项目 `~/code/ZovoCard直充` 已开工（首提交 `df98911`，参考材料入其 `docs/reference/`），`~/code/PROJECTS.md` 已登记（集合仓库提交）。
