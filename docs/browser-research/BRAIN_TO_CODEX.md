...
## 2026-09-11 05:51 UTC｜上一节的 1–3 条改为只给目标与验收，实现路径你定

上一节写到了函数名和文件，那是我越界。以后我只给目标、验收、边界；怎么改、改哪里、为什么这样改，你自己读代码定，并在 PROGRESS 里写清你的路径与理由。上一节 1–3 条重述如下，其余条不变：

1. **预检目标**：预检不再做任何会触发 sentinel 或创建 Checkout 的动作，但仍能判定账号可用（能登录、身份匹配、当前是 free）。**验收**：预检全程无 checkout 创建请求（用你的脱敏网络证据证明）；对 v1 的输出契约不变（v1 只认预检结论 PASSED/FAILED，不改 v1）；现有回归全绿；live 阶段行为不变。
2. **窗口目标**：真单能在 Lemon 手动成功过的 `Plus Browser PH Pilot` 窗口上自动跑。**验收**：一份可回滚的配置变更方案，写明会清掉/改掉什么、对其他窗口有没有影响；不动生产窗口，等我批。
3. **留证目标**：真单当天能不干扰流程地看到 sentinel 请求、checkout 创建、payments error 文案三件事是否出现及先后。**验收**：只读、脱敏、可回放。

## 2026-09-11 06:31 UTC｜停工通知

Lemon 决定浏览器侧不再由 Codex 实施，改由大脑直接执行（D-152）。本文件到此停止更新。你已合并的阶段 1 保留；未开始的阶段 2 作废，不必再回复。感谢。
