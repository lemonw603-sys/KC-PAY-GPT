...
## 8. 低调用次数策略（用户确认后的执行约束）

卡台开始限制 API 调用次数后，后续 Browser 与非 Browser 都按“事件驱动 + 快照复用 + 最小必要轮询”执行：

- 卡台账户、卡段、卡目录使用已有快照；未过期时不重复拉取。
- 卡片详情只在发现、状态变化、任务进入关键阶段或对账需要时读取；不在 Browser 每个页面动作前重复读取。
- 卡片余额和交易使用后台同步/对账任务，Browser 只消费 `cardReadyEvidence` 和最近同步时间。
- 充值提交后优先查询已有外部订单/卡片引用；未知结果不通过新建订单来“试探”。
- `pending/processing` 不需要每秒查询；采用递增间隔和上限，必要时转人工。
- Browser 页面流程中的登录探针、Checkout 读取和最终权益核对都采用每阶段一次，失败后停止，不循环打 API。
- 运营后台默认展示本地快照，并明确快照时间；只有人工点击“刷新”或调度器到期才访问卡台。

因此，状态不会为了少调用而删除；会把状态分层：本地状态负责记录，外部 API 只在状态转换或对账时调用。`SUBMIT_UNKNOWN`、`MANUAL_REVIEW`、`RECONCILIATION_REQUIRED` 不能省略，因为它们正是“少调用且不重复扣款”的安全状态。
