...
> 当前阶段：**D4 已完成现场核对，D5 被生产 release 混装阻断**
> 最后更新：2026-09-06  
...
| D4 部署前对齐 | 已完成，发现阻断 | 已复核 release/服务/数据库/资金、047→048 前滚和回滚边界；发现 27 个受控文件与 release 标注 commit 不一致 |
...
- 对生产受控目录按 release 标注提交 `04e08e6` 做全量 SHA-256：231 个预期文件中 204 一致、27 不一致，另有 2 个额外文件；生产是多提交混装 release，不能把目录名当整套代码版本。
- 混装 release 携带旧订单重试逻辑，曾每分钟重新生成并执行自动开卡任务。用户确认无需恢复该架构后，生产 timer 已 `inactive/disabled`，数据库自动开卡总开关已关闭；跨一个完整周期验证任务数不再增长。
- 生产当前 API 路线不接新单、Browser 路线接新单，但 Browser systemd Worker 停用且 heartbeat 为空；D5 前必须明确本机 Browser 执行器是否在线，不能只看路由开关。
...
D4 已完成现场核对，报告见 `CARD_SOURCE_D4_PRODUCTION_ALIGNMENT_2026-09-06.md`。下一步不是直接部署，而是先建立“单一 commit 完整归档 + 全量 manifest 验证”的不可混装发布候选，确保主线已存在的重复开卡修复不会再次被局部发布覆盖；随后重新做一次部署窗口 preflight，取得用户对 D5 的单独确认。当前仍未部署 migration 048、未导入生产卡、未启动 Browser 付款。
