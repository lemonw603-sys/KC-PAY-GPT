...
> 当前阶段：**D5 生产部署与只读验证已完成；D6 真实验收未开始**
> 最后更新：2026-09-06  
...
|---|---|---|---|---|---|---|
| NAME-01 | — | 已实现 | 已实现 | 已通过 | 生产代码已上线，页面待 D6 | 已部署待真实验收 |
| SRC-01～09 | 已实现 | 已实现 | 已实现 | 已通过 | 结构/配置已验证，行为待 D6 | 已部署待真实验收 |
| IMP-01～02 | 已实现 | 已实现 | 已实现 | 已通过 | 入口已上线，未导入卡 | 已部署待导入验收 |
| UI-01～02 | — | 已实现 | 已实现 | 已通过 | 生产静态代码已上线，待页面验收 | 已部署待真实验收 |
...
D5 已完成，证据见 `CARD_SOURCE_D5_PRODUCTION_DEPLOYMENT_2026-09-06.md`。生产已运行单一 commit release `20260906-card-sources-8012da8`，migration 048、全量 825 文件 manifest、备份/恢复演练、服务与数据库安全状态均已验证。未导入备用卡，Browser Worker 与付款仍关闭，旧自动开卡 timer 永久保持停用。下一步是 D6：先做生产后台/API 无资金验收，再导入明确选择的备用卡样本并进行真实订单验收；HNSKJ 当前只读目录接口返回 maintenance 403，不能把上游恢复前的状态误报为可用。
