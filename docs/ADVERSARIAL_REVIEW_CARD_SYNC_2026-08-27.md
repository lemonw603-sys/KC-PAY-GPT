...
运营确认：`1065` 与 `917` 两张卡均已实际消费过，一张用于 Claude 200 美元档位会员，另一张用于 ChatGPT Plus。根据既定规则，已消费/绑定订单的卡原则上不得重新分配；因此这两张卡不能作为可用库存。仍需保留其历史记录和订单交叉追溯，具体“哪张对应哪个产品”目前尚未从现有记录中单独核实。
