...
- Session 风险：凭证泄露可能导致账号接管、隐私泄露或未经授权的账号操作；任何报告只记录脱敏元数据和错误，不记录 Session 原文。
- 接单开启但派发关闭：不触发自动资金履约，但会创建真实订单、消耗 CDK 并产生生产订单数据。
- 派发开启但 Provider 写入关闭：不会直接完成 Provider 付款，但可能产生任务、租约、卡片分配或等待状态。
- 真实资金风险边界由 Provider 写入、卡台写入和单订单资金栅栏共同决定，不能只看接单/派发开关。
