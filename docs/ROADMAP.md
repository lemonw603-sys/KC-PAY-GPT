...
- [ ] 目标账号当前为 Plus 时必须在付款前有资格闸门并进入原订单 Session 可恢复状态；闸门可复用同一 Browser Context 的必要只读状态读取，不能未经 A/B 就新增独立探测请求。API 只有在存在明确低副作用只读接口时才增加预检；ZZSHU `40030` 继续作为安全拒绝兜底但不冒充本地预检。
