...
- 用户已在后台手动开启接单和自动充值/派发，准备第二单 API 灰度；三类 Provider 写入的最近独立核验仍为关闭，Browser 付款和自动补卡保持关闭。客户 Session 到达前不得开启 Provider 充值写入。
- 当前卡片规则已落地：`6807/1477` 保留原有 `ASSIGNED`；`4744/1065=PRODUCT_ONLY(claude)`；其余当前旧批次 17 张均 `RETIRED`。未来新卡不继承旧批次结论。
- 新接管的 `1628/6185` 已由用户手动补余额，后台显示“可直接分配 Plus · 余额 `$16.00`”，作为第二单 API 灰度候选卡。人工库存模式下不触发低库存提醒；只有真实订单等待卡片时才按订单提醒一次。
