...
- 生产接单、派发和 Provider 写开关仍保持关闭；本轮没有执行开卡、充值、付款或状态写入。
- 生产只读复核发现 Provider 卡 `493`（尾号 `8590`）有一笔 2026-08-18 的 OPENAI 成功交易；用户已确认该卡永久不可用，因此它不得再进入可分配库存，但生产当前仍显示 `active/AVAILABLE`，这是下一项必须修复的数据状态问题。
- 当前运营规则：尾号 `6807` 可用；尾号 `4744` 仅用于 Claude，不安排其他套餐；除此之外的现有卡（包括 `8590`）均属于同一批服务器更换导致的永久不可用卡，永不分配。该历史批次结论不限制未来新开卡，新增卡仍以实时 Provider 证据为准。
- 当前最近的执行顺序：先修正永久停用/个性化卡片规则并完成只读核验，再继续库存最小模型与后台展示收敛；Browser 线继续非付款工作，真实 Browser 付款仍需单独确认。
- 架构约束：这是个人内部使用系统，后续卡片策略修复采用最小字段和最少流程，优先稳定与资金安全，不建设额外的复杂策略服务或过度敏感信息隔离层。
- 工作区有未提交文件 `docs/BROWSER_RECHARGE_MODULE_REPORT_2026-08-25.md`，属于其他窗口产出，未擅自改动。
...
- [x] 完成后台配置入口、用量/剩余展示和修改审计；生产启用及现场验收未完成；
