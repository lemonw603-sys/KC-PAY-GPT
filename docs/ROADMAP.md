...
### 第二步还必须补一个逻辑缺口（2026-09-12 Lemon 指出「抓取不够准确，太多订单归为付款不明」后查到）

历史上 8 单进过 `SUBMIT_UNKNOWN`，最终 3 单标为 `HUMAN_VERIFIED_NOT_CHARGED`、3 单 `CARD_DECLINED`、
2 单成功。**但这些数字不能当作依据**——Lemon 2026-09-12 指出：历史上扣款不明的单他基本都是自己
手动付掉的，那段时期的订单记录反映的是人的处置行为，不是系统的判定能力（与
`no-intervened-data-as-autonomy-evidence` 同一类教训）。

**本条的真实依据是代码逻辑缺口，不是历史比例**——那个缺口下面写明，它与历史数据无关，读代码即可确认。

需要说清的是：**判成 UNKNOWN 那一刻本身是对的**——放手那一刻系统无法知道人会不会去点那个人机
验证弹窗（2026-09-12 那单，Lemon 若点了验证钱就真扣了）。问题不在判定时机，在**判完之后停在
那儿走不出来**。

逻辑缺口在 `browser-mvp/src/live-post-payment-recovery.js:93`：

```js
const plus = await verifier.confirmPlus();
if (!plus.confirmed) {
  return { outcome: 'UNKNOWN', reasonCode: 'PLUS_ACTIVATION_UNCONFIRMED' };  // ← 直接返回
}
// readCardTransactions 写在这之后，账号没开通时永远执行不到
```

**账号没变成 Plus 就直接判不明，根本不看钱动没动。** 应补全为：

| 账号是 Plus | 卡被扣 | 应判 | 现状 |
| --- | --- | --- | --- |
| 是 | — | `CONFIRMED` 自动收口 | ✓ |
| **否** | **否** | **`DECLINED` 自动收口（确定没扣款）** | **误判为 UNKNOWN** |
| 否 | 是 | `UNKNOWN`，必须人管（钱扣了没开通）| ✓ |

中间那一行是最明确的失败，却被当成最不确定的情况。

**两个缺口是叠加的，必须一起补**：只补逻辑而证据仍是占位假数据，等于拿假数据下结论，比现在更危险。
所以顺序不变——先第一步接通真实交易证据，第二步再同时补判断矩阵与节流。
