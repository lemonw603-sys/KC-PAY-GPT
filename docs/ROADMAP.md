...
- [ ] 以消费账本作为卡片—订单正式关联，移除新流程对 `cards.order_id` 一卡一单假设；旧订单提供兼容回退。
- [ ] 增加全局 1–4 次成功充值上限设置和后台入口，变更立即约束未来分配，不改写历史。
- [ ] API/Browser 在同一容量门禁下预留、消费、释放和 UNKNOWN 锁定。
- [ ] 订单驱动自动补余额优先补已有合格卡；无合格卡才进入自动开卡；定时器仅作兜底。
- [ ] 隔离 MySQL 验证一张卡连续服务多订单、达到上限停止分配、余额不足自动补足和并发互斥。
...
- [x] 本地 MySQL 8.4 空库迁移 001–043 全部通过；普通回归 441/441
- [ ] 更新并通过隔离 MySQL 多订单复用/旧兼容夹具
- [ ] 完成候选发布前对抗复查；生产资金写入仍未开启
