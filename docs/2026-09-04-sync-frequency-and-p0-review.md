...
受控并发第一版已提交 `6aff7fe`：只读同步 runner 默认最多 4 个并发槽位，硬上限 6；每个槽位独立领取租约，单任务失败不取消其他任务，进程等待所有槽位结束后再退出。未改变 Provider 写权限或资金逻辑。已通过 `node --check` 与同步/总览定向回归；生产尚未部署，需在隔离 MySQL 做并发租约集成测试后再发布。

隔离 MySQL 复验先发现旧测试库尚未执行 045（`Unknown column priority`），已仅对该隔离库执行完整迁移 001–045；随后库存只读同步集成场景通过。该过程没有连接生产数据库。

## 自动补余额首次生产触发（2026-09-04）
