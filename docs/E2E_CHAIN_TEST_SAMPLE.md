...
### 第 7 次（2026-09-13 07:21–07:32 UTC，**第一个真实客户单**，中途被我的 bug 卡住 10 轮后修复跑通）

订单 `PJV1-XrDZCfpakRwaoRQLrEbS`，卡 3159（卡段 `53211304`，当天新开），实扣 $15.72。

**这单和前六次不同：它是真实客户的单，不是 Lemon 自己提的测试单。**

前 10 轮全部失败，根因是我当天开卡时把 highvcc 返回的两位年份 `28` 原样入库，
执行器的过期校验把它算成 343 < 24321，判定卡已过期、拒绝付款（详见 D-194）。
**10 轮全是 `BEGIN_RUN → PRE_PAYMENT_ABORT`，没有一条 `PAYMENT_SUBMIT`，钱一分没动。**

修复（读取侧容错两位年）落盘后，supervisor 自己拉起 worker 加载新代码，这一轮的时间线：
...
| 07:31:52 | 卡台 `1572分 PENDING` | 与提交相差 18 秒 |
| 07:32:15 | `PAYMENT_UNKNOWN` | 客户页显示「支付已提交,正在和 ChatGPT 核对开通结果」（D-192 当天改的，正常色） |
| 07:32:26 | `PAYMENT_CONFIRMED` / `PLUS_ACTIVATED` / `CANCELLATION_CONFIRMED` | 同事务提交 |

**修复后这一轮 152 秒全自动跑完。** 订单总时长 665 秒，差值全是白跑的 10 轮。

**这一单额外验证了三件当天上线的东西**：新的客户态 `VERIFYING`（付款确认那几秒不再显示
「遇到点问题」）、匀速进度环、以及"卡材料预检拒绝可疑卡"这条保护本身是有效的——
它挡住的正是一张被错误记录的卡。
