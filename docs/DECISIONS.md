...
| D-095 | 一卡多充的第一优先级是可靠落实“每张卡已成功支付次数/剩余次数”，初步目标为每卡最多成功支付 3 次（暂不启用，仍待正式确认）。计数不能只依赖 Provider 交易同步：本地订单达到 `RECHARGE_SUCCESS` 且绑定该卡时必须先占用一次额度；Provider `PURCHASE/SUCCESS` 唯一交易用于确认、补账和发现遗漏，注资/余额转出不计入。订单关联因此是计数安全所需的占用事实，同时承担追溯和退款用途。 | 规划（待设计确认） | 2026-08-28 真实订单 `PJV1-FqFnMiSKBtLGN14GyP7W` 已成功但其卡本地交易同步尚未出现，证明仅靠交易同步会漏计；Provider 当前费率已只读核实为开卡费 0.50 USD、支付费率 0.5%、拒付费 0.40 USD |
