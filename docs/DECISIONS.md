...
| D-101 | Provider 卡 `493`（尾号 `8590`）被用户确认：2026-08-18 曾用于一个 Plus 充值；因卡台更换服务器，该卡此后永久不可用。该卡应作为历史已消费、永久停用候选保留追溯，不再进入可分配库存；在本地订单/Provider 交易主键证据补齐前，不自动回填消费账本，不删除历史记录。 | 用户确认的业务事实；系统状态变更待实施 | 生产只读审计已发现 Provider `PURCHASE/SUCCESS` 1 笔、余额 `$0.01`、本地 `order_id=NULL`；详见 `docs/CARD_CONSUMPTION_PRODUCTION_AUDIT_2026-08-28.md`。 |
