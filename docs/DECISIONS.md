...
3. **开/关接单、开/关派单不写 `admin_setting_events`**：Lemon 定「改」→ 已改（提交 `2a…` 见 git log：`updateSetting` 值有变化时写一行 old→new+actor，端点传 `req.admin.id`，单测 +1，v1 810/744/0）。**代码在 main、生产未发布**，随第④步 release 一起上；在那之前生产的接单/派单开关仍无审计行。
