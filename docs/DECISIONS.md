...
| D-077 | 将 Session 整理器升级为通用 `secure-inbox`：仓库外 AES-256-GCM 密文、macOS Keychain 密钥、`add/list/materialize/purge` 四个最小命令；通用层不解释业务、不输出原文，Session/卡台/证据由适配器按需读取 | 已实现（MVP） | 使用说明见 `tools/SECURE_INBOX.md` 与 `tools/secure-inbox.js`；当前只完成本地加密收件与临时取出，尚未接 Browser Session 适配器 |
...
| D-079 | `secure-inbox` 的用户界面收敛为单入口拖拽：文件拖到 `tools/secure-inbox-drop.command`，自动加密并只显示编号；`list/materialize/purge` 仅保留内部调试，不作为日常操作 | 已实现（入口） | 用户明确要求最简单、最便捷；拖拽入口已落盘并通过 Node/脚本语法检查 |
| D-080 | 用户入口进一步收敛为桌面弹窗直接粘贴：双击 `~/Desktop/安全收件箱.command`，粘贴内容并点击保存，工具加密后只弹出编号；拖拽仍作为文件输入的备用方式 | 已实现（入口） | 用户明确要求不经过文件拖拽，直接复制粘贴；AppleScript/Zsh/Node 语法检查通过 |
| D-081 | 正式用户入口升级为 macOS 菜单栏原生 App：点击顶部 `🔐` 图标打开多行 NSTextView，原样传递粘贴内容到本地加密核心；桌面 `.command` 保留为备用入口 | 已实现（MVP） | `tools/SecureInboxMenuBar.swift` 可编译，`~/Desktop/安全收件箱.app` 已构建并打开；未改变加密存储和 Browser 解耦边界 |
...
| D-069 | API 充值路径不增加独立账号预检；由充值 API 的权威响应判断资格。明确的 40030/账号不可充值属于充值前拒绝：保留审计、清除本地资金风险、订单回到 Session 可恢复状态，不删除账本、不重试、不换卡；只有外部结果未知时才进入 UNKNOWN 并禁止回退重付。 | 已确认 | 用户明确说明 API 本身就是充值动作，执行时发现账号问题即可回退；独立预检可能增加风控和重复访问风险 |
| D-089 | Plus 卡台路线在运营后台提供人工切换入口。切换只影响新订单，既有订单继续使用创建时冻结的路线；切换前要求目标路线只读检查开启、熔断关闭且不在重试窗口，并记录确认词、操作者和原因。暂不做自动故障切换。 | 已实现（后台 API + UI） | 036 迁移、后台服务/API、后台“卡台路线”视图和敏感操作确认已落盘；生产未迁移、未部署、未切换 |
...
| D-092 | 卡片库存同步采用 fail-closed：缺失 cardTypeId 只允许 Provider 快照唯一精确名称映射；历史 discovery 不自动重建；详情读取最多 3 次；AVAILABLE 每 10 分钟同步；低库存 Bark 同一 OPEN 事件只推一次，解除后重开才推送。 | 已实现、待部署验证 | 生产只读证据确认 1477 字段漂移、历史重复 discovery、同步新鲜度冲突和告警重推条件；合同见 `docs/contracts/2026-08-27_card-inventory-sync-contract.md` |
...
| D-094 | 未来可能从“一卡一充”扩展为“一卡多充”（同一卡承载 2–3 个 Plus 账号）。消费计数数据基础已实现，但跨订单自动复用仍未启用；当前卡片分配规则继续维持既有边界。账本记录卡、订单、attempt、产品、金额、Provider 交易 ID 和状态，Provider 交易同步只作确认/补账证据。 | 数据基础已实现；复用未启用 | 2026-08-28 用户提出后完成 migration 038/039、账本服务和资金链接线；现有 `cards.order_id` 与库存资格仍阻止自动跨订单复用，未擅自改变分配业务规则 |
| D-095 | 每张卡默认最多成功支付 3 次；付款授权/资金栅栏阶段必须在锁定卡片的同一事务中预留一次额度，付款确认后转为已消费，明确未提交失败才释放，UNKNOWN/付款后失败保留核对占用。Provider `PURCHASE/SUCCESS` 唯一交易用于确认、补账和发现遗漏，注资/余额转出不计入。 | 账本与 API/Browser 资金链已实现；生产未部署 | 2026-08-28 依据真实订单和并发风险实现；全量单元测试 442/442 无失败（37 项环境跳过），隔离 MySQL 账本并发 1/1、完整 MySQL 34/34、Browser MySQL 2/2 通过；详见 `docs/CARD_CONSUMPTION_LEDGER_IMPLEMENTATION_2026-08-28.md` |
...
| D-098 | 卡段采用“人工刷新目录 + 持久选择默认卡段”模式：运营点击刷新时读取并展示当前 Provider 卡段及费用；运营手动选择一个卡段后，后续开卡长期使用该选择，不要求每单选择。运营需要更换时再次调整即可。取消无意义的高频自动目录读取，降低 API 次数；新卡段仍先进入待确认，不会因刷新自动启用。 | 已确认，待实现 | 2026-08-28 用户提出；目标是减少 Provider API 调用并保持运营简单 |
...
| D-101 | Provider 卡 `493`（尾号 `8590`）已确认：2026-08-18 曾用于一个 Plus 充值；因卡台更换服务器，该卡此后永久不可用。该卡作为历史已消费、永久停用卡保留追溯，永不进入可分配库存；在本地订单/Provider 交易主键证据补齐前，不自动回填消费账本，不删除历史记录。 | 已确认业务事实；系统状态变更待实施 | 生产只读审计已发现 Provider `PURCHASE/SUCCESS` 1 笔、余额 `$0.01`、本地 `order_id=NULL`；详见 `docs/CARD_CONSUMPTION_PRODUCTION_AUDIT_2026-08-28.md`。 |
| D-102 | 当前卡片运营事实：尾号 `6807` 是当前可用于充值的卡；尾号 `4744` 虽可用但已专门用于 Claude，不再安排其他套餐。除这两张外的现有卡（包括尾号 `8590`）均因同一批卡台服务器更换而永久不可用、永不进入可分配库存。未来新开卡不受此历史批次结论影响，仍须按实时 Provider 证据重新判定；不得把 `6807/4744` 写成永久卡号白名单。 | 已确认运营事实与个性化规则；系统状态标记待实施 | 用户 2026-08-28 补充澄清：`8590` 与其他现有不可用卡属于同一永久失效批次；只有 `6807` 和 `4744` 例外。 |
