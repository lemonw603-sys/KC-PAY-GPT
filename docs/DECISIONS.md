...
| D-122 | `PHP 982.14` 不是不可实现的历史价格，而是已被真实成功 API 订单和运营方近期多单确认可成交的目标总价。当前 `PHP 1100.00` 只代表现有菲律宾 BitBrowser Profile + 当前 Checkout 信号组合产生 12% VAT；不得把它推广成所有菲律宾充值的固定成本。Browser 首笔真实付款前必须先用非付款 A/B 确认造成 `982.14/1100.00` 分流的定位信号，并始终以地址填写后的最终稳定总额作为付款输入。 | 目标价格已确认可成交；Browser 路径机制待验证 | 历史成功 API 订单 `982.14 PHP` 与卡台 PURCHASE 已落盘；当前 Browser Delaware 非付款结果为 `1100.00 PHP`。研究与实验边界见 `docs/browser-research/PHILIPPINES_VAT_ROUTE_DIFFERENTIAL_2026-09-03.md`。 |
