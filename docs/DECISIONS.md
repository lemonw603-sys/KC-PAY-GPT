...
| D-119 | Browser 第一版容量采用 6 个常驻 BitBrowser Profile：单 Profile 同一时刻只处理一单，Profile 间并行；订单间必须清理客户页面、Session Cookie 与站点存储，清理失败的 Profile 不得回池。冷启动物理窗口必须顺序打开，避免 BitBrowser Local API 部分成功；窗口连接后页面任务继续并行。最终 Checkout 税费/总额必须在填写卡片与账单地址后稳定，再绑定权威付款 permit 和 submit intent。 | 代码与真实 1→3→6 Profile 访问/隔离验收已通过，未部署、待共享队列非付款闭环 | 六个真实 Profile 同开 HTTP 200、Cookie/storage 隔离和运行时指纹摘要差异均通过；当前共用一个菲律宾出口。实施与现场证据见 `docs/browser-research/BROWSER_SIX_PROFILE_POOL_IMPLEMENTATION_2026-09-02.md`、`docs/browser-research/BITBROWSER_SIX_PROFILE_ACCESS_AND_ISOLATION_VERIFICATION_2026-09-03.md`。 |
