...
| D-094 | 未来可能从“一卡一充”扩展为“一卡多充”（同一卡承载 2–3 个 Plus 账号）。当前仅确认这是后续规划，尚未启用或确定具体计数规则。数据基础应记录每张卡的成功消费交易/绑定订单数量，并保留 Provider 交易 ID、产品、金额、时间和订单交叉关系；卡片是否可再次分配仍以可验证交易证据和运营规则为准。 | 规划（待设计确认） | 2026-08-28 用户提出；当前交易读取已能区分 `PURCHASE/SUCCESS` 与注资/余额转出，但尚未实现“一卡多充”计数与容量字段 |
| D-095 | 一卡多充的第一优先级是可靠落实“每张卡已成功支付次数/剩余次数”，初步目标为每卡最多成功支付 3 次（暂不启用，仍待正式确认）。计数不能只依赖 Provider 交易同步：付款授权/资金栅栏阶段必须在锁定卡片的同一事务中预留一次额度；订单成功后转为已消费，明确未提交失败才释放。Provider `PURCHASE/SUCCESS` 唯一交易用于确认、补账和发现遗漏，注资/余额转出不计入。订单关联因此是计数安全所需的占用事实，同时承担追溯和退款用途。 | 规划（待设计确认） | 2026-08-28 对抗式审查依据真实订单发现：仅靠成功后计数还存在并发超额风险；该订单 Provider 交易同步晚于订单完成；Provider 当前费率已只读核实为开卡费 0.50 USD、支付费率 0.5%、拒付费 0.40 USD |
