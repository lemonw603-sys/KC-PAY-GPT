...
| D-105 | 运营后台只提供一个全局“默认充值方式”入口，在 `API 充值` 与 `Browser 充值` 之间人工切换；不提供逐订单路线覆盖，也不要求运营每单选择。切换影响尚未进入履约的订单，已经建立 attempt、开始 API 调用或进入 Browser 执行的订单保持原路线并锁定；不得在付款结果未知时切换执行器。当前默认 API，Browser 完成生产灰度后可长期切为 Browser。 | 已确认，待实现 | 用户 2026-08-30 明确否定逐订单覆盖，要求一次选择后长期使用；实现复用现有 route/executor profile，不新建平行路由系统。 |
