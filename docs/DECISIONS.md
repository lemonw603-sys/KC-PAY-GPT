...
| D-123 | 美国出口可在单次明确允许下作非付款诊断对照，但不因此改变 Browser 生产出口保持菲律宾的决策。美国出口常规升级入口已实测切到 `US/USD`，Delaware 地址后为 `USD 20.00 + 0.00 tax`；该结果不是 `PHP 982.14` 复现。 | 非付款对照已完成，生产决策未变 | 若继续社区线索，下一实验必须显式分离“US 出口”与“PH/PHP Checkout 创建”；见 `docs/browser-research/US_EXIT_DELAWARE_TAX_AB_NONPAYMENT_2026-09-04.md`。 |
