...
## D-346（2026-09-22，UTC+8）同CDK历史失败展开查看，列表以最新充值为主

针对“同一码第一次失败、第二次成功，是两行还是最新一行展开历史”的问题，Lemon选择“展开后看之前的失败”。同CDK多次充值的默认列表以最新一次为主，过去失败记录在展开后查看；保留真实订单、时间、邮箱及付款证据，不删历史、不合并底层订单。不同CDK不因邮箱相同合成一条。该决定只确定展示颗粒度，不代表同意合并CDK与订单页面，也不更改D-339成功率统计口径。具体展开交互、日期筛选语义与异常保护仍待设计验证；未实现。
