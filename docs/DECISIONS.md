...
| D-124 | 显式地区 Checkout 观察必须复用当前官方 UI 生成的请求，只允许改写 `billing_details.country/currency`，保留官方动态认证、Sentinel、设备与目标路由上下文；单次观察最多向上游放行一次 Checkout 创建。禁止重新引入附件旧 token 裸调 API。 | 已实现，离线与上游前拦截验证通过；待一次零付款现场复验 | 官方请求拦截确认 bearer/Sentinel/设备/目标路由头均存在且上游请求为 0；新 helper 单测验证只改地区、保留请求头并阻断第二次请求。 |
