...
| D-042 | Browser 登录采用新的最小 Session Adapter：参考上号器的 Cookie 输入规则和 legacy 的分块/真实验证思路，但不安装扩展、不伪造 auth API、不补 Bearer、不以 Cookie 存在作为登录成功 | 已确认 | 静态分析确认扩展机制简单安全但缺少服务器/账号验证；legacy 代码能力更全但包含会伪造登录证据的拦截逻辑 |
