...
- **删补余额区块** → 会**断掉一条既有待办链路**：工作台队列有「卡补余额待人工 N 笔」会 `jump:'stock'` 跳到卡片页，删了就没地方看。生产实查当前为 0（`card_funding_attempts` 仅 6 行、全 FAILED、最后一次 2026-09-14），所以**当下删无害**，但工作台那条待办要一并处理。
