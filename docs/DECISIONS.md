...
| D-095 | 一卡多充的第一优先级是可靠落实“每张卡已成功支付次数/剩余次数”，订单关联不是启用计数的前置条件。初步目标为每卡最多成功支付 3 次（暂不启用，仍待正式确认）；计数以 Provider `PURCHASE` 且 `SUCCESS` 的唯一交易 ID 为准，注资/余额转出不计入。订单关联字段保留为可选，后续用于追溯和退款。 | 规划（待设计确认） | 2026-08-28 用户强调计数优先并提出 3 次目标；Provider 当前费率已只读核实为开卡费 0.50 USD、支付费率 0.5%、拒付费 0.40 USD |
