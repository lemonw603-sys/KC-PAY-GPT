...
| D-102 | 当前卡片运营事实：只有尾号 `6807` 与 `4744` 可用；尾号 `4744` 已用于 Claude，属于仅针对该卡的个性化保留要求，不再为其安排其他套餐充值。其他卡当前均不得按可用卡处理，仍需以后通过实时 Provider 证据重新判定，不得把这两个尾号写成永久白名单。 | 已确认运营事实与个性化规则；系统状态标记待实施 | 用户 2026-08-28 明确说明；`6807/4744` 仅代表当前状态，未来新开卡或卡台变化后须重新同步、验证。 |
