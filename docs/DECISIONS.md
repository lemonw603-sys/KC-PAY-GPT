...
## D-276（2026-09-18）V2 落实改「做 / 审 / 裁」三段流程；审查员固定用八种跑偏清单（Fable 额度收缩后的防跑偏机制）

Lemon 顾虑「其他模型做做就跑偏」；Fable 复核：跑偏根因是「没证据就下结论」+「顺手多做」，不是某个模型。**定**：每块 = 执行窗口做（Opus 5）→ Codex 只读审（`docs/reviews/`，按 `REVIEW_PROTOCOL.md` 新增的八种跑偏清单）→ Lemon 看 P1 → 有争议叫 Fable 裁。Fable 额度只用于裁决、第⑦块（碰付款链路）全程、最后集成验收。任务书粒度不变（窄、反例验收、白名单）。
...
## D-279（2026-09-18）CDK 板块七条需求进第⑥块（Lemon 定）；面五「CDK 页沿用旧页」作废

Lemon 直观三条（生成后结果分不清新旧；码前缀按产品 PLUS-/5X-/20X-；后台无按码搜索）+ Fable 补四条（单码列表批次作筛选；作废单张码；生成即复制；状态用客户页同一套说法）。全部写进 `docs/tasks/2026-09-18-impl-step6-workbench-and-settings.md`。前缀改动三处同改（生成 / 客户页验码 / 导入校验），旧 PJ 码按哈希继续有效。
...
## D-281（2026-09-18）后台前端重做但不换栈：原生 JS + CSS 按页拆文件，不引入框架与打包工具（Lemon 同意）

理由：单运营、四页、十几个现成接口，框架收益小；代价是构建链进发布、第三方依赖审计、接手门槛。条件：按页拆、公共小助手、沿用客户页规范。多人/手机/页面过十再议。写进第⑥块任务书。

## D-282（2026-09-18 13:xx UTC）⑤b 复审补修（Codex F-56~F-60 全采纳，含 F-51 一起做）+ F-47 与收窄一起发（受控打破 D-275 ⑥）；已发布

> 编号更正：本条原误标 D-281、与 D-281（前端不换栈）撞车，改 D-282。起因是补登记时没查编号占用——正好违反 D-280 里「新建区前先查占用表」那条纪律，自己撞了。
...
1. **营业条走方向 A**：按已选定原型 C 的形态＝**接单 / 派单 / 付款 三个 toggle**，**路线切换块留在工作台**（带面一 C1 的四项校验、拒切显示逐项原因）。据此否定两个走偏方向：`8cd6d7e` 把营业条做成「五决定按钮组」、在途 `7c1a5c0` 把路线切换挪去设置页（后者与任务书 A 项和 D-280 ⑦「卡台路线切换搬工作台」冲突）。供给参数（水位/开卡金额/每卡单数/钱包底线等）仍归设置页。
   > **事实更正（2026-09-19 实施时核查）**：本条「在途 `7c1a5c0` 把路线切换挪去设置页」**不成立**。真实情况是工作台一直有路线区（`#wb-routes`），但渲染的是**卡台选择**；设置页只有「开发中」占位、提到路线只是计划文字。真正的缺口是**路线切换（走 API / Browser）从来没有入口**——按钮 `.default-recharge-method` 全项目从未被渲染，且 `state.rechargeMethod` 从未被赋值导致 `VERSION_MATCH` 恒失败。裁定本身（方向 A、路线块留工作台）不变，已按此实施并三层验收通过，详见 DISPOSITIONS「续二」。
...
## D-285（2026-09-19）工作台数字墙与待办队列一律按原型 C 恢复（Lemon 定）

首次做成界面层验收（本地 v1 + 隔离库 + 真实浏览器 1280×900，与原型 C 同尺寸对比）后，把两处「实现与原型不一致」摆给 Lemon，他裁定**两处都按原型恢复**：

1. **数字墙五格按原型**：今日单数 / 成功率 / **自动完成率** / **今日花费（按台）** / **异常支出**。撤销在途版擅自换成的「可分配卡 / 待核对 / 昨晚的数量」（那是上一窗口把三个「待接入」占位换成手头有的指标，HANDOFF 当时已标「待与 Lemon 再对齐」）。**自动完成率仍按 D-284 ③ 先空着标「待接入」、不许自拟算法**；今日花费/异常支出若后端无现成聚合，同样标「待接入」，不许用相近字段顶替。
2. **待办队列按原型补齐来源**：原型 C 队列画了四类——无主扣款差异（看逐张／导出 CSV）、**待销到期**（逐张确认已删）、待登记手动用卡（我手动用了这张）、**token 状态**（贴新 token）。**「待销到期」与「token 状态」放回工作台队列**，据此**修正执行者先前把它们判为「卡片页范围、本轮不做」的处置**（见 DISPOSITIONS F-64）：原型明确画在工作台，F-64 应在第⑥块闭合，而不是推给卡片页。完整处理动作仍可在专页，但工作台必须有提醒、且跳转要带定位、有真实落点。

**依据**：原型 C 是 Lemon 已确认的视觉与交互基准（D-283），不因它用示例数据而失效；实现与它不一致时以原型为准，除非另有裁定（如 D-284 ② sidebar 皮肤）。

### D-285 补记（2026-09-19）：token 状态措辞有意偏离原型，Lemon 追认

原型 C 的队列里 token 那条写「HighVCC token 有效」。实施时核查发现**后端拿不出能证明「有效」的信号**：`/api/v1/admin/backup-cards/highvcc/status` 只返回 `configured` 与 `updatedAt`，而该 token **两小时不活动即过期**——`configured=true` 只说明「配过」，推不出「此刻有效」。把它显示成「有效」属于拿局部字段补业务结论（CLAUDE.md 明令禁止的那类错）。

**执行者据此偏离原型，Lemon 追认**：
- 有 `PROVIDER_TOKEN_EXPIRED` 告警 → 队列报「卡台 token 已失效，开卡会失败」（这是权威的失效信号）；
- 没有该告警 → **不写「有效」**，队列不出这一条（不制造虚假安心）。
- 单测锁住：`没有 token 失效告警时，不得擅自显示「token 有效」`。

**遗留局限**（已记 UNVERIFIED_LEDGER）：失效信号取自 `/admin/alerts?limit=100`，而 `listAlerts` 不支持按类型过滤、只按时间倒序；OPEN 的 warning/critical 若超过 100 条，token 告警可能被挤出而漏报。修法（加类型过滤参数或轻端点）留待队列后续那块。

**若将来要严格照原型显示「有效」**：需先由后端提供可证明有效性的信号（例如一次真实探活，或记录最近一次成功调用卡台的时间），不得由前端从 `configured` 推断。

## D-286（2026-09-19）CDK 补两个功能：交付负债可见（已发出标记）+ 码有效期与下线保护（Lemon 定）

执行 D-279 期间，执行者基于现场核查提出 CDK 的两个缺口，Lemon **全部同意加做**，并入第⑥块 CDK 这一轮（D-279 六条之外）。

**① 「已发出 / 还在手里」必须分得开——否则说不清欠客户多少次交付**
- 现状：`cdks.status` 只有 `AVAILABLE`/`REDEEMED`/`REVOKED`，表里**没有任何「何时、发给谁」的字段**（完整列已核：`id, code_hash, hash_version, status, batch_no, plan_type, order_id, created_at, redeemed_at, revoked_at, revoke_reason`）。于是 `AVAILABLE` 混了两种性质相反的东西：**已售未兑＝负债**（欠客户一次交付）与**未售＝库存**（还能变现）。
- 后果：一天几十上百单后答不出「我现在欠多少次交付」——这直接决定备多少卡、钱包留多少钱、能不能安全停业务；也答不出「还有多少能卖」。
- **补不回来**：等码发多了再想追溯「哪个码何时发给谁」，没人记得。所以趁这轮在改 CDK 表和页面时一起做，边际成本最低。
- 做法：`cdks` 加 `issued_at`（NULL＝未发出）+ 去向备注；「已发出」是与 `status` **正交**的属性（已发出的码在被兑换前仍是 `AVAILABLE`），**不塞进 status 取值**。

**② 码没有有效期，产品下线后变「哑弹」**
- **已真实发生过**：D-245 关掉 305/306 后，20X 的码仍在客户手里、状态仍 `AVAILABLE`，兑换会撞 `ORDER_ROUTE_UNAVAILABLE` 503；客户体感是「买的码用不了」，而后台**无法主动识别哪些码当前兑不了**，只能等投诉。
- 做法：`cdks` 加 `expires_at`；「当前可不可兑」运行时按产品路线的 `accepts_new_orders` 判断并在列表标出，不落成静态字段（路线随时会切，静态字段必然过期）。
...
## D-288（2026-09-20）卡片页 D-280 ②④⑧ 落地；highvcc 余额改「全部按需查」；⑦ 两条因有副作用暂缓（待 Lemon 定）

**核查推翻我先前两处说法，并查出一个我自己造成的回退**：

1. **④ 根本不需要新增端点**：`GET /api/v1/admin/cards/:providerCardId` 早就返回完整流水（`card_transactions` 200 条 + 状态事件 + 分配历史），详情抽屉 `openCard()` 也早就有。我此前报「需新增按卡查流水端点」是错的。
2. **我上一轮造成的功能回退**：把卡列表重做成表格时删掉了每行的 `data-card`，而点击处理器绑在 `document` 上仍在——**「点开看流水」这个既有入口静默断了**，审查员没抓到，本轮自查发现并接回。
3. **`loadHighvccStatus` 随 `loadStock` 无条件跑**，token 配了就查 `/highvcc/wallet`，即「打开卡片页就打外网」。我先前说「打开页面不打外网」同样是没核查既有代码就下的结论。**Lemon 裁定：全部按需查**——页面加载不查，展开开卡区或点按钮才查。

**② 开卡不重做**：两台的开卡 UI（金额/卡段/费用预估/确认闸门）早就在折叠区里完整存在，台账栏的「开卡…」只负责展开并滚过去，**绝不另造一个会花钱的入口**（有测试守着不许发起 `card-stock/jobs` 或 `highvcc/open`）。
**② 新增唯一真缺的端点**：`POST /api/v1/admin/backup-cards/highvcc/refresh`，与 CLI `sync-highvcc-snapshot --commit` 同一个服务、同样三步（快照/钱包/流水），每步独立报成败（有失败返 207）。不花钱故用 `adminWriteGuards`；比 hnskj 慢得多（list + 每张 detail），前端提示「要几十秒」。
**④ 流水↔订单**：只认账本里明确记了 `provider_transaction_id` 的，没记的留空，不按时间相近推断（Lemon 硬约束）。**用相关子查询而非 JOIN**——账本这列没有唯一约束，JOIN 一旦遇重复会把流水行乘出来；`ledgerOrderCount > 1` 时显示「账本记了 N 单」而不是随便挑一个。**生产覆盖率：76 笔流水中仅 7 笔能关联（账本 71 行只有 7 行记了交易号），其余 69 笔订单列留空。**
**⑧** 导入备用卡降为折叠高级入口「接入无 API 的卡台：上传导出表」，能力保留。
**⑦ 第三条「卡片录入审核降高级入口」本来就已满足**（新卡接管记录早就是 `details`）。

**⑦ 另两条暂缓，因为照字面执行会有副作用，摆给 Lemon 定**：
- ~~删补余额区块~~ → **Lemon 2026-09-20 裁定：删，工作台那条待办一并拿掉**，已执行（提交 `033df99`）。后端、数据与 `operationalBacklog` 计数都没动（`card-funding` 仍在三类真金白银弹窗里），将来恢复入口时连待办一起接回。
- **卡台路线切换搬工作台** → 会**丢掉「同时接管 N 单」能力**：卡片页的切换按钮支持 `data-takeover="true"`，而工作台的 `#decision-card-source-apply` 写死 `takeoverWaiting: false`。要搬得先把接管能力补到工作台。

**验收**（隔离库端到端 + 真实页面）：底线 $30/$20 取自 `wallet_floor`；`TX-LINKED` 显示订单 `PJV1-TXLINK`、其余三笔留空；**配好 token 后页面加载 wallet 调用 0 次、展开开卡区后 1 次**（决定性验证）。新增 8 条守门测试，全量 951/883/1（仅 F-65 有意留红）。

> **过程自纠**：seed 用 `INSERT IGNORE` 吞掉了 `orders`/`card_consumption_ledger` 的插入失败（缺 `cdk_id` 等必填列），脚本照常打印 `seeded`，我差点把「订单没显示」当成代码 bug 去改。**又一次 E 类（不验证工具失败时说什么）。**

## D-289（2026-09-20）D-280 ⑦ 收尾：卡台切换搬工作台（先补接管能力）；并记一次「测试全绿而后台不能用」的误删事故

**⑦ 最后一条落地**：Lemon 定「先把接管能力补到工作台，再把卡片页改只读」。
- 工作台新增接管勾选，**三态分明**：有排队单 → 给勾选并写清勾与不勾各自会发生什么；0 单 → 「当前没有排队等卡的单可接管」；读取失败 → 「待接管单数读取失败」**而不是静默当成 0**（吞成 0 会让这个选项在卡台断供那天悄悄消失）。
- 切换逻辑抽成 `applyBrowserCardSource`，把卡片页原有的**三条安全行为完整搬过来**：①切成功后读失败仍算成功（说成失败会让人重复点）；②响应丢了只说「未能确认」，**绝不说「原选择未改变」**（可能已经切了）；③被四项校验拒绝才说「卡台没有改变」并逐条给原因。**原来工作台只有一句笼统的「卡台切换失败」，等于在声称没改变——是两条既有测试把这个退化抓出来的。**
- 卡片页那张表改只读（「可切换 · 去工作台切」），同一个写操作不再有两个入口。

**同轮事故：951 条测试全绿，而后台已经不能用了**

删 `loadCardFundingAttempts` 时我用「下一个 `async function`」当删除边界。两个函数之间夹着 **14 个顶层事件绑定**，被一起切掉：侧边栏导航点不动、订单筛选与分页失灵、导出和对账表全哑、`document` 级委托整个没了。

**为什么没被发现**：`node --check` 只查语法——切掉若干完整块之后语法仍然合法；其余测试走 snippet / DOM harness，不碰这些顶层绑定。**「全量 951 绿」是真的，「后台能用」是假的。** 是界面验收时「点卡片页没反应」才暴露，而且我一开始还误判成视口坐标问题，绕了好几步才查到真因。

**恢复**：从 `1f60b8c` 按顶层块边界（行首的 `}` / `});`）精确切出 16 块，只剔除该删的 2 块 funding，其余 14 块原样补回；逐块 `node --check`。与上一提交比对确认现在缺的**全部是有意删除的**（funding 五处 / providerRoutes 切换 / startBusiness 死代码 / renderDecisions 改签名），无误删残留。

**新增守门测试**「后台的关键顶层事件绑定必须都在」，按存在性钉住 9 个绑定，并**反向验证过它会抓人**（删掉 navItems 那行即精确报「侧边栏导航」）。

**三条教训**（已写进记忆）：
1. **删代码要按语法边界，不能按「下一个某某」的字符串距离**——那不是边界，只是距离。
2. **`node --check` 通过不代表没删错东西**：删掉完整的块，语法永远是对的。
3. **UI 改完必须真点一遍**。这次是唯一发现问题的途径；任何测试都没抓到，也抓不到。

## D-290（2026-09-20）设置页采用 B「一张大表」；策略表与钱包底线第一次有写路径；每卡单数按产品暂不做；5X 最低余额应为 $95（待改）

**比稿**：`docs/design/prototypes/step6-settings-compare.html` 三版（A 按台分组 / B 一张大表 / C 按用途分组），Lemon 挑 **B**——6 行摊平，两台哪里不一致一眼看得出来。

**前置核查的最大收获**：任务书「D 发现 1」牵出我上一轮的 bug（卡片页水位/日限读了被取代的全局键，见上一提交），并查清了每一项的真实来源：

| 项 | 真实来源 | 写路径 |
|---|---|---|
| 水位 / 开卡金额 / 每日上限 | `card_supply_policies`（台×产品，调度器读它） | **此前没有，本次新建** |
| 钱包底线 / 告警线 | `provider_accounts.wallet_floor` / `wallet_alert_threshold` | **此前没有，本次新建** |
| 最低余额 | `minimum_required_card_balance:<plan>` | 已有 |
| 每卡单数 | `card_max_successful_payments`（全局） | 已有，但不按产品 |
| Session 门槛 | `session_replacement_window_hours`（生产 72） | 暂只读 |

**前两组此前只有读、没有任何写端点**——真正决定「何时自动开卡、开多大金额、钱够不够」的数，过去只能改库。

**安全**：列名不能参数化，所以字段走白名单（策略 3 个 / 钱包 2 个），值全部参数化。实测 `updated_by`、`account_code`、`'target_available; DROP TABLE cards'` 均被拒。写入走事务 + `FOR UPDATE` 读旧值 + **仅在值真的变了时**写 `admin_setting_events`。

**交互**：**不做失焦即存**——这些改错直接影响资金。改完先变色、出「保存」，点了才写，且只提交真的变了的格；一格失败不掩盖另外几格已经存进去了（逐格报）；存完重读一次，让页面显示的是库里的值而不是刚敲进去的字。

**两处缺口的裁定**：
1. **每卡单数按产品（D-221：Plus 3 / 5X 1 / 20X 1）——Lemon 定本轮不做**，设置页只读显示并写明缺口。⚠️ **5X/20X 自动化接入前必须补上**（CLAUDE.md 硬约束已写）。
2. **5X 最低余额应为 $95**（Lemon 2026-09-20 定；生产现值 16.00，而 5X 开卡金额 $100 —— 按 16 的门槛可能分出一张钱不够的卡）。**尚未改**：这是生产配置变更，等设置页上生产后用页面改（有审计），改前单独确认。20X 现值 150.00 未提出异议。

**验收**（隔离库端到端 + 真实页面）：改 HNSKJ·Plus 水位 2→4，库里确实变成 4、`updated_by=admin`、**backup-a 未受影响**；审计写了 `supply_policy:legacy-primary:plus:target_available` old=2 new=4；钱包底线 30.000000→35.50；四类非法输入与两类非法字段全部 400/404；读取失败显示「先不要照这里的值做判断」而不是「还没有配置」。新增 7 条测试，全量 963/895/1。

## D-291（2026-09-20）对照规划核查：第⑥块范围我理解偏了；Lemon 定先上生产再进 ⑦，D 里只先修 D5

**方向偏差（我的）**：任务书第⑥块是 **A 工作台 + B 设置页 + C UI/交互 + D「归入本块的 7 条发现」**。我把它理解成「四个页面」，A/B 做完、C 出了比稿，**D 的 7 条只做了 2 条**——因为它们不属于页面，做页面时自然漏过去了。

| D | 内容 | 实查状态 |
|---|---|---|
| 1 | 清掉失效的库存阈值 / 全局日限 | ✅ 本轮（改读 `card_supply_policies`） |
| 3b | 手动用卡登记＝标 RETIRED override | ✅ |
| **5** | `ORDER_WAITING_FOR_CARD` 两处共用 dedupe_key | ✅ **本轮修**（见下） |
| 2 | 「付款前失败、放弃并放卡」按钮（契约表三 #3） | ❌ 后台零引用 |
| 3 | 建演练单的正式脚本 | ❌ 只有关演练单的 `close-rehearsal-order.mjs` |
| 4 | `pojia-bark-notifications` 生产是哑的 | ❌ |
| 6 | ZZSHU 零原因失败「停单不退码」 | ❌ |
| 7 | `state-check.sh` 偶发取值失败 | ❌ |

**页面本身的遗漏**（规划要求但未做，非有意）：数字墙「今日花费（按台）」「异常支出」两格仍是「聚合待写」；「卡用了几张剩几张」只做了 Plus，规划要的是**按台按产品**；Session 门槛做成了只读，任务书 B 列的是可配项；日对账看板 4 个数，任务书要求分开的三类里未见 `AWAITING_RESOLUTION`。
（**有意留空、不算遗漏**：自动完成率 D-284 ③；每卡单数按产品 D-290。）

**两个方向问题**：
1. **四个页全做完才上生产，风险集中在一次发布上**；而任务书 C 的验收条件「含 Lemon 实际用一天的反馈」在生产没上之前**根本无法满足**，⑥ 上一次正是「发布后当天回滚」。
2. **⑦ 有前置未做**：8 步表 ① 里的 `pro5/pro20` 映射验证「并入第⑦步」。

**Lemon 裁定（2026-09-20）**：① **先把第⑥块上生产、真用一天，再进 ⑦**；② D 的 5 条未做的里**只先修 D5**，其余 4 条偏运维、排到后面。

**D5 修复**：实查坐实两个产生点用**完全相同**的 `order-waiting-card:${orderId}`，且都 `severity=VALUES(severity)` → 真「开不出卡」的 critical 会被「补卡重试中」的 warning 降级。查清后发现比任务书描述的更微妙：`workflow-repository` 自己已会判断（有补卡在途就 RESOLVE、没有才写 critical），冲突只来自补卡任务失败时写的那条。改法：后者拆成 `ORDER_REPLENISH_RETRYING` + 自己的 key；**不进推送白名单**（理由写进 `NON_PUSH_REASONS`：它说的是「会继续尝试」、没卡住客户，而调度器开卡失败另有 `CARD_SUPPLY_OPEN_FAILED` 在推，再推就是第三遍）；订单取消时的 RESOLVE 同步覆盖两个类型与两个 key。生产存量核查：该类型现有 14 行**全部 RESOLVED**，无 OPEN 行受影响。

## D-292（2026-09-20）营业条与原型对不上的真实差距＝2 处；建「视觉一致性比对」机制，肉眼验收改成机器比数字

**起因**（Lemon 原话）：「为什么设计图和实际做出来就有这么大差距呢……你只解决了眼前问题没有用，根源是你实际做出来的和设计有差距，你却不知道，这是**机制和底层问题**，你需要彻底解决。」——关键在**「却不知道」**：不是手艺问题，是**验证方式本身查不出差距**，只能等 Lemon 看见。

**先查清「为什么不知道」**，四个坑，任一都足以让视觉判断失真：

| # | 坑 | 后果 |
|---|---|---|
| 1 | 视口 emulation 会被悄悄清掉（回合结束、窗格宽度变化） | 实测：先 resize 后 navigate → `innerWidth` **280**（命中 `max-width:640px` 堆叠规则）；反过来才是 1440。**我一直在看三段竖排的窄屏版，Lemon 看的是三段并排的宽屏版** |
| 2 | 用 CSS `zoom` 放大看细节 | `zoom` 参与布局，放大 2 倍＝可用宽度砍半，三等分段被挤成两行。这个"缺陷"是放大手法自己造的 |
| 3 | `computer zoom` 的 region 裁剪在 Browser pane 不支持 | 静默退化成全屏截图，且缩到 800×500 ＝真实分辨率的 **55%** |
| 4 | 前三条都躲开了也没用 | 「段高差 5px」「纵向对齐差 23px」肉眼在缩水截图上判不了 |

第 1 条最致命：**它不报错，只是安静地给你一个不同的世界**。当轮它两次拦下本会做错的判断（断言 `innerWidth` 时抛 280）。

**真实差距（实测，非目测）**——把原型丁版和实现放进同一 1440 视口逐项量：

| 项 | 原型丁版 | 我的实现 | 判定 |
|---|---|---|---|
| 三段纵向对齐 | **居中**（colTop=colBot：34.5/12/29.5） | 顶对齐（底部空 **52/12/42**） | ✗ 实现时擅自换了对齐方式 |
| 路线段两行间距 | 7px | 2px | ✗ 段高因此差 5px |
| padding / 字号 / 字重 / 字距 / 标题 margin / 行内 gap / 控件高度 | — | — | ✓ 全部一致 |

改这两处后三段几何量与原型**完全吻合**（115/34.5/12/29.5 逐项对上）。Lemon 报的三处观感问题（开关拥挤、两条按钮不成比例、下拉与切换割裂）此前已修，本轮是它们之外的第 4、5 处——**正是"我不知道"的那部分**。

**机制（本决策的重点）**：`scripts/visual-parity.mjs` + `docs/design/parity/*.json` 契约 + `wrapup-check.sh` 第 9 项。做法写在 `docs/design/VISUAL_PARITY_PROTOCOL.md`。

- **零依赖**：系统已装 Chrome（headless）+ Node 内置 WebSocket 走 CDP，**不装 puppeteer**（不为一个验收工具往项目里塞运行时依赖）
- **只比结果量**：`height` / `contentTop` / `contentBottom` / padding / 字号字重。`contentTop`/`contentBottom` 性价比最高——**顶对齐、居中、底对齐的差别全在这两个数上**，而这正是肉眼最容易放过、观感上最扎眼的一类
- **结构不同构的量不比**：原型 `.col` 包着标题和内容，实现里两者平级，比 `gap` 必误报；两边容器宽度本就不同（1162 vs 1126），`width` 不比
- **退出码分三档**：`0` 一致 / `1` 有差异 / `2` 跑不起来。**`2` 必须单列**——「环境坏了伪装成通过」是这类工具最常见的失效方式

**工具自己先做变异测试**（项目既有规矩：判断工具的假阳性比报错危险）：撤掉居中 → 报出 4 处（差 23px/17.5px）✓；行距改回 2px → 报出 7 处（差 5px）✓；恢复 → 通过 ✓。
**自测抓出了脚本的真缺陷**：`#wb-routes` 是接口返回后才渲染的，`load` 事件后立刻测会时灵时不灵（连跑 5 次挂 4 次）——补 `waitForSelectors`。**不做变异测试只会看到一个偶尔"通过"的脚本。**
第二个缺陷同样由连跑暴露：登录接口限流 5 次/15 分钟，每跑一次重登一次 → 第 2 次起全 429。补 session 本机缓存（600 权限、12h 过期、从不打印），现在连跑 7 次全绿。

**仍然要用眼睛的**：颜色、留白节奏、信息层级、文案啰嗦（Lemon 明确要过：**界面上不要写解释旁白**）、交互是否割裂。但用眼睛前必须：先设视口且**测量前断言 `innerWidth`**；放大只用 `transform: scale()`，**绝不用 CSS `zoom`**；知道截图被缩过，细节结论让数字兜底。

**适用范围**：Lemon 当轮定「如果做的好，以后你也用这套做法」。卡片页、CDK 页重做时各加一份契约。

## D-293（2026-09-20）营业条定为乙-3「一条工具栏」；比对机制被自己的变异测试拆穿并修好；三套 CSS 收敛立棘轮

**一、营业条改乙-3**。Lemon 看过丁版实现后说「仍然不协调」。实测三处客观证据（同一 1440 视口）：三个小标题不在同一水平线（y **36/13/31**）；三段等宽 375 而内容宽 216/195/272，右侧留白 **122/144/68**；分段器里的按钮 24px 而旁边控件全是 28px。前两条同根：**三段内容量天然不等（1 行/2 行/1 行）却硬切等宽**——居中换来标题错位，顶对齐换来底部空洞，前后两版各踩一个。

出七版给 Lemon 挑（`step6-opsbar-v5.html`）：甲两段不等宽 / 乙一条工具栏 / 丙维持三等分只拉平标题 / 丁左右两张卡，乙又出三种分布。Lemon 先选乙、指出「除了生成，其他的有点挤在左边」（实测组间隙 **0/0/156**），再从三种分布里选 **乙-3：间距均分、不画竖线、靠留白分组**。

落地实测：条高 **117px → 56px**（省下 61px 给数字墙），四组间隙 **48/49/49**。结构从「三段 `.wb-seg` + 小标题」改成「四组 `.wb-grp`」，`#wb-routes` 用 `display:contents` 好让它渲染的两组直接做工具栏的项。

**二、比对机制被自己的变异测试拆穿（本决策最要紧的一条）**。为让原型不失真，我让原型直接 link 真实 CSS（经 `proto-server.mjs` 反代）。**后果是原型跟着实现一起变——参照物跟着被测物走，比对永远通过。** 变异测试当场证明：把 `justify-content` 从 `space-between` 改回 `flex-start`（正是 Lemon 说的「挤在左边」），脚本仍报「与原型一致」；改 padding 也照样通过。**没有这一步，我会带着一个永远通过的脚本继续往下走，还以为有保障。**

修法：原型 link `_frozen/opsbar-v5/` 里的**定稿快照**，不是实时样式。只在重新定稿时更新快照；日常改实现不更新——实现偏离快照正是要抓的东西。修完四个变异全部抓到（justify-content / padding / 按钮高 28→34 / 分段器按钮 24→28），恢复后通过。

过程中原型侧还暴露两处**原型自身失真**（都是实现对、原型错）：只 link workbench.css 时数量框 30px 而实现 28px，差的 2px 是边框，因为 `*{box-sizing:border-box}` 写在 admin.css——原型必须按 index.html 的顺序加载三份；原型用自己的 `.grp` 而非 `.wb-grp`，导致 `.wb-ops .wb-grp .wb-btn{height:28px}` 在原型侧落空、按钮回落 34px——**定稿那一版原型必须一个原型专属 class 都不带**。

比对脚本同时加了 `left`/`right`/`flexWrap` 采集与 `equalGaps` 自洽断言（「各组是否均匀铺开」只比每组宽高是看不出来的，挤在一边和均匀分布量出来一样；间隙又不受内容宽度影响，比绝对位置更适合守）。不比 `width`/`left`/`right`——下拉里是真实卡台名，两边文字本就不同。

**三、三套 CSS 收敛（Lemon 定「并且持续更新优化」）**。规范：`docs/design/DESIGN_SYSTEM.md`，以候光令牌为基底，控件三档 28/34/44。实测起点：`admin.css` 298 行 10 token **157 处字面色**、`workbench.css` 272 行 12 token 5 处、`cards.css` 80 行 **0 token** 65 处；控件高度散成六种。

当天已付的代价写进了文档：`.wb-cdk` 在同一文件里定义两次（center/gap8 被 flex-end/gap10 盖掉，发码组因此与其余三组不同步）；数量框宽度 `.wb-num{52px}` 与 `.wb-cdk input{72px}` 打架；`box-sizing` 只在 admin.css。这些都在本轮一并清掉。

**不搞一次性大重构**（旧页在生产跑着，风险大于收益），改为棘轮：`scripts/css-drift-check.mjs` 记下基线，**只许降不许升**，跟着每次重做逐块吃掉。棘轮自己也自测过，并修掉一处假阳性——第一版把 `@media` 里的响应式覆盖报成「重复定义」（`.wb-grp ×2`），剥掉 at-block 后 workbench 从 2 降到 0、admin 从 30 降到 3。**判断工具的假阳性比报错危险，人会学会略过它。**

**四、其他**：`design` 插件（Anthropic 官方，含 design-critique / design-system / ux-copy）已渲染安装卡片，Lemon 说「现在安装」但复查 `ListPlugins` **未生效**，待重试；插件不是这几件事的前提，不装也照做。原型预览新增 `scripts/proto-server.mjs`（把 `/admin/assets` 与 `/assets` 反代到后台，让原型同源）与 `scripts/proto-bundle.mjs`（打自包含单文件，便于换设备看——Lemon 当天就因为换设备打不开 localhost）。

全量 972 / 904 pass / 1 fail，唯一那条复核确认仍是既有 F-65。

## D-294（2026-09-20）数字墙布局等真实数据再调；先清掉界面上写给自己看的话，并立文案闸门

Lemon 复看后改了判断：「这个数字墙，我觉得还可以吧。等后期数据进来了，我们再调，还是说现在要调好？」

**裁定：拆成两类，只做第一类。**

- **和数据无关的（现在做）**：界面上写着开发备忘和内部编号。这是明确的错，跟数据来不来没关系，改几个字符串就完。
- **和数据有关的（等）**：五格的轻重分布、哪些指标值得留、大数字还是小字。**现在三格是空的，怎么排都是猜**；真跑一天才知道哪些数字天天盯、哪些永远是 0。正好接上 D-291「先上生产、真用一天」。

**清掉的四处**（都是界面可见文本，不是注释）：数字墙三格副标题 `口径待定（D-284 ③）` / `按卡台，聚合待写` / `无主扣款金额，聚合待写` → 一律留空（「待接入」本身已说清楚）；卡与钱那句 `今日花费按台、highvcc 钱包水位：待接入` → 删。

**扫出来的另外两处**（本来不知道）：设置页显示 `D-221 要按产品（Plus 3 / 5X 1 / 20X 1），现在只有这一个全局值，暂不可改` → 改成「三个产品暂时共用这一个值，还不能分开设」；卡片页显示 `…能力保留，只是平时不用，收进这里（D-280 ⑧）` → 改成「highvcc 快照入库，也用来接入没有 API 的新卡台」。

**闸门**：`scripts/ui-copy-check.mjs`，已接进 `wrapup-check.sh` 第 11 项。扫 JS 与 HTML 里会显示给人的中文文本，命中内部编号（`D-\d+`/`F-\d+`）或开发备忘用语（待写/待定/口径未定/TODO…）即失败。

**这支检查器自己也修了一次假阳性**：第一版逐字符配对引号提取字符串字面量，模板串里的 `${escapeHtml('x')}` 有嵌套引号，配对当场错位，36 处报出来 34 处是注释里的编号。改成按行扫（先剥块注释、再剥行尾注释），降到 2 处，两处全是真的。**判断工具的假阳性比报错危险，人会学会略过它** —— 这是本窗口第三次栽在同一条上（前两次：CSS 棘轮把 `@media` 覆盖报成重复定义；比对脚本因原型引用实时样式而永远通过）。

**一条测试跟着改了，但没有弱化**：`每卡单数只读显示并写明 D-221 缺口` 原先断言界面上要出现 `D-221 要按产品`。测试的真实意图是「缺口要可见 + 只读」，两条都还守着，只是改断言人话版；**并新增一条 `assert.doesNotMatch(out, /\b[DF]-\d+\b/)`，把新规矩也钉进测试**。

全量 972 / 904 pass / 1 fail（既有 F-65）。

## D-295（2026-09-20）按产品统计库存前先修一个真 bug：最低卡余额没跟着产品走，20X 会把「可分配 0」显示成 4

Lemon 要求「先想清楚这个卡片到底应该展示什么」，照做时查规划、查数据，**在动手前撞出一个生产级错误**。

**查规划的结论（先说，因为它推翻了我自己前一轮的方案）**：D-283 原文「二排＝卡与钱（**按台按产品**）」，原型 C 的渲染是每台一行、行内按产品显示「用 N / 剩 N」，底下一行合计。而实现只做了 Plus，并把维度换成「可分配 / 在库 / 使用中 / 用过」四个状态。**这块难看的根因不是设计没想清楚，是没按规划做**；D-283 自己就把「按台按产品卡用量」列为三个待写聚合之一，实现时拿现成的单产品字段顶上了。据此**作废我前一轮出的四版排版方案**——它们都在错误的维度上换排版，治不了病。

**顺带纠正一处含糊表述**：「今日花费（按台）」是**数字墙第五格**（D-285），不属于卡与钱这块。卡与钱的标题「今天花了 / 卡用了几张」从原型 C 起就多说了一件事——原型 C 的 `cardsHtml()` 里根本没有花费，实现照抄了标题。标题改为「卡用了几张（按台按产品）」。

**bug 本体**：`providerCardStockSql()` 里的 `minimumSql` **写死取全局键** `default_minimum_required_card_balance`，传进来的 `productCode` 只影响资格规则、不影响余额门槛。而真实建单走 `minimumRequiredCardBalanceForPlan()`：先查 `minimum_required_card_balance:<product>`，缺了才回落全局。生产实值：plus `16.00`、**pro_20x `150.00`**、pro_5x `16.00`。

**代价（生产实测，不是推演）**：在库 9 张里余额 ≥16 的 4 张、≥150 的 **0 张**。改前按产品统计，20X 会显示「可分配 4」，真实是 **0** —— 把「一张都充不了」显示成「能接 4 单」，运营据此以为有货，客户一下单就失败。

改后生产实跑两个产品的权威 SQL（语句存 scratchpad，输出如下）：

| 卡台 | 在库 | plus 可分配 | pro_20x 可分配 |
|---|---|---|---|
| legacy-primary | 2 | 0 | 0 |
| backup-a | 7 | **2** | **0** |

**只调 plus 时这个 bug 永不暴露**，所以它躺在那里没被发现；一旦按产品统计立刻变成错数。这条和「判断工具不许抄业务规则」同源：**显示用的口径必须就是真实决策用的那一个**。

**回归测试**已加（`card-inventory-eligibility.test.js`），并做了变异测试：改回写死全局键 → 测试红；恢复 → 绿。全量 973 / 905 pass / 1 fail（既有 F-65）。

**Lemon 当轮另外两个裁定**：① 「还能接几单」口径＝**当前可分配卡数**（不掺预测）；② 「今日花费」口径＝**只算给客户充值消费掉的钱**（不含开卡成本）。后者数据齐备：`card_consumption_ledger` CONSUMED 19 行金额全非空，按卡台分组实跑 highvcc 14 笔 $224、hnskj 5 笔 $80，`todayCst8WindowSql()` 现成 —— 数字墙那格不必再空着。

**据实查提出、Lemon 已同意要加的两样**：
1. **「剩 N」旁边标「会不会自动补」**。策略表实查：plus 水位 2，而 **pro_20x 与 pro_5x 水位都是 0** —— 水位 0 意味着调度器永远不会为它们自动补卡，而生产 20X 已跑 9 单、用掉 3 张卡。「20X 剩 0」要人工开卡，「Plus 剩 0」系统会自己补，界面只显示「剩 0」看不出这个区别。**另记一条观察：20X 水位 0 是否有意为之，待 Lemon 确认**（本轮只报、未改）。
2. **显示「正在等卡的单」**（查询已跑通，当前生产 0 单）。「剩 0 且 3 单在等」与「剩 0 没人在等」紧急度完全不同；这块此前只讲库存不讲需求。

## D-296（2026-09-20）大额卡默认不给 Plus 用（>$75）；最低余额口径收敛成唯一一份

**Lemon 定**：「不要把那种大额的 100 美金和 150 美金的那种 5x 和 20x 的卡判定为 Plus 可以用的，那个是绝对不允许的」「比如说金额大于 75 美金，就默认不能给 Plus 充」。同时澄清 **20X/5X 水位 0 是正常的**（前期资金少，还没给它们单独做库存卡；等资金够了会在后台放几张随时充）—— D-295 里那条「水位 0 是否有意」的观察就此了结，不是缺口。

**为什么必须有这条（实查三件事叠起来）**：① `cards` 表**没有任何产品字段**，卡本身不记「我是为哪个产品开的」；② `card_stock_jobs` 记了 `product_code`，但开出的卡**不回写**归属；③ 唯一的产品限定是人工标 `card_operational_overrides.PRODUCT_ONLY`（生产现有：RETIRED 35 张、PRODUCT_ONLY claude 1 张，**5X/20X 一张都没有**）。所以往后台放一张 $150 的 20X 卡，只要忘了手动标，它就满足 Plus 全部条件（余额 ≥ $16）进 Plus 池：Plus 单花 $16，剩下的继续被 Plus 吃到每卡单数上限，**一百多美金卡死在那张卡上**。这条规则不依赖任何人记得打标记。

**两个边界我按最严的定，已向 Lemon 说明**：
- **判定金额取 `GREATEST(当前余额, 充值金额)`**。只看当前余额的话，$150 的卡用掉一半降到 $70 就又能给 Plus 充，等于没堵。
- **「默认」＝可被显式覆盖**：明确标了 `PRODUCT_ONLY=plus` 的卡仍然放行（运营知道自己在做什么）。
- **只约束 Plus**：大卡给 5X/20X 用本来就是它该去的地方；小卡给大产品会因余额门槛自然挡住，不必再加。
- **阈值可调**：`app_settings.plus_max_card_balance`，没有这个键时用 75。本轮不写迁移 —— 缺键时 `COALESCE` 回落 75，零发布风险，将来要改再加键。

**生产实测（只读）**：在库 9 张，判定金额最大 $50，**零误伤**；规则生效前后 Plus 可分配都是 **2 张**（backup-a 的 eb30、1eca）。

**顺带收掉一份重复实现**：`minimumBalanceSql(productCode)` 早就存在于 `card-source-selection-service.js:40`，而我在 D-295 修 `providerCardStockSql` 时**又写了一份一模一样的** —— 同一个口径两处实现，正是 D-273「别为已经实现的东西再造第二份」说的那类，而且我刚在 D-295 里引用过这条教训。已收敛到 `card-inventory-eligibility.js`（资格规则模块，更底层），`card-source-selection-service` 改为转出。

**测试**：3 条新断言 + 派生口径不被碰坏的断言（`stockCountingCardSql` 是「资格规则去掉新鲜度那一句」正则派生的，在末尾加条件不该碰坏它）。做了三个变异，全部被抓到：判定金额只看当前余额 → 红；守卫对所有产品生效 → 红；去掉显式覆盖那半 → 红。全量 975 / 907 pass / 1 fail（既有 F-65）。

**仍未做、已登记**：开卡任务按 `product_code` 自动回写卡的产品归属（堵自动补卡）、人工导入时金额命中 5X/20X 开卡金额则不许默认进 Plus 池（堵快照入库）。这条 75 规则是不依赖人的兜底，但源头回写仍值得做。

## D-297（2026-09-20）工作台三块落地：卡与钱按台按产品、待办单行紧凑、页头一行；今日花费口径改为含开卡成本

**Lemon 当轮三句裁定**：①「今日花费的金额，我需要加一下开卡成本」（推翻当天早些时候「只算给客户充值消费掉的钱」）；②同意顺序＝三块做对 → 卡片页/CDK 页 → 发布 → 真用一天 → 回头调数字墙布局 → ⑦；③「工作台最终的效果我还没有看到」。

**今日花费口径（含一处我自己排掉的坑）**：`card_transactions` 里有 `card_recharge`（往卡充钱，16 笔 $309）和 `card_issue_fee`（开卡手续费，2 笔 $1.25）。**`card_recharge` 不能算**——往卡充 $50、再用这张卡给客户充 $16，两个都算就是 $66，而当天实际只流出 $50，会重复计算。最终口径＝**当日 `card_consumption_ledger` CONSUMED 金额 ＋ 当日 `card_issue_fee`**，两者不重叠。按台聚合，数字墙那格显示两台合计、副标题列各台。

**卡与钱**：按 D-283 原规划「按台按产品」。后端三个产品各查一次 `providerCardStockSql`（资格规则本身按产品不同：余额门槛 plus 16 / pro_20x 150、大额卡守卫只对 plus 生效），带出 `plus_target_available` 判断**会不会自动补**——水位 0 表示这个产品没做库存卡、断了只能人工开，而「20X 剩 0」与「Plus 剩 0」严重程度完全不同。另加 `ordersWaitingForCard`：库存讲「有多少」，这个讲「有多少人在等」。

**一处自己发现并改掉的设计错误**：第一版把三个产品做成三行大字格子，整块 **381px**，比改之前的 291px 还高，与「精简」背道而驰 —— 六格里通常五格是 0（5X/20X 还没做库存卡），等于让最没信息的部分占最大的视觉重量。压成一行式后 **313px**，并让有货的格子用墨色+绿底、全 0 的退成弱色，一眼看出哪个产品真能接单。

**实测高度变化**：页头 58 → **34**；营业条 117 → **56**（D-293）；需要我处理 379 → **304**；卡与钱 291 → 313（信息量从「4 个状态标签×2 台」变成「3 产品×用/剩/补法×2 台 + 今日花费 + 等卡单数」）；**整页 1237 → 1169**。

**两条闸门当场生效**：CSS 棘轮抓出我新引入的字面色 +1 与重复选择器 +2（`.wb-qi .wb-qt b` 早已存在，我又写了一遍），已修；文案闸门全绿。标题里的「（按台按产品）」按「界面不写解释旁白」去掉，改成**「卡还够不够」**——直接回答运营的问题。

**测试**：`admin-read-service` fixture 补四个查询结果并加断言（三个产品都在、水位 0 标成不自动补、20X 已有卡在服务、今日花费按台、等卡单数）；`public-isolation` 里原先断言旧 chip 文案「Plus 可分配」，改为断言新表达并**additionally 钉住** `autoReplenished`／`byProduct`／`ordersWaitingForCard`。全量 975 / 907 pass / 1 fail（既有 F-65）。

## D-298（2026-09-20）Lemon 验收三处修正：今日花费只算充值成功的、「需人工开」弱化、侧栏底部重排

看过工作台成品后 Lemon 提三处，其余通过。

**一、今日花费口径第三次收窄**：「需要调整为**给客户实际充值成功**的金额加上开卡的费用」。账本记了 `CONSUMED` 不等于客户拿到了东西——扣了钱而充值没成功的单同样留下 CONSUMED 行。改为 `JOIN orders AND orders.status='RECHARGE_SUCCESS'`。**生产实查：当前 19 笔 CONSUMED 全部对应 RECHARGE_SUCCESS（$304），两种口径此刻恰好一致**——但口径要写对，否则第一次出现「扣了钱没充成功」时这个数就虚高。（三次演进：只算充值消费 → 加开卡成本 → 只算充值**成功**的 + 开卡成本。）

**二、「需人工开」弱化**：原先橙色加粗。Lemon：「不需要那么明显，我知道」。5X/20X 没做库存卡是既定安排（D-296 已确认水位 0 正常），不是异常，天天用警示色喊一件他早就清楚的事是噪音。改成与「自动补」同级的弱色，**保留区分、去掉强调**。

**三、侧栏底部重排**：原先三处别扭——状态点用 `grid-row:1/3` 垂直居中，视觉上和两行文字都不对齐；「安全退出」是常规大按钮，比登录状态本身还重；两块间距 14px 过松。改成：点与「管理员已登录」同一行对齐，「人工确认模式」换行缩进，退出退成文字按钮。

**过程中改坏一次并当场发现**：只把点从 `grid-row:1/3` 改成 `1`，`<small>` 就自动流到第二行第一列——「人工确认模式」跑去左列、标题被挤到右边。整块改 flex 才对。**这次是截图看了才发现的**，不是等 Lemon 指出——但也说明「改 CSS 不看效果」的老毛病还在，视觉比对脚本管不到没有定稿原型的块。
...
## D-300（2026-09-20）四条硬门槛：提方案前先交依据清单、打勾附证据、改完说看到什么、验收前置

Lemon 问：「我应该如何要求你，你才能把这个页面做得绝对不出问题？」这条记的是答案，以及为什么是这四条。

**一个窗口内的实际错误，有规律**：

| 类型 | 次数 | 实例 |
|---|---|---|
| 没查依据就提方案 | 3 | 卡与钱出四版排版（没查 D-283，**维度本来就选错**）；卡片页提「九块压三块」（没查 D-280）；提「待销并进列表」（**那正是 Lemon 否掉的 B 版**，A 版的特征恰恰是待销独立成块） |
| 把数据当业务结论 | 2 | 说「生产有 3 个卡台」（实际 `provider_accounts` 三行里 zzshu 的 `purpose='RECHARGE'`，是旧直充系统）；对照 D-280 时把 token 那条判成 ✅ |
| 判断工具自欺 | 3 | 比对脚本引用实时 CSS 永远通过（D-293）；CSS 棘轮把 `@media` 覆盖报成重复定义；文案检查器 36 报里 34 是假阳性 |
| 改了不看结果 | 2 | 侧栏底部改坏（D-298）；卡与钱第一版 381px 比改之前还高 90px |

**前两类同一个根：用「我看见的」代替「该看的」**——看到 9 个块就设计 9 个块的方案，看到 3 行就说 3 个卡台。后两类同一个根：**验证的是「我预期会变的东西」，不是实际结果**。

**四条门槛**（已写进 `AGENTS.md`「提方案前先交依据清单」一节与个人记忆）：
1. **提任何方案前先交依据清单，列不出不许提**：决策编号 + 原型文件名与挑定版本 + 生产查询语句和原始输出。**它的价值在于 Lemon 一眼能看出我漏了什么**，而不是我自己判断查没查够。
2. **打「✅ 完成」必须附证据行**（代码位置 / 查询输出）。当天实例：D-280 第 1 条要求「token 状态 + **贴 token 输入框**，失效整栏标红」，我判成 ✅；实际贴 token 表单埋在「highvcc 一键开卡」折叠块深处、不在卡台栏。要求每个勾给代码位置就当场露馅。
3. **改完说「我看到了什么」**，不是「改好了」。
4. **动手前先说验收怎么验**，做完按它逐条走。

**明确承认挡不住的部分**：这四条只挡已知失败模式。token 那条是读 `CURRENT_STATE` 读到「highvcc token 每 2 小时过期、过期后无任何告警、只能人贴」才意识到的——**在那之前不知道该去查它**。对「不知道自己不知道」，唯一有效的是 Lemon 反复追问「你就没有遗漏的吗」「你了解清楚了吗」；被这样问时不辩解、去补读。

**顺带记两条这轮补读出来的生产事实**（它们改变了卡片页的优先级）：
- **highvcc token 每 2 小时过期、过期不叫人、只能人贴**（`CURRENT_STATE`），而快照 timer 此刻就是 `failed`。贴 token 是这页最高频的运维动作之一，却是最难找到的入口。
- **`tokenFault` 取自 `supply_fault_state='FAULT'`**（开卡失败时写的供给故障），**不等于 token 过期**。所以可能出现：token 已死两小时、快照全部失败，而页面那格还写着「已配置」——**这页答不了「token 还活着吗」**。

## D-301（2026-09-20）补读四份文档挖出六个冲突，Lemon 逐条裁定；5X 最低余额已改正到生产

Lemon 让我「先把整个项目了解清楚」后再谈重构。补读 `PRODUCT_SIMPLIFICATION_DISCUSSION` 接班基线、V2 §4、face-1/3/4/5 账本、`ADMIN_CONSOLE_MAP`，挖出**六个基线与生产不一致**，逐条裁定如下。

| # | 冲突 | Lemon 裁定 |
|---|---|---|
| 1 | 20X 卡金额：基线「220 以上」vs 生产策略表 150 | **150 是对的**，基线那句过时 |
| 2 | HNSKJ 开卡账户余额：基线「250 以上」vs `wallet_floor` 30 | **30 是对的**，基线那句过时 |
| 3 | 5X 最低余额：应为 95 vs 生产 16 | **就是 95**，当日改正 |
| 4 | 确认弹窗：V2 §4.2「只保留开卡/打开付款/作废 CDK/关闭订单」vs D-280 第 5/6 条要求待销与手动用卡确认 | **确认词不用留** |
| 5 | 今日花费：face-4 看板六问要「开卡+付款+**拒付**」vs 当日口径没算拒付 | **拒付也算** |
| 6 | 我报「工作台缺 token」 | **我错了** —— 待办里本来就有 |

**第 6 条是我的错，但查产生点时挖出真缺口**：`PROVIDER_TOKEN_EXPIRED` 的唯一产生点在 `card-supply-scheduler-service.js`（补卡调度器）——**只有调度器去开卡、撞上 token 死了才会报**。而 highvcc 快照 timer 每小时跑、因 token 过期而 `failed`，**这条路径不叫人**；水位满时调度器不开卡，token 可以死一整天而待办上什么都没有。`CURRENT_STATE` 那句「过期后无任何 operator_alert」第⑤步只补上了「开卡时发现」一条路。**而 `localStorage.access_overtime` 能直接读到剩余有效期**（`sync-highvcc-token.mjs` 已在读），是更直接的信号，只是没接进后台。

**第 3 条已落生产**（当日 06:08:21 UTC）：新写脚本 `v1/scripts/set-minimum-card-balance.mjs`（正式连接池 + 事务 + `FOR UPDATE` 读旧值 + `admin_setting_events` 审计 + 默认预览、`--apply` 才写），先在隔离库试跑、再生产预览、最后 apply。**预览即报影响**：用分卡时真正执行的资格 SQL 算出「该产品可分配卡 2 张 → 0 张」。那 2 张本就是余额 $50 的 Plus 卡、5X 需要 $100 —— **「2 张」是假的，「0 张」才是真的**。5X 路线 `accepts_new_orders=0`、零在途单，不影响任何在跑的单。新连接独立复核：`minimum_required_card_balance:pro_5x = 95.00`（06:08:21.487），审计行 `16.00→95.00`。

**为什么要新写这个脚本**：这个值目前唯一的后台入口在卡片页块 2 里，而那块按 V2 §3.3 / D-284 ① 要撤；**而设置页 D-290 那张大表漏做了「最低余额」**（只做了水位/开卡金额/日限/钱包底线/告警线）。face-5 S2 明写它该在设置页那张表里。**由此得出一条删块 2 的前置条件：先把「最低余额」搬进设置页，否则删完就没入口了。**

**顺带记几条补读出来的事实**（都改变了卡片页的判断）：
- **D-287 否掉 B 版的理由是安全，不是美观**：原文「待销混进主列表，**删卡按钮与日常行同处易误点**」。我此前建议的「待销并进列表」不仅是被否版本，否它的理由至今成立 —— **彻底作废**。
- **token 主路径是收藏栏书签**「📌 刷新卡台登录（拖我）」，不是那个输入框；`ADMIN_CONSOLE_MAP` 明文「不要让 Lemon 去跑命令行」。
- **`POST /cards/sync` 对 highvcc 卡不生效**：那些卡的 `current_balance` 是**导入时的快照**、`last_transaction_synced_at` 为 NULL。「判断有没有扣款时不能只信这个数」。
- **`card-intake` 链路保留、页面降高级入口**（face-5 §6 Lemon 已答），所以「新卡接管记录」折叠块是对的。
- **face-5 §6 Lemon 早就说过**「后台 4 页是过去定的，**现在可以重新设计、增减改**」——与当日他说的一致。
...
## D-303（2026-09-20）删块 2 的前置清零：最低余额设置页早就有（我误判），每卡单数改为可编辑

准备执行任务书 §B1（删卡片页块 2）时，按新立的门槛先 grep 确认写路径，**结果推翻了我自己写进任务书的「硬前置」**。

**我的误判**：任务书 §B1 写「设置页 D-290 漏做了『最低余额』与『每卡单数』，必须先补再删块 2」。

**真相（实查）**：
- **最低余额设置页早就做了**——`renderSettingsThresholds` 里按产品三行（`data-plan` × `SETTINGS_PLAN_ORDER`）、带保存按钮，保存调 `/api/v1/admin/card-stock/minimum-balance`，**与块 2 里那份是同一个端点**。块 2 那份是**重复入口**，删掉即可。
- **我为什么误判**：只查了 `card-supply-policy-admin-service.js` 的 `POLICY_FIELDS`（它管 `card_supply_policies` 表与 `provider_accounts` 列），而最低余额走 `app_settings`、另一个端点。**在一个地方没找到就断言「不存在」**——这是本项目记忆里第七类惯犯，当天第二次（第一次是 `waiting_for_card`，见 D-302）。

**每卡单数确实是真前置，已处理**：设置页那项此前是只读，理由是 D-221 要按产品、且块 2 还有可编辑入口。块 2 一删就没入口了，故改为可编辑——**复用既有端点** `/card-stock/max-successful-payments`（早就存在，整数 1~4 校验），不新建端点、不新建 service。「三个产品共用这一个值，还不能分开设」的说明保留（那是事实，欠账 3）。

**测试加了两条**：① 每卡单数必须可编辑且带 `max="4"`；② **最低余额按产品三行必须存在**——这条专门钉住我误判过的地方，免得下次又有人（或我）断言「设置页没有最低余额」。
...
## D-306（2026-09-20）自动开卡**不做**总开关：停它走水位。F-65 就此结案，不再算缺口

**Lemon 问的**：「自动开卡的开关必须要做吗？直接不接单不就行了吗？」

**答一半对一半不对，两条都查了代码与生产**：

**① 不接单停不了自动开卡。** 补卡调度器（`card-supply-scheduler-service.js`）里 `accept_new_orders` 出现 **0 次**——它压根不读那个开关。它读的是 `loadState()` 里的 `card_auto_replenishment_enabled`，加上策略表的水位：

```js
const demand = Math.max(policy.targetAvailable, waitingCount);   // 水位 与 等卡单数 取大
deficit: demand - available - inflightCount
```

生产实查：两台 plus 水位都是 **2**，当前等卡单数 **0**。**一张单都没有时，只要可分配 < 2，它照样开卡。** 不接单只拦新单进来，与开不开卡是两条线。

**② 但水位就是那个开关，而且比总开关好。** 水位设 0 → `demand = max(0, 等卡数) = 等卡数` → 没人等卡就不开。而水位在设置页已经能改（按台 × 按产品）。生产 6 行策略里 5X/20X 那 4 行水位本来就是 0，真正要改的只有两台的 plus 两行。
...
**裁定：不做总开关。** 除了「按台 vs 全局」这条，还有一个理由：后端 `setSupplyAutomation` 一次写两个键（`card_auto_replenishment_enabled` + `card_balance_recharge_enabled`），而补余额已弃用（D-218）、生产刻意把两键设成不同值（true / false）——**给它做界面等于把这个 bug 接回来，调一次就抹平**。

**随之办掉的三件**：
1. **前端孤儿处理器删掉**。`data-supply-toggle` 全项目无渲染处，只剩一个永远匹配不到的 handler；留着会让下一个人以为「入口只是丢了，接回来就行」。后端端点保留但就地注明为什么没有界面、以及接它之前必须先拆成单键。
2. **设置页把水位的这层含义写出来**：「水位填 0 ＝ 这台这个产品不再自动开卡（还有客户在等卡时仍会为那几单开）」。不写运营不会知道可以这么用。
3. **F-65 那条测试随决定改写**。它挂着「有意留红」的名头很久，本轮查清：**红的是第一行写死的 `admin.js?v=61`，后面三条守门断言从 `d33544c` 起一次都没执行过**。把版本断言改成「带版本 + 只增不减」后，真断言第一次跑起来，立刻红在「供给三态文案必须存在」——而那个控件在工作台改乙-3 时就拿掉了。既然决定不做这个控件，那条断言随决定作废；剩下的改成钉住本决定：旧谎报文案不许回来、`data-supply-toggle` 与 `operations/supply-automation` 前端不许出现、设置页必须写着水位那句话。**三个变异全被抓**（接回处理器 / 删掉水位说明 / 旧文案回潮）。

**F-65 / F-67 后半就此结案**，从 `PROJECT_MAP` §4.1 的缺口清单里撤下，改记为「已裁定不做」。

**全量测试 1004 / 937 pass / 0 fail** —— 这是很久以来第一次全绿，**是因为那条红测试守的东西被裁定不做了，不是因为把它改绿**。

## D-307（2026-09-20）「可分配」拆成两个数：库存口径当主数，分配口径只在不一致时补一句

**Lemon 问的**：「HNSKJ 为什么可分配是 0 张卡，明明我卡台中有两张，一张 50 美金，另一张可能有 18 美金，为什么你没有同步？」

**卡没丢，同步也正常。是这个数用错了口径。**（他记的 18，生产实际是 $16.00）

**逐条查那两张卡，十几个条件只卡在一条**（生产只读实查）：

```
0577  AVAILABLE / active / ACCEPTED / source_present=1 / 有凭据 / 已用 0 次 / 无占用 / 无退款 / 无运营标记 / 余额 $16.00
6754  同上，余额 $50.00
唯一不过：last_transaction_synced_at >= 现在 - 15 分钟
        上次同步 10:03，下次 13:03，当时 10:56 → 差 53 分钟
```

**根因是两个周期对不上**：hnskj 的卡**每 3 小时**同步一次（`card_sync_jobs` 实证：10:03 / 07:03 / 04:02 / 01:02 全部 COMPLETED，没有失败），而资格规则要求「15 分钟内同步过」。**同一批好卡在每 3 小时里只有头 15 分钟算「可分配」，其余 91.7% 的时间是 0。**

**这个坑 Lemon 2026-09-18 就指出过一次**，`stockCountingCardSql` 的注释里原话都在：

> 分卡时那句「15 分钟内」是**分配前的证据要求，不是这张卡不存在**…窗口外按资格 SQL 数是 0 张，调度器会以为缺 2 张、两分钟开出 2 张（Lemon 2026-09-18 指出）

当时修了**补卡调度器**（改用库存口径数，所以它不会误判缺卡去开卡，这点一直是对的），**卡片页与工作台这一侧漏了**。而我上一轮还在 D-305 里写「与工作台同口径」—— 两处同口径没错，**用的都是错的那个**。

**改法：一个数说不了两件事，所以算两个。**

| | 含义 | 用在哪 |
|---|---|---|
| `stock_available` 库存口径 | 这张卡结构上还能不能服务新单（余额、次数上限、活动分配、资金/退款风险、RETIRED/PRODUCT_ONLY 全查，**不查同步时效**） | **页面主数**——「卡够不够」问的就是它 |
| `bindable_now` 分配口径 | 此刻能不能立即绑单，多一条 15 分钟时效 | 只在**小于**库存口径时，脚注补一句并说明会自行恢复 |

三个数依次收紧：`in_stock ≥ stock_available ≥ bindable_now`。生产实跑：

```
legacy-primary  在库 2  库存口径 2  可立即绑 0
backup-a        在库 7  库存口径 2  可立即绑 2      ← backup-a 是 MANUAL_IMPORT，豁免时效
```

**不造第二份**：库存口径早就存在（`stockCountingCardSql`，从资格规则派生、只替换新鲜度那一句，替换不到就抛错）。它原先在 `card-source-selection-service.js`，而 `providerCardStockSql` 在更底层的 `card-inventory-eligibility.js` —— 直接引会让底层反向依赖上层，所以**把它搬到底层、上层转出**，与 `minimumBalanceSql` 当初同一个处理。

**界面**：主数标题从「可分配」改成「**还能服务**」（「可分配」这个词正是歧义的来源）；两个数不一致时脚注补「此刻可立即绑 N 张，其余在等下一次同步（会自行恢复）」，**一致时不显示**——天天挂一句一样的话就是噪音。工作台合计同理。

**验收**：隔离库造出「卡好但在同步窗口外」（把一张 AUTO 卡的同步时间推到 60 分钟前）复现生产场景，真实浏览器实测主数 2、脚注「可立即绑 1 张」，highvcc 两数一致时不显示。**4 个代码变异 + 1 个契约变异全被抓**（主数换回分配口径 / 一致时也啰嗦 / 库存口径悄悄放宽 / 工作台换回分配口径 / 删掉脚注）。全量 **1006 / 939 pass / 0 fail**，三条闸门全绿。

**顺带对齐了演示库的造数**（本轮被自己的假数据误导两次）：`minimum_required_card_balance:pro_5x` 16→95 对齐生产；`chargeback` 的负数改回正数。

**只报不改的两条**（生产实查）：
- **今日花费可能算出负数**。演示库里 `chargeback = -20.00` 时，「今天花了 -18.80 USD」直接显示在工作台上。聚合是 `SUM(t.amount)`，**对符号没有任何约束**。生产 chargeback 四笔全为正（$462.25），所以没咬过人。**卡台会不会给出负的 chargeback（拒付撤销/冲正），我没有真实响应可查，不编。**
- **两张卡的账对不上**：`5501 开卡 $2 / 购买共 $159.16`、`7402 开卡 $49 / 购买共 $158.62`，且都没有任何补余额记录。干扰项：这些流水的 `first_seen_at` 全是 `2026-09-17 15:52:25` 几乎同一秒，像批量补录；真实交易时间在 `trade_time_raw` 里没解析出来（`occurred_at` 全表 NULL，既有 bug）。**原因未知，Lemon 定了先搁置。**


## D-308（2026-09-20）「可分配」这个词留着；统一全后台的状态术语

**Lemon 问的**：「『可分配』为什么改成『还能服务』？我觉得没问题啊。」

**他对，我改错了。** D-307 修数的时候我顺手把台账栏标题改成「还能服务」，理由写的是「『可分配』是歧义来源」——**这个理由不成立**。歧义从来不在词上，在那个数取错了口径；数修对了，词本来就对。

**而且我的改动制造了新的不一致**（实查，改动后全后台仍有四处叫「可分配」）：

```
index.html:179   就在那个格子正上方："可分配"指现在就能安全绑定 Plus 订单的卡
index.html:358   设置页：水位＝这台这个产品至少保几张可分配的卡   ← 水位就是那格的分母
admin.js:69      STOCK_CATEGORY_LABELS  READY: '可分配'
admin.js:58      INVENTORY_LABELS       AVAILABLE: '可分配'
```

改了一个标题，留下四处旧词，其中两处紧贴着它。**这不是消除歧义，是把术语搞散。已改回「可分配」。**

**顺着这条线查下去，发现运营状态在页面上本来就有多个叫法**（不是我这次引入的）：

| 状态 | 卡片列表徽标 | 详情抽屉 | 台账栏/说明 |
|---|---|---|---|
| READY | **待分配** | 可分配 | 可分配 |
| RETIRED | **停用** | 永久停用 | — |
| PRODUCT_ONLY | **限其他产品** | 限定产品 | — |

运营天天看的恰恰是徽标那一列。同一个状态两个叫法，逼人自己猜它们是不是一回事。**全部统一到详情抽屉那份用词**（`可分配 / 永久停用 / 限定产品`）。`RETIRED` 特别说明：「永久」两个字不能省——这个动作不可逆，省一个字省不出什么，却让人以为还能恢复。

**顺带删掉一个孤儿常量**：`STOCK_CATEGORY_LABELS` 全文件只有定义、零引用。

**这里我犯了本轮第二次同型错**：判断它「零引用」时只 grep 了常量名，没查内容——而 `public-isolation.test.js` 有一条断言按**内容**撞它（`/可分配.*使用中.*暂不可用.*永久停用/s`）。删完全量才红。**「说不存在之前查三种形态」这条，我自己又只查了一种。**

那条断言的意图是对的（状态要用人话不用英文枚举），但它撞的是个孤儿常量、而不是运营真正看到的那张表。改成直接钉 `CARD_STATE_CHIPS`：五个状态全中文、不许出现英文枚举、且与 `INVENTORY_LABELS` 同名状态必须同词。**三个变异全被抓**（徽标改回「待分配」/ 孤儿常量回潮 / 台账栏标题又被改词）。

**顺带把那句定义改准并压短**（它原文写的是分配口径，而主数已改成库存口径）：

```
改前  “可分配”指现在就能安全绑定 Plus 订单的卡，按第③④块那份资格规则算——和工作台同一个数。“在库”只是没退役，两者差得远，别混用。
改后  “可分配”＝还能服务新单的卡：余额够、次数没用满、没被占用或标记，和工作台同一个数。“在库”只是没退役，比它宽。
```

全量 **1007 / 940 pass / 0 fail**，三条闸门全绿。真实浏览器实测：台账栏「可分配 / 水位（Plus）」、在役徽标「可分配 / 暂不可用 / 余额已用尽」、历史徽标「永久停用」。

**这条留给以后的教训**：改词之前先数一遍这个词在页面上出现几次。只改其中一处，比一处不改更糟。

## D-309（2026-09-20）「取消续费未确认」移出待销清单，搬进工作台队列；高级区按业务合并、不为美观凑数

**Lemon 定的四条**（对齐清单的回复）：① 「余额已用尽」文字与待销规则里的 DEPLETED **都不动**；② 待销只去掉「取消续费未确认」，其余五条理由留着（同意我的判断）；③ 卡台管理表删掉、状态并进台账栏；④ 配置类不挪设置页，**「不要为了美观而强行合并，如果合并能更稳定和提效，那才可以」**。

### 移出待销：我最初的理由是错的，结论对

我说「这跟卡该不该销没关系」——**错**。查到 D-248 打架 4 的原话：

> 取消续费没确认的卡是「**客户可能续费扣我们钱**」的卡，它可能没用满。

关切完全成立，**错的是落点**：待销清单的动作是「去卡台删掉这张卡」，而这件事要做的是「去订单里把续费关掉」。

**停下来核实了两件，才敢改**：
- `eligibleInventoryCardSql` 里 `cancellation` / `subscription` 出现 **0 次**
- 待销清单是「派生查询不建表」，不写任何状态

→ **进这个清单从来没有阻止过这张卡被继续分配**，它的效果只有「提醒」。所以移走不会放开任何闸门，D-248 的关切原样保留、只换位置。

**新落点**：工作台队列 `待复核续费 N 单`，文案「已扣款且充值成功，但续费没确认关掉——**不关下个周期会再扣一次**」（正是 D-248 的关切），按钮跳订单页的「需要处理」筛选。

**顺带修掉一个「算了没人用」**：`metrics.cancellationReview` 后端一直在算（生产实查 **7 单**），前端**一个地方都没消费**——和 F-71/F-72 同型。

**顺带删掉**待销取数 SQL 里那个每行都跑一次的 `cancellation_unconfirmed` EXISTS 子查询。

### 高级区：按你给的原则逐件判，结论是 4 件不是 3 件

**我先前说的「压到 3 件」是凑的。** 按「只为稳定和提效才合并」重判，生产使用频率为据：

| 件 | 生产实据 | 判断 |
|---|---|---|
| 两台开卡 | 手动开卡 3 次（hnskj）、highvcc 自动开卡 2 次，都在用 | **不合并**。两个表单结构完全不同（hnskj 有卡段/数量/快捷选择/费用预估/风险确认；highvcc 有 token 管理/卡段拒付统计/费用查询），合成一个块内切换要引入状态管理，**降稳定**；而台账栏每台已有「开卡…」直接跳，入口问题已解决 |
| 新增备用卡台 + 导入表格 | 卡台账户 2 个、最近建于 09-05；导入卡 16 张 | **合并**。它们是**一条链**：接入没有 API 的新卡台 = 先建卡台、再导入表格。现在分两块，做这件事要跳两个地方。两个低频表单、无状态耦合 |
| 补卡执行记录 + 新卡接管记录 | 自动开卡 970 次、接管批次 519 次 | **合并**。排查「卡怎么没进来」时本来就要连着看「开了吗 / 接管了吗」。两个只读列表、无耦合 |
| 卡台管理表 | — | **删**（③已定），熔断/运营停用状态并进台账栏 |

**一处差点误判**：查 `backup-a` 时看到「非 MANUAL_IMPORT 的卡 = 0」，差点写成「highvcc 一键开卡一张都没开出来过」。再查一层才发现**开出过 2 张**，只是入库时 sync_tier 也标 MANUAL_IMPORT（highvcc 没有 API 同步能力，统一按快照处理）。**又一次差点给观察补原因。**

### 本轮变异测试抓到我自己一条假断言

「这条理由不许回来」那条测试，第一版 fixture **少了 `cancellation_unconfirmed` 字段** → `Number(undefined) === 1` 恒 false → 把那条理由加回代码里测试照样全绿。**断言在空转。** 变异测试当场抓到，补上字段后才真正生效。不做变异测试就带着一条假断言走了。

**另一个自己踩的坑**：新待办第一版读 `operationalBacklog.cancellationReview`，页面上那条就是不出现、也不报错——那个字段实际在 **`metrics`** 里。**按代码位置猜字段归属，又犯了一次**；已对着真实响应改正，并加断言钉住「放错位置时不许显示」。

全量 **1010 / 943 pass / 0 fail**，三条闸门全绿。真实浏览器实测：待办出现、点「去处理」落到订单页「需要处理的订单」（筛选器 `REVIEW_REQUIRED`）。演示库造的数已还原。

## D-310（2026-09-20）卡片页第四条落地：按钮精简、高级区 4 件、卡台管理表并入台账栏、删掉两键端点

**Lemon 的原则**：「不要为了美观而强行合并，如果合并能更稳定和提效，那是可以的。」照此逐件判，生产使用频率为据。

### 做了什么

| 改动 | 依据 |
|---|---|
| 卡片列表标题栏三个按钮清空 | 「刷新本地列表」调的就是 `loadStock`，和切进这一页完全同效；另两个是自动链路的手动兜底（余额交易**每 15 秒**自动同步、新卡**每 5 分钟**自动发现），降进高级区的「执行记录」，并在那里写明「自动本来就在跑，这两个按钮只是不想等时催一次」 |
| 高级区 6 件 → **4 件** | 两台开卡**不合并**（表单结构差异大，合成块内切换要引状态管理、降稳定；台账栏每台已有「开卡…」直达）；新增卡台 + 导入**合并**（一条链：先建台、再传表）；两个执行记录**合并**（排查「卡怎么没进来」要连着看） |
| 「卡台管理」表删除 | 四列里能力是静态配置、Browser 操作只是一句指路；有用的「告知状态」与「快照时间」并进台账栏（`providerHealthIssue`，异常时在卡台名旁挂徽标、`is-bad` 时整栏标红） |
| 两个「刷新这台」分开 | 它们做的**根本不是一回事**：hnskj 刷的是卡段规则（开卡块里已有同端点按钮，重复且名不副实），highvcc 才是拉快照/钱包/流水。hnskj 那个删掉（它的卡每 15 秒自动同步），highvcc 改名「同步这台」并加 title 说明刷哪三样 |
| 错误码翻人话 | 实测 hnskj 刷新失败时界面原样吐 `刷新失败：not_found`。新增 `friendlyApiError`：认识的翻译，不认识的**原样带出**并标「系统内部错误码」——不吞、不含糊 |
| 删 `/operations/supply-automation` | D-306 已裁定不做这个控件；它一次写两个键会抹平生产配置。**能力没丢**：停自动开卡有设置页水位归零（按台按产品）和 `set-supply-scheduler-flag.mjs off --apply`（只写单键、带预览与审计）两条更对的路 |

### 我在这一轮犯的错，都是自己写过的规矩

**一、又按字符串距离删代码（D-289 同型）。** 删 `setSupplyAutomation` 时用「切到 `return {` 为止」，**把夹在中间的 `closeAlert` 一起删了**——那是工作台「关闭告警」在用的函数（生产 123 条 OPEN 告警）。`node --check` 过，5 条测试红了才发现。重做时改成**按函数的大括号配对**切，并加断言「删掉的片段里不许出现 closeAlert / setBrowserPaymentWrites」。删 1887 字符 → 1370 字符，多吃的 517 字符就是那个函数。

**二、差点把自己的测试手法报成 bug。** 改完看到控制台 385 条 409，比之前的 27 条暴涨。查下来是我在同一个 tab 里反复 navigate + `dispatchEvent(new Event('toggle'))` 累积的——开新 tab 干净测一次，完整走一遍只有 **7 个请求、每端点各 1 次**，没有循环。

**三、演示库造数第三次误导我**（前两次：5X 门槛 16 vs 生产 95、chargeback 符号）。这次是 `backup-a.last_full_snapshot_at` 为空 → 台账栏报橙标「未导入完整快照」，而生产有值（2026-09-18 02:52）不会报。已对齐。**演示库偏离生产时，它既会假装成 bug，也会掩盖 bug。**

**四、变异测试抓到契约的两处盲区。** 「标题栏塞回按钮」和「hnskj 也给同步按钮」两个变异第一轮都没被抓。补了「台账栏按钮」探针（按个数，抓到了 3 vs 4）；而「标题栏多一个按钮」**几何比对抓不到**——那一行是 flex、按钮没有 h2 高，塞回去整行高度不变。这条改由结构测试守，并在契约 note 里标明这个盲区，**不让人以为它守住了**。

**五、一条测试一直在给死代码发绿灯。** `Diagnostics manual refresh actually awaits all five loaders` 跑的是 `#refresh-button` 的 handler，而那个按钮从 `7c1a5c0`（工作台重做）起就不在页面上了（HTML 0 处、JS 动态创建 0 处、git 历史确认）。**测试绿 ≠ 功能可达。** 意图（进诊断页要 await 全部五个 loader）改挂到真正活着的 `switchView` 分支上。

### 验收

全量 **1010 / 943 pass / 0 fail**，三条闸门全绿，契约 20 条探针。真实浏览器 1440px 实测：4 块结构、卡表标题栏 0 个按钮、台账栏 `hnskj=开卡…` / `highvcc=开卡…+同步这台`、高级 4 件标题正确。

## D-311（2026-09-20）「标记这张卡不可用」：入口换语义、理由做成选项、理由决定下一步

**Lemon 问的**：「如果某个卡片被上游禁用或卡本身有问题，如何标记不可用？标记后是否不会匹配到此卡？」

**答**：机制早就通了——`card_operational_overrides` 的 `RETIRED` 在资格规则里被排除（`NOT EXISTS ... allocation_policy = 'RETIRED'`），标了就不会再被分配。**缺的是一个语义正确的入口**：唯一接近的是「我手动用了」，而那只是三种情况里的一种。

### 生产数据正好分成三类，只是全塞在自由文本里

```
highvcc-manual-used   Lemon 手动用这张卡给客户充了 20X（余额 145 → 2.46）
highvcc-cancelled     Lemon 在 highvcc 上取消了这张卡，余额退回钱包
hnskj-voided          卡台把卡作废了
```

这三个前缀是**脚本**写进 `reason` 的，界面写不出来，事后只能靠读字符串区分。

### 改法：不动数据结构，只改入口

**先查了 F-57 的原意**，确认现有行为是对的、不能推翻：

> F-57 / P1：手动用卡登记复用已销卡确认，写 RETIRED 终态和完成事件，尚待销的卡退出 due。

它要的就是「**未真正销掉的卡留在待销清单里**」。所以停用后进待销清单是设计，不是 bug——**不需要新端点，不需要迁移**。

1. **按钮**「我手动用了」→「**停用这张卡**」（原来那个词只覆盖三分之一的情况）。
2. **理由做成选项**：手动充值用掉了 / 卡台已禁用作废 / 卡有问题 / 已在卡台取消 / 其他。说明仍必填。
3. **原因码写进 `reason` 最前面**（`PROVIDER_VOIDED: 说明`），待销清单据它显示**接下来该去卡台做什么**——这不是装饰：

   | 原因 | 下一步 |
   |---|---|
   | 卡台已禁用/作废、已在卡台取消 | 卡台那边已经没了，直接登记即可 |
   | 手动充值用掉了、卡有问题 | 去卡台把它删掉 |

   此前待销清单对这些卡一律显示「运营已标永久停用」，运营看不出该做什么。
4. **认不出原因码就不猜**：本轮之前写的 override（生产那 15 条）没有前缀，`stopCauseOf` 返回 null，按原来那句显示，**不编一个下一步出来**。

### 验收

隔离库真实点一遍：停用 5590 选「卡台把它禁用或作废了」→ 提示「已停用（卡台已禁用/作废）。卡台那边已经没了，直接登记即可。」→ 待销清单显示同一句 → 历史里撤销 → 卡回到在役、override 行清空。**新连接独立复核**：审计 `CARD_OVERRIDE_CLEARED` 的 `previous_json` 里记着 `"reason":"PROVIDER_VOIDED: 卡台后台显示已作废"`，原因码进了审计。

**4 个变异全被抓**：reason 退回自由文本 / 认不出原因码时瞎猜一个 / 理由改回单个文本框 / 待销清单不显示下一步。

全量 **1013 / 946 pass / 0 fail**，三条闸门全绿。

## D-312（2026-09-20）分配口径那句说明从界面上去掉

**Lemon 把这句话贴回来，只说「这句话去掉」**：

> 此刻可立即绑 0 张，其余在等下一次同步（会自行恢复）

**他对。** 这是 D-307 我加的，理由写的是「不提等于隐瞒」。重看之后，它至少有四个毛病：

1. **「绑」是内部词**（绑定订单），运营不一定知道绑什么。
2. **「其余」是错的**——hnskj 当时一张都不能绑，不存在「其余」。
3. **「会自行恢复」听起来像出了故障**，而它根本不是故障。
4. **最根本的：它在说一件运营做不了、也不用做的事。** 客户下单分不到卡时，`assignCard` 会把订单置 `WAITING_FOR_CARD` 并排一次按需同步，60 秒重试分卡——这个数自己就回来了，对客户没有影响。摆在界面上只会让人以为卡出了事。

「不提等于隐瞒」这条在这里不成立：**隐瞒的前提是那件事对看的人有用**。一个会自己恢复、且不影响下单的瞬时值，不显示不叫隐瞒，叫不制造噪音。

**改法**：台账栏脚注和工作台合计那两处说明删掉，连同只为它们存在的 `.cardrig-note` 与 `.workbench .wb-total .wb-sub` 两条样式。**主数仍是库存口径**（D-307 的核心修复原样保留）。

**`bindableNow` 字段保留但不上界面**，并在 `providerCardStockSql` 的注释里写明**这是有意的**——留着有两个用处：诊断「这张卡为什么分不出去」，以及让 `card-inventory-eligibility.test.js` 能钉住「两个口径除了新鲜度那一句必须完全相同」。**它没有 UI 消费方与 F-71/F-72 那种「标记落了没人用」不是一回事**，不写清楚下一个人会把它当漏来修。

测试跟着改成钉住「分配口径一个字都不上界面」（断言 `此刻可立即绑|会自行恢复|等下一次同步` 都不许出现）。

全量 **1013 / 946 pass / 0 fail**，三条闸门全绿。

**留下的教训**：界面上每多一句话，先问**它对着的人能不能据此做点什么**。做不了的，写得再准确也是噪音。

## D-313（2026-09-20）CDK 页重做：A 版「一条流水线」；批发做进来；六条口径与交互裁定（Lemon 定）

**起因**：Lemon 判 CDK 页「完全不合格，没有任何整体设计」（与卡片页同因）。动手前做四件核对（对数／对人／对齐演示库／报口径），四件都查出东西，其中一条推翻了执行者自己的前一轮判断。

### 一、比稿与选型

三版比稿 `docs/design/prototypes/step6-cdk-compare.html`（A 一条流水线 / B 两条动线 / C 生命周期段），**Lemon 挑定 A**。否 B 的理由是它「两块容易长出两个口径 —— 正是这页现在的病」。交互稿 `step6-cdk-a-interaction.html`（可点），Lemon 逐条确认。

**此前 CDK 页没有任何原型**（三形态查过：按文件名无、按内容只有客户兑换页、git 历史无）。上一轮的 CDK 前端是直接写的——这是「没有整体设计」的直接来源。

### 二、Lemon 当面定的六条

1. **批发做进来**（按批发出 / 按渠道记）。此前执行者建议「批次这层已经空了，腾出地盘」，**被 Lemon 第 2 条信息推翻**（见下）。
2. **渠道自由填**，不建渠道表。代价是按渠道汇总依赖拼写一致，接受。
3. **批发金额记批次**，不按张均摊。「批发谈的就是总价，均摊是编出来的数」。零售金额仍走 `customer_payments`（按 cdk_id 一码一行）。
4. **金额选填、不强制**，只在有需要时记。
5. **异常那格剔除 2026-09-07 之前的历史**（退回机制上线时点）。剔除后是 1，不剔是 17。
6. **「绑定订单」列换成「客户邮箱」**，Lemon 两次强调「我需要看到客户邮箱」→ **完整邮箱、不脱敏**，与 D-286 裁定明文码直接显示同一条理由（后台只他一人用）。邮箱可点进订单详情，订单号不单独占一列。

### 三、执行者三层判断塌了两层——记下来，因为塌的方式有共性

执行者据生产数据提了三条判断，Lemon 一条信息掀掉两条：

> 「目前 cdk 都是我自己在用，至今没有给客户发过，之前给客户充值都是我自己用 cdk 直接充值。本次升级完整后就会对外部客户开始开放，用新的 cdk。」

- **「批次这层已经空了」——作废**。「66 批里 65 批只有 1 张码」不是业务淘汰批发的证据，是**自用期来一单生成一个的痕迹**。批发不是死的，是还没开始。
- **「钱那一半是空壳」——部分作废**。`customer_payments` 73 行全 `EXTERNAL_UNSPECIFIED/PAID`、`amount` 全空，不是设计缺陷的证据，是自用走流程的痕迹。
- **「状态机漏一格」——站得住，但性质变了**。那 17 条卡在 REDEEMED 的现在没有客户在等；产生它们的路径对外后才会产生真挂账。

**根子上的错**：拿生产数据反推业务形态，而这批数据全是自用痕迹，对「运营会怎么用」零证据力。**这一页的设计输入只能来自 Lemon，不能来自库里那 75 行。**

### 四、「兑换＝开通吗」——Lemon 的直觉对，执行者的差额解释错

Lemon 质疑执行者把「码被兑」与「开通成功」分成两个并排指标：「他兑换成功就等于开通了啊，你想表达什么？」

查完时间线，**他对**：

- `REDEEMED` 确实写在**接单**那一刻（`order-intake-repository.js:218`），理论上 ⊋ 开通成功
- 但退回机制（commit `f8c1d30`，release `20260907-cdk-rules-44b00cd`）会把没付款证据的失败单的码退回
- **实证：09-07 15:00 之后终结的 FAILED/CLOSED 单，码还卡在 REDEEMED 的只有 1 条**，就是第④步那张故意留的演练作死码。同期 RETURNED 40 次
- 那 17 条里 15 条终结于机制上线前（13 条挤在 09-05 23:10 同一秒），1 条比发布早 81 分钟

**所以健康状态下两者就该几乎相等**，差额不是业务指标而是**异常信号**。执行者上一轮说「挂账被藏进已交付」是错的——那是历史沉淀，不是当前系统的产出。

**但「已交付 37」这个数仍要改**：`summarizeCdkLiability` 的 `delivered = SUM(status='REDEEMED')` 算的是「码被消费掉多少张」，而同页行标签 `cdkStatusLabel` 把「已交付」定义成「REDEEMED 且订单成功」＝20。同一个词两个数。**统一到「已成功交付＝20」**。
...
**批次号不出现在界面上**（答 Lemon「我需要都记住批次号吗」）：批次以「去向」的面目出现，点去向＝筛出整批；批次筛选下拉只列**多于 1 张**的批次并用人话（生产 66 批里 65 批单张，全列是噪音）；零售单张码去向列不可点。

**登记去向在生成那一步**（答「什么时候操作，要足够高效」）：数量 >1 自动滑出「发给／这批收／有效期」三个字段，三项都可留空（留空＝囤货）；生成后自动筛出这批 + 顶部给下载条。补登记走批量条，是次路径。两条路径写同一份底数（码级 `issued_at`），**不产生第二个分母**。

### 七、四个必修 bug（本轮之前一直在生产代码里）

1. **搜索框搜不到当前页之外的码**：`listCdkCodes` 先 LIMIT 再在 JS 里过滤 `q`。生产 75 张码、页大小 50 → **最早 25 张搜不到**。对照：顶栏全局搜索走 hash 精确查找，任何码都能搜到。一个后台两个搜索框，一个对一个错。
2. **「最近动静」对已作废的码显示创建时间**：取值链 `redeemedAt || issuedAt || createdAt` 不含 `revokedAt`。生产 17 张 REVOKED 相差 4～9 天。
3. **批次列表每 10 秒在 50 和 66 之间抖**：`setInterval` 刷的是 `loadCdkBatches` 而它带 cursor 累加，`switchView` 又只 reset 了单码分页。本轮批次整块删掉，该 bug 随之消失；但自动刷新该改刷主列表——现在主列表根本不在刷新链里。
4. **`expires_at` 只有读没有写**：三形态查完，全项目没有一条写它的语句，`expires_at` 永远 NULL、「已过期」永远 0。D-286 ② 只实现了一半（真正解决「20X 码变哑弹」的是按路线现算的 `redeemableNow`）。本轮补写入口。

**这一页此前零测试覆盖**（三形态查过：按端点名／按函数名／按页面 id，`v1/test` 与 `scripts` 全部无命中）。上一轮「端到端验通」是一次性手工验证，没留回归。

### 八、迁移 056（方向已同意，应用生产前单独确认）

`cdk_batches` 加渠道 / 金额 / 币种 / 发出时间四列，只加列、不动存量。**不往 `customer_payments` 加 `batch_no`**。

### 九、执行者自己翻车一次，记在这里

第一版探针用 `mysql.createPool(DATABASE_URL)`，漏了 `timezone:'Z'`（生产是 `v1/src/db/pool.js:23`），据此得出「批次翻页第 2 页恒空」的**假 bug**并写进了汇报。按同配置重跑后第 2 页正常。**自己写的探针必须与生产同连接池配置**——否则它测的是另一个系统。

### 十、Lemon 另外定的两件

- **那 21 张自用期老码作废**：已同意，时点定在**本页上线后用页面批量做**（筛「在手里」→ 全选 → 作废），顺带验收批量作废按钮。不另写脚本。
...
## D-314（2026-09-21，UTC+8）CDK 不做撤销发出回库，不做囤货拆批多渠道销售

用户对接班评估明确回应：①「为什么要撤销？直接作废不就行了嘛」；②不会先生成一大批库存，再分多次、按不同价格卖给不同渠道。

- 业务不需要「撤销发出后重新当库存出售」；不要为此扩展退货回库系统。未兑换码不再交付时走作废，A 版里的「撤销发出」入口不纳入实现。作废仍仅限 AVAILABLE；已绑定在途订单与付款未知不能借此绕过原有收口规则。作废不等于执行退款。
- 不做同一生成批次拆给多渠道、多价格销售。因此接班报告 C-02 不作为本轮阻塞，不新增销售批次/拆批模型；保留 D-313 的批次渠道、总额、币种、发出时间方案。
- C-01 的业务处理方向已定，但旧撤销路径还在代码中，不能标成已修复。此决定不删除无付款失败自动退码：后者仍归原客户的重试权益，并非撤销销售回库存。
- 本轮只更新决定与交接，未改业务实现、未连接生产、未实际作废任何码。有效期、统计语义等其他问题未因此裁定。

## D-315（2026-09-21，UTC+8）CDK 有效期从生成起默认30天，可自设、可延期；简单稳定优先

用户原话：「默认30天，也可以我自己设定有效期，允许延期，从生成时间算起即可」「尽可能别把我系统搞的太复杂和臃肿，稳定性优先」。实际客户通常很快或一周内兑换，不需要围绕少见边界建设复杂售后系统。

- 新生成 CDK 默认到期时间＝生成时间＋30天；运营可自行设定有效期，并允许之后延期。
- 不采用执行者此前建议的「从发出起算」「默认不过期」「首次受理后永久保留重试资格」等扩展规则；不添加自动延期、渠道下游计时或责任归因流程。
- 沿用现有无付款失败退码与付款未知锁定机制；过期后的特殊处理使用人工延期入口，不为此新增权益状态机。
- 默认30天针对新生成码，不据此批量修改历史码。零售单张与批量生成适用同一有效期规则，不能只有数量大于1才能自设期限。
- 覆盖 D-313 任务书中「有效期留空＝不过期」的旧默认。A 版布局仍为基准，必要入口遵循本条，不扩展新页面或业务模型。
- 本轮仅落盘需求，未改代码/数据库，未完成实现验收。后续最低验收：默认30天、自定义期限、延期后可正常验证/提交、到期拒绝新建单、原有付款未知锁定不受影响。

## D-316（2026-09-21，UTC+8）正常发码不强制去向；少量应急备码单独标记

用户明确拒绝每次生成必填去向：客户着急时可能忘填。大多数场景为客户需要时现生成；少量库存可备注，主要用于网络异常、没带电脑等无法临时生成的应急发码。

- 正常生成不强制填写客户/渠道/备注，不因缺备注阻止生成或复制；不能把「备注为空」推断成「尚未出售的库存」。不要求每笔正常发码另做一次登记。
- 应急备码在生成时明确标记为备码，可加备注，与正常客户用码区分；继续从生成起默认30天，允许自设/延期（D-315）。不增加必填去向或渠道销售模型。
- 应急备码可能离线从预存清单发出，后台无法当场感知；「未兑换」不证明「没卖/没发」。不能将后台剩余未兑备码承诺为实时可售库存，也不能将复制/下载当成客户实际收码凭证。
- 不为应急备码引入 D-314 已排除的复杂拆批销售/分价结算模型。备码逐张应急发出与批发拆批结算不同。
- 统计名称和离线后最小补登记交互仍按此边界收敛；未实施代码、未修改历史码。此前执行者提出的“每次生成必填给谁”明确不采用。

### D-316 补充确认（2026-09-21，UTC+8）

用户回复「可以」，确认上一轮完整流程：平时选产品/数量直接生成复制、备注选填；应急备码生成时勾选、可备注并预存手机；离线发出时在预存清单标记已发避免重复，方便操作后台后补标记。不要求离线时打开后台，不增加正常发码步骤。备码列表显示兑换事实，不把未兑换自动等同未售。这里只确认流程，未实施代码。

## D-317（2026-09-21，UTC+8）卡网投放选择不过期，不对接销售订单

用户补充：码上传卡网后由卡网自动售卖发货，卡网不显示有效期；用户确认「卡网这批码不过期」方案。

- 生成有效期支持默认30天、自定义、不过期；普通生成仍从生成起默认30天（D-315），卡网投放时主动选择不过期，可备注卡网名称。不是所有新码自动不过期。
- 生成/导出后上传卡网，由卡网记录销售和发货；本系统负责验码、充值及结果。不增加卡网销售接口对接，不要求每售一张回后台登记。
- 已上传卡网的码不再作为应急库存，也不重复投放到其他地方。未兑换可能是未售，也可能已售未兑；本系统不推断销售状态。
- 不过期意味着已售未兑换权益不能仅因时间久就作废；未售码撤回需先在卡网停止销售并核实，不新增自动撤回或退款系统。
- 此条补充 D-315 的有效期选项，不批量修改已有卡密，不取消 D-316 的应急备码事后补登记。顶部统计名称/口径仍待最终确认；本轮仅文档落盘，未实施、未连接生产。

## D-318（2026-09-21，UTC+8）四格定稿，开始 CDK 实现

用户「同意」确认四格并授权按已讨论范围实现：待兑换／应急备码／已成功交付／要看一眼。待兑换＝普通及卡网投放、或已登记发出的备码，尚未使用且未过期；应急备码＝明确标记备码、未登记发出、未使用且未过期；成功交付以订单成功为准；异常以失败/关闭未退码为准、继续排除 D-313 的历史范围。处理中/过期/作废在列表筛选，不增数字格。待兑换不等于已售，应急备码以后台登记为准。

批量操作保护：明确选中目标、作废前确认数量、拒绝已使用码、失败不假报成功。仅授权本地实现与验收，生产迁移、发布、处理旧码仍需另行确认。测试使用隔离样本，不把上一窗口的生产数量当成永久断言。

## D-319（2026-09-21，UTC+8）CDK 实际试用获用户确认

用户在8804隔离演示页实际试用后回复「同意，没问题」。确认当前CDK页面与操作，不继续无依据调整布局或重开已定业务讨论。实现基线173af45，演示入口b1f0eea。

本确认不等于生产发布/迁移授权，也不等于免除扩大MySQL回归的12条既有失败。下一阶段为第⑥块四页联合验收及发布前检查；涉及生产连接、055/056迁移、发布与处理旧码仍分别取得当次确认。演示服务不在本次确认中自动停止或清库。

## D-320（2026-09-21，UTC+8）修复联合核查J-01～J-04，不扩功能

用户同意修复两处发布阻塞（告警重开漏通知、今日订单失败伪装空态）并纠正72小时退休提示、CDK日期截断。布局与既定CDK规则不变，8804试用环境不重启、不清库。仅本地实现与验收，生产迁移/发布另行确认。

J-01实施采用告警轮次而非更新时间：新增057在operator_alerts/alert_notifications各加incident_version，数据库触发器仅在非OPEN→OPEN时递增，通知认对应轮次；发送回执携带领取轮次，旧轮次不覆盖新轮次。普通内容更新不重推，白名单/失败有界重试保留。新增迁移只在本地隔离库跑，生产应用须单独批准。

## D-321（2026-09-21，UTC+8）补迁移恢复与无卡Session恢复，不扩业务

发布前隔离演练发现055～057断点不能续跑、无卡重贴Session未恢复分卡队列后，用户同意最小本地修复。迁移SQL本身不变：正式runner先做批次权限/结构预检，055～057按受校验的逐项计划续做；同名异定义拒绝，不自动删结构/改权限。无卡Session在原资金检查后恢复唯一分卡任务，下游先暂停，实际分卡成功再恢复；有效租约不抢占，重复提交不复制任务。保留原有已绑卡路径，不触碰Browser付款代码。

本决定只授权本地实现/隔离验证，生产权限、DDL、维护窗口和发布仍另行确认。原型与四页业务方案不重新设计。

## D-322（2026-09-21，UTC+8）补验三项重要边界，不扩功能或授权发布

用户在全项目只读了解后回复“同意”，确认补验：付款开关真实范围、付款不明从工作台到数据收口、备份恢复后的解密与触发器可用性。使用本机隔离环境与合成数据，不接真实付款，不擅自改变开关语义、不重做CDK、不启动Pro。发现问题给证据后再讨论修复；生产部署、权限与迁移仍需当次确认。

结果见 `reviews/2026-09-21-step6-safety/report.md`：四条Plus收口路径通过；已证实付款门只限Browser、单库备份缺DEFINER会导致恢复后告警UPDATE报1449。页面范围提示与恢复脚本增强尚未实施，不能把“验收跑完”当“问题已修复”。

## D-323（2026-09-21，UTC+8）付款开关不改，恢复方案先解释

用户明确“付款开关：我知道就行，不用改”。保留现有名称、提示和控制语义，不再将付款文字调整列为发布前待修项。用户要求先解释备份恢复的问题、解决方式及影响；本轮仅解释，不实施恢复脚本、权限或生产变更，后续实施须确认。

## D-324（2026-09-21，UTC+8）补恢复检查，不改业务规则

用户理解恢复缺DEFINER身份导致1449以及旧检查只验表数的问题后回复“好的，明白了。同意”，批准最小本地修正：补明确的恢复身份/必要权限步骤、隔离业务读取/解密/告警功能验收，失败不得报成功。付款开关按D-323不改；不增页面或日常登记，不改CDK/订单/付款规则，不执行生产安装、恢复、授权或发布。

实施约束：恢复验证器只接受带本次恢复标签、无网络、无端口映射的临时容器；唯一允许补建的身份是已核实057的DEFINER，锁定登录、仅告警表必要权限。未知身份/规则变化拒绝；已有账号权限不足只报错、不自动加权。旧备份无057显式标“不适用”；没有解密样本不能当验证成功。真正灾备仍按独立账号/密钥恢复步骤及生产审批执行，不复制临时演练账号配置到生产。

## D-325（2026-09-21，UTC+8）诊断补功能并轻量整理，先确认可点击稿

用户同意诊断两项功能修正：付款不明不得经通用“解决”只关case，应统一去订单正式收口并加后端限制；补已有日对账逐卡明细。随后询问布局美化是否一起做，并同意轻量整理方案：运行状态→需要核对的问题/逐卡差异→按订单排查→低频导出/账单配置；沿用候光设计，不换框架、不改付款执行规则、不删排障证据、不动其他页面。先提供一份可点击稿试动线，用户确认后再接正式功能。

原型 `docs/design/prototypes/step6-diagnostics-interaction.html`，仅合成数据，CSP禁网络连接/表单外发，不加载admin.js；现有8899原型服务预览。当前交付是待试用的交互草案，不是已确认的视觉基准，也不是功能修复已完成。发布继续等待诊断确认及实施验收，付款开关依D-323不变。

## D-326（2026-09-21，UTC+8）诊断交互稿获确认，接正式功能

用户在诊断交互稿展示后回复“没问题”，确认D-325的四段轻量布局与动线，进入已同意的两项功能修正及真实接线。沿现有订单/卡片详情、Browser控制、账单与CSV接口，不新建付款操作；通用case关闭在后端拦付款不明。仅本地实施与隔离验证，不是生产发布授权。

本地实现/证据：`reviews/2026-09-21-diagnostics-implementation/report.md`。新增diagnostics.js/css，原型外部CSS冻结、几何契约及变异验证入库。原生产历史case不自动清理，Pro/付款开关/数据库迁移规则不变。

## D-327（2026-09-21，UTC+8）两项UI建议不改，推进既定统一发布

用户明确成功率口径“我知道就行”、钱包两个金额框“影响不大暂时不改”，两项不作发布阻塞。随后回复“同意 推进吧”，按已说明的第⑥步统一发布方案执行：固定66bfe98，推送/prepare、备份、短暂停接单及相关服务、055～057迁移、专用迁移账号临时SUPER并立即撤回、隔离恢复核验、switch与运维入口安装、独立复验后恢复原运行状态。

停止条件及收敛依发布方案：资金未定/在途/结构冲突/备份或权限撤回失败则停止后续，保持必要维护，不自动倒库或删结构。不另开真实付款/开卡/退款/提现测试，不清理旧码与历史case，不重启本机Browser池。Pro路线、付款开关以及两项UI原样保持。此条不是⑦⑧步或额外业务变更授权。

## D-328（2026-09-21，UTC+8）修复日对账存储容量，验收后继续发布

用户对“最小存储修复，补真实落库、读回及恢复测试，再继续发布”回复“同意”。增加058将app_settings.setting_value从VARCHAR(255)扩大为MEDIUMTEXT，保留原值/非空/字符规则；055～057不改。迁移前校验及DDL后未记版本的续跑保护覆盖058。先隔离真实持久化、读回、重复/跨日语义、恢复与完整runner验证，通过后重定候选并继续D-327同一发布边界，不再重开UI或增加服务。

**更正前轮解释**：31348字符是展示报告，不是直接写入的内容。生产只读代理拦截INSERT、未执行写入，捕获实际保存摘要544字符（date/generatedAt/discrepancyFingerprints/persistentFingerprints），仍超过255。代码原本只保存摘要，本轮保留这一点，不改存完整报告；前轮“完整报告落库”的文字撤回。原故障及扩容必要性成立，但不能拿展示长度冒充存储长度。

## D-329（2026-09-21，UTC+8）第⑥步使用反馈先统一规划、逐项整改，再完整使用一天进入⑦

用户提出卡与钱数据/刷新、token入口及待办跳转、Browser与卡台切换、历史待办、余额下时间、完成率历史数据、客户页衔接、订单脏数据与订单页重构。要求客观判断，不迎合；稳定/效率优先，不过度增加理解成本。先共同确认规划，再按规划一步一步处理，规划/决定/验收/剩余项按项目规则落盘。

**新的进入⑦条件**：本批问题全部处理并验收后，再整体使用一天，没问题且用户确认后进入⑦；覆盖D-291/D-328的“上线后先用一天就进⑦”安排。不能边留本批缺口边宣称验收完成，也不能以没有真实单样本的一天空跑代替真钱验证。

本轮只读初查并形成 `tasks/2026-09-21-post-step6-feedback-plan.md`（FB-01～09，A～E建议顺序），**具体规划待共同确认，未授权立即修代码/发布/清数据/改token/切路线**。订单重构方向由用户提出，但字段/筛选/动作/历史展示须先对需求和可点击稿。失败、关闭、历史不自动等于脏数据；未知付款、账务与欠交付保护不放宽。

D-327成功率不改曾是已定项，本次是重新讨论指标与历史样本范围，未自动批准改算法或删数据；钱包两个输入标签和付款开关没有被此次反馈明确要求改变，继续保留原裁定。

## D-330（2026-09-21，UTC+8）整改必须审跨板块影响，支付不退化；允许计划性维护

用户补充：不要只修片面、不顾其他板块；不能小问题修好却不能给客户充值。为了优化而有计划地暂时停止充值可以接受。执行前必须列影响面及核心保持项，每批都做受影响链路与支付保护回归，不把核心验证拖到最后。D-254受限付款代码、未知付款不重付不换卡等边界继续有效；计划性暂停不是强杀在途付款、任意关闭核实通道或新增付费测试授权。实际维护先明确影响/停止与恢复条件，按原状态恢复并独立验证。

钱包入口：用户提出将工作台“钱包见卡片页”做成跳转，直达卡片高级highvcc开卡区内的刷新入口，也接受更方便稳定方案。现有顶部“查余额”和高级“刷新余额”均调用同一钱包读取接口；规划保留两条候选，先比较真实交互与失败反馈，不新增钱包服务/页面，不让余额查询触发开卡。本轮只更新规划，具体落点及实现仍待核验裁定。

规划新增G0跨板块影响清单/支付基线/每批回归和维护恢复闸门，A～E顺序及D-329“全处理完再整体用一天进⑦”不变。

## D-331（2026-09-21，UTC+8）确认反馈整改规划，开始G0/A

用户回复“好的，按规划推进吧”。按D-329/330与反馈规划实施，先建立保护回归基线，再做A批最小入口与反馈修正。钱包采用用户提出的直达现有高级highvcc钱包区域；token待办直达现有输入框；不复制保存服务、不改变收费确认或资金规则。先隔离验收，不自动发布、写生产凭据、切路线或清理历史。订单页仍先讨论需求和确认原型；全部反馈闭合后再完整使用一天、确认进入⑦。

## D-332（2026-09-21，UTC+8）Demo反馈确认：钱包原位查询、撤回卡台未保存提醒、去掉方式切换弹窗

用户明确“第3点不改”是撤回新增卡台未保存提醒，“第4点”是没懂Demo测试场景，其余建议同意。撤回未保存卡源拦截，不撤回原有卡源选择与真假结果反馈修复。API/Browser方式切换去掉原生confirm，保留后台确认字段、安全校验、处理中防重复点击与结果反馈；不取消开卡/付款等收费动作确认。操作提示移到更醒目的顶部，失败/未知不自动消失。

工作台highvcc显示钱包金额USD和原位刷新，查询成功显示金额/时点，失败保留本次页面已查到的旧金额并标旧，未查到显示未查询，不冒充实时值；更新登录仍复用原入口。卡台续期机制重新调查，但未批准无证据实现保活、常驻登录或真实凭据写入。Demo测试场景只供模拟，不进正式系统。先本地实现/验证和更新Demo，不自动发布。

## D-333（2026-09-21，UTC+8）D-332交互试用确认，继续A的续期与数据核验

用户对新版Demo回复“这部分很好，同意，继续推进”。D-332交互方向与试用通过，保留现有工作台原位钱包、无新增未保存提醒/方式弹窗、顶部提示，不再重开比稿。本次不是生产发布授权。继续highvcc续期及A数据核验；必要真实登录由用户在卡台操作，凭据不进聊天/报告，未经明确确认不调用续期、登录写接口或写生产token。

## D-334（2026-09-21，UTC+8）等待登录复测期间，开始B批只读分类

用户同意先核对历史待处理、订单来源/一致性和成功率口径，交付“保留／建议归档或关闭／仍需核实”清单再共同裁定。A批未结项；23:20登录闲置复测不受影响。不关闭案例/提醒、不删除订单、不退旧CDK、不修改账本或统计算法，不接highvcc请求。B的清理与UI修改仍须按具体预览另确认。

## D-335（2026-09-21，UTC+8）批准受控关闭2旧案例与10付款不明提醒

用户对B报告提出的精确范围回复“同意”：按preview.md的2case+10alert名单复核后收口，保留历史和审计，不改orders/attempts/CDK/cards/账本/分配，不重付、不重新人工判款、不放开通用409。范围外记录不动，证据变化即停。默认dry-run，固定脚本版本，生产apply前必须匹配预览摘要并备份目标记录，一笔事务内更新状态及新增同状态order_events审计；失败回滚。脚本放独立maintenance目录，经正式createDatabasePool执行，不改生产release或重启服务。之后独立查状态/数量/业务不变。后续如需撤回已提交结果先重新评估、取得确认，不能整体倒库或触发新告警轮次。A的23:20只读复测不变。

## D-336（2026-09-21，UTC+8）Bark保留逐笔余额，其他按需降噪并精简文案

用户明确业务初期继续逐笔余额通知，后期单量多再考虑取消；其他按“需要介入才响、同一问题避免重复、普通动态留后台”的方向。正常客户提交转后台、不删除事件，覆盖D-175对来单每笔推的旧要求；余额保留D-249/275现有能力，不提前加未来开关。资金最终无法核实、拒付、客户真卡住、人工验证/取消续费问题继续保留。

文案短标题＋必要金额/币种/订单与下一步，不长篇解释，不靠截断丢掉资金信息，不把不确定写成成功。先本地完成白名单与纯推送展示层，后台原始记录保留；跨类型同故障合并须有可靠关联证据，尚未通过的合并不擅上线。未授权真实发送测试、静音全部或直接生产发布，不增加通知服务/迁移。23:20复测不变。

## D-337（2026-09-21，UTC+8）Bark订单通知优先显示充值邮箱

用户基本不看订单号，要求通知说明具体邮箱，其他D-336方向同意。手机订单通知优先取关联订单customer_email，不从自由文本推测；没有可靠邮箱/查询失败/订单号矛盾时用原订单号兜底，不因此丢通知。后台完整单号、审计和详情保留，不在手机摘要重复塞单号。此授权覆盖向既有Bark接收端展示充值邮箱，不包含密码/Session/token/卡资料；本轮仍只本地测试，不真实发送或发布。

## D-338（2026-09-21，UTC+8）按既定降噪方向处理已证实的钱包提醒重叠

用户同意继续核查重复提醒合并，承接D-336。代码和生产只读证据显示同账户“余额低于告警线”与“钱包不够开卡”可同时出现；本轮最小实现仅覆盖这组。较重提醒仍OPEN、观察时间不早于较轻提醒、同账户且当前事故轮次正常待发/发送中/已发时，暂缓较轻提醒的手机领取。主提醒投递失败、关闭、旧轮次/旧观察、不同账户或未知key格式时不覆盖。后台原始alert及轻提醒待发记录保留，不装成已解决/已发送；余额变化、拒付和订单资金提醒不受影响。

不新增通用时间静音、表、配置或定时器，不承诺所有相似通知都能合并；已经送到手机或先于重提醒产生的通知无法追溯撤回。highvcc token失效与HNSKJ快照过期属于不同卡台，不合并。仍只本地验证，生产发布/真实手机测试另行确认，23:20登录复测不变。

## D-339（2026-09-21，UTC+8）工作台成功率改为近7个北京自然日的已结束经营样本

用户对下列口径回复“同意”：按订单`created_at`统计含当天的近7个北京自然日；只有`RECHARGE_SUCCESS`/`RECHARGE_FAILED`/`CLOSED`已结束订单进分母，进行中不算失败；仅排除`order_events.metadata_json.closeRehearsalOrder=true`明确证实的演练，普通失败与未成功`CLOSED`仍保留。成功是当前状态成功，或`CLOSED`但有历史`RECHARGE_SUCCESS`事件。

工作台显示“近7天成功率”和“成功N/样本M”；M=0显示“—”，不显示0%或100%。指标点击打开同一个样本predicate的“近7天统计样本”清单，并清掉旧搜索/日期，防止清单暗中二次缩小分母。原“全部”/“已完成”保留全历史，不删订单、不更改订单状态，不与尚未定义的自动完成率混合。

本地实现与证据见`reviews/2026-09-21-feedback-b-success-rate/report.md`；实例MySQL边界验收4项、真实DOM/核心保护26项、默认1079 tests（1010 pass/0 fail/69 skipped）通过。生产只读按新口径当时为1/2=50%，仅是小样本，不当稳定性结论。未发布，不含生产改动授权。

## D-340（2026-09-21，UTC+8）客户每次来单恢复Bark通知

用户明确要求“客户来单改为通知我”，覆盖D-336中“普通客户提交只留后台”的部分；D-336其他降噪与简文案决定继续有效。

- `BROWSER_ORDER_SUBMITTED`重新进入现有Bark白名单，每个事故轮次只投递一次；后台原事件与原文保留。
- 手机标题为“收到客户充值”，正文优先显示充值邮箱和“已收到，正在排队处理”；无可靠邮箱时回退订单号。不承诺一定另推结果通知。
- 余额逐笔、拒付、付款不明、续费取消异常及D-338钱包提醒覆盖规则不变。不新增通知配置页、定时器、表或服务。
- 本轮授权仅限本地实现、测试与落盘；未授权生产发布或真实Bark测试发送。

## D-341（2026-09-22，UTC+8）发布第⑥步使用反馈第一批

用户在D-340本地收口后回复“同意”，明确同意发布当前已确认的反馈批：D-332/333工作台钱包与反馈、D-336～340 Bark规则、D-339近7天成功率。固定提交`7e88952`，无新迁移，切换前非终态订单/活动attempt/open run均0。

生产已切到`20260921-feedback-d340-7e88952`；web/worker/bark均运行新release，live/ready 200，关键开关不变。历史36条来单通知仍全部SENT且attempt_count=1，无补推。本轮未主动发真实Bark、未真实付款/开卡；手机真机效果等下一笔客户来单自然验收。证据`reviews/2026-09-22-feedback-release/report.md`。

## D-342（2026-09-22，UTC+8）7笔续费待核不进客户账号，Lemon用销卡阻断后续扣款

7笔历史成功单均无“已取消续费”证据。Leila建议逐账号确认/取消后，Lemon明确“不用处理，我会销卡”。不再进客户账号、不点取消续费；通过在卡台销卡阻断下一周期扣款。
...
执行结果：Lemon在听取逐项用途、影响与原因后回复“以上同意”。2026-09-22 00:01 UTC固定脚本`90066ea`按dry-run摘要在一笔事务完成：16个CDK改REVOKED且保留订单关联，2条ledger改RELEASED，新增16+2审计；订单及保护业务行不变。00:02 UTC新连接独立核对全部命中，release/服务/支付开关未改。证据`reviews/2026-09-22-d343-test-cdk-closeout/report.md`。

## D-344（2026-09-22，UTC+8）本批不修5X/20X关路后的客户页提示，随产品正式建设统一处理

FB-07只读审查发现：生产5X/20X路线关闭时，现有AVAILABLE码仍会先验码通过、要求客户粘贴Session，最后建单才返回路线不可用。Leila提出验码前置查路线与首屏中性文案的最小修正后，Lemon明确：“本次不用操作任何修复，5x和20x等我们系统稳定后会重点做，现在做修复没意义。”

- 本次反馈批不修改客户页、验码服务、建单服务或产品文案，不发布、不切路线、不处理现有Pro码。
- 该发现作为**已知且接受的暂时边界**保留，不列为第⑥步/进入整体试用的阻塞项；不再用“待用户裁定/待本批修复”描述。
- 当前优先级是Plus与整套系统稳定。未来正式启动5X/20X时，应把客户页宣传、验码、路线、真实开通链和各产品验收作为一个整体处理，不为现在的关闭状态单独堆临时兼容代码。
- 现有建单端`ORDER_ROUTE_UNAVAILABLE`保护继续保留，因此暂时边界不会创建订单、占用CDK或触发付款；这不等于5X/20X客户链已经可用或完成验收。
- 证据保留在`reviews/2026-09-22-customer-flow-audit/report.md`，只作后续5X/20X工作输入，不自动转成当前开发任务。

## D-345（2026-09-22，UTC+8）订单页先明确日常浏览与CDK联动需求，未定合并或实现

Lemon说明现页信息杂、状态混乱，希望以客户邮箱为主；平时打开就浏览最近所有客户充值，不只是出问题时搜索。关心充值邮箱、时间、是否成功、用哪张卡/哪个卡台、API或Browser；详情需要看到这单对应CDK，与CDK模块联动。是否合并仍纠结，未裁定。

- 以上为已明确的使用需求，不是重构/部署许可；时间窗口、时间列含义、状态分组、列表颗粒度、合并方式、原型均待共同讨论。
- 用户同意换模型后先恢复业务背景。本轮只读代码与规则、记录需求；不改业务代码，不清数据、不切路线。
- 当前代码证据：`admin.js:308`订单号主行/邮箱次行；`admin.js:2454`详情包含多个技术状态区；`admin-read-service.js:1567`的CDK投影无明文码；`cdk-service.js:436`已有批次解密匹配机制，老批次缺恢复材料时不能伪造明文。
- 关系依据：D-120、迁移051及`order-intake-repository.js:101`；同码无付款失败后可新建单并保留历史，WAITING_FOR_SESSION重贴沿用原单。同一邮箱不作为合并不同充值记录的唯一键。
- 必须保留已有安全取消、付款未知核实、人工付款确认、交易同步与诊断入口的业务能力；收敛展示不等于删除资金保护。本轮未逐项实点验收。

## D-346（2026-09-22，UTC+8）同CDK重提历史失败展开查看，列表以最新记录为主

针对“同一码第一次失败、之后允许重提时，是两行还是最新一行展开历史”的问题，Lemon选择“展开后看之前的失败”。这里的“同CDK多条记录”只指无付款失败后的原码重提、同一进行中订单的重复提交或Session重贴产生的历史，不是一个码可以正常重复充值；成功、付款已提交、付款未知或账本已有资金证据时都不会把码退回。默认列表以最新一次记录为主，过去失败记录在展开后查看；保留真实订单、时间、邮箱及付款证据，不删历史、不合并底层订单。不同CDK不因邮箱相同合成一条。该决定只确定展示颗粒度，不代表同意合并CDK与订单页面，也不更改D-339成功率统计口径。具体展开交互、日期筛选语义与异常保护仍待设计验证；未实现。

## D-347（2026-09-22，UTC+8）双时间先同列展示；具体进度以稳定高效为前提

- Lemon希望提交时间与充值成功时间都看到，同意先在列表同列两行；若原型或后期使用拥挤，可把其中一个移至详情，尚未指定移哪一个。未成功时不以失败结束时间冒充成功时间。
- Lemon希望看到具体步骤，而不只是“处理中”，但明确以系统稳定、高效为第一优先级，不接受以严重增加复杂度或不稳定性为代价。
- 本轮代码核对：`v1/src/services/order-stage.js:6`已有收到/排队/登录核对/填写结账/付款/核实开通等只读投影，`admin.js:313`已渲染stageChip。细度来自已有订单/attempt/run证据；不把现有粗粒度推断宣称为实时精确步骤，不为API凭空生成Browser内部步骤。
- 仅记录需求与约束，不授权新增付款链埋点、抓取客户页面、提高轮询频率或重写状态机；具体展示方案/映射仍待核验与原型确认。本轮无业务代码或生产写入。

## D-348（2026-09-22，UTC+8）订单/CDK两版原型否定，先校准正式CDK再继续

Lemon反馈两版订单/CDK可点击原型都不理想，说明执行者没有充分理解现有CDK系统。A/B只改变入口或分区，没有保留正式CDK已经确认并在代码中实现的普通发码、应急备码、卡网投放、有效期、四格统计、单码/批量保护、明文恢复与订单联动边界。

- 两版原型保留文件和报告作追溯，但不作为实施基线，也不再要求Lemon在A/B之间选择。
- 先精读正式CDK页面、CDK服务/策略、订单详情与同码多订单关系，向Lemon复述已核实业务；在复述和需求共同确认前不画第三版、不合并页面、不改正式代码。
- 订单方向仍沿D-345～347：邮箱优先、日常浏览最近充值、同CDK最新记录主行并展开历史失败、提交/成功时间同列、具体步骤以稳定高效为前提、详情与CDK双向联动。

本次仅纠正需求基线与交接文档，无业务代码、数据库、生产写入或部署。

## D-349（2026-09-22，UTC+8）订单页结构按完整业务重做，Demo验收通过

Lemon要求重新了解业务后再给方案，并同意按重新审查后的边界制作结构 Demo。订单页定位为“客户充值运行台”，与CDK管理页分开；默认浏览最近充值，以邮箱为主，按同一 `cdk_id` 以最新记录为主、历史失败就地展开；CDK已兑换与充值成功分开；提交/成功时间分列；支付路线、卡台、卡尾号分开；付款结果未知明确禁止重复充值；详情提供对应完整CDK和双向定位入口。

Demo `docs/design/prototypes/step6-orders-structure-v2.html` 使用合成数据、CSP禁止网络请求。Playwright真实浏览器验收覆盖首屏、同码历史展开、付款未知详情、CDK复制/定位提示；首次发现历史按钮误开详情，已修正为主行下方展开并重新通过。证据 `docs/reviews/2026-09-22-orders-structure-v2.md`。

本决定只确认结构与交互方向，不等于正式订单页实现、数据库查询改造、生产发布或清理历史数据。正式实现前仍需任务书、跨模块影响清单和按 `cdk_id` 的后端只读归组方案。

## D-350（2026-09-23，UTC+8）订单页最小正式接线：卡片联动、highvcc命名、摘要筛选与轻量布局

Lemon同意订单页结构复核后的最小实现：付款卡从订单列表跳现有V1卡片详情；用户可见卡台名统一为HNSKJ/highvcc；状态数量由摘要卡片承担唯一状态筛选入口，删除重复状态下拉；列表做轻量桌面/手机排版整理；卡片详情页继续复用V1，不重构。

本地实现范围：订单只读查询补充卡片 provider/account/card 标识；订单摘要只在订单页请求时按搜索/时间范围聚合，处理/成功/需处理/付款待核实与各自列表使用同一业务谓词；付款待核实不把普通对账案例混入；现有工作台/其他订单读取不额外执行摘要查询。订单行卡台/卡尾号复用 `openCard` 现有弹窗，highvcc旧确认文案同步更正。

验证：`npm --prefix v1 test` = 1014 pass / 0 fail / 70 skipped；新增摘要合同测试覆盖无摘要兼容、付款未知谓词一致、搜索/时间条件保留。未部署、未接生产、未触碰付款/卡分配/CDK规则。订单Demo的“在CDK页定位”仍只是演示提示，不把它宣称为已实现双向跳转。

## D-351（2026-09-23，UTC+8）Lemon否定D-350订单页交互，暂停发布并重新理解业务

Lemon实际试用本地正式后台后明确反馈：整体订单页与其他后台页面割裂；卡片摘要过大、占空间且无法筛选已充值订单；订单行点击体验不清晰，几乎所有位置都跳详情；点击付款卡打开的V1卡片详情没有回答订单场景下“这张卡与本单是什么关系、下一步看什么”的问题。结论是D-350交付不合格，不能继续打补丁或发布。

- D-350代码与本地提交保留作审查现场，但不视为通过、不发布生产。
- 先重新读取现有后台的视觉体系、订单/卡片实际任务与操作路径，再共同确定订单页的真实主操作、卡片联动目标、摘要/筛选关系和行点击边界。
- 不能把“复用V1详情”“增加摘要卡片”“整行打开详情”直接当成业务合理性证明；下一版方案前必须给出代码/现场证据和验收路径。

## D-352（2026-09-23 14:40 UTC+8）新窗口接班；方向不偏、次序偏；执行顺序改为「先让系统能无人跑、Lemon 自己跑真钱单验收」

背景：Lemon 问「怎样要求你才能完全接住」，并要求审视项目问题与方向。本窗口三路只读盘点（生产现场 / 代码结构 / 三十天投入分布，2026-09-23 06:08～06:18 UTC，证据见 HANDOFF_LOG 同日章节）。Lemon 对盘点结论「非常认可」，并补充两条前提：**目前没有客户在充值**（之前充值多、全是自用）；**后台与充值页稳定后要把大量时间转去其他工作，需尽快做完**，不能等客户单来了再做。

已验证事实（摘要）：生产 78 单 / 成功 20（其中人工收口 10）；最近真钱成功 2026-09-16、跑旧版；此后 6 次发布 0 真单；09-19 起 0 单；近 30 天提交 62% 纯文档，决策里后台 UI + 流程规则 119 条、付款链路 44 条且 09-18 后仅 1 条；highvcc token 09-19 起失效、快照 4 天未更新。代码显示（未在生产复现）：付款不明冻结整条 lane（`production-live-pool-worker.js:262`）、入单不查执行器心跳（`order-intake-repository.js:84`）、处理中订单无老化转人工、Session 30 分钟门槛 vs 等卡 7 天、「不许重付」三份状态副本、本机测试 70 项 MySQL 集成全跳过。

判断：**方向（CDK 驱动、六个面、资金栅栏）不偏；次序偏。** 八步做的是「运营在场时更方便」，北极星要的是「运营不在场」；六条结构性问题没有一条在八步内；真钱验收被排到⑧之后，D-329 又加了门槛。

决定：
1. **本窗口接班执行者继续做到可离开**，不另开窗口。每块结束向 Lemon 交「我理解的决定」清单核对（Lemon 2026-09-23 定）。
2. **执行顺序覆盖 PROJECT_MAP §4 八步表中⑥之后的部分与 D-329 门槛**：块 0 落盘（本条）→ 块 1 highvcc token 更新 → 块 2 A/B 收口只验已发布项、不排新后台功能 → 块 3 运营不在场三硬伤（付款不明只隔离账号不冻结 lane；入单查执行器心跳，停摆即停单并明确告知客户；处理中订单超时转人工并推 Bark）→ 块 4 **真钱集成验收由 Lemon 自己跑**（Plus 走 API 一单、走 Browser 若干单，Lemon 在场，每单对照 `contracts/2026-09-18_delivery-criteria-contract.md`）→ 块 5 订单页按块 4 真实单定需求后做一版 → 块 6 = 原⑦（Pro 同型 PoC → 改造 → 重开 305/306，20X 真钱一单 + 客户页四态）→ 块 7 = 原⑧删表 + 可离开收尾。
3. **D-329「反馈全部整改 + 完整使用一天 → 才进⑦」退休**；替代为块 2 + 块 4。「用一天」并入块 4 的真钱单过程。
4. **工作台 / CDK / 卡片 / 设置四页沿用**，只修挡住使用的问题，不重做。**订单页不先做**：D-351 被否的根子是订单场景真实需求不明，块 4 的真实单先回答它；D-349 Demo 与 D-350 代码保留，届时可捡。
5. **「可离开」定义（交付终点）**：连续 10 单 Plus 无人介入自动完成（含取消续费与账本）；执行器停摆时客户侧得到明确答复且 Bark 有推；付款不明只隔离该账号；`RUNBOOK` 有一条每周自检命令。达到后 Lemon 可转去其他工作。
6. **文档减负**：`HANDOFF_NOW` 压回真正一屏（状态 / 下一项 / 禁区 / 未验证各 ≤5 行）；`DECISIONS` 只记方向与范围决定，实现细节进 HANDOFF_LOG；新发现的 AI 漏法往 `wrapup-check.sh` 加断言而非加文档规则。
7. 不变的：D-254 付款前三件不许改；资金与生产动作当次确认；三单真钱（Plus/API、Plus/Browser、20X/Browser）仍是 V2 交付判据，只是时点提前到块 4 与块 6。

本条只改文档，无业务代码、无部署、无生产写入。

## D-352 补记（2026-09-23 15:20 UTC+8）真钱测试压到最少；订单页时点改为块 3 之后

Lemon：「自费测试是勉强同意的，尽量不用测试，如果是必测才测」；并问订单页何时做。据此修改 D-352 第 2、5 条：

- **真钱只留两单**：Plus Browser 一单（09-16 后六次发布，付款后半段在当前代码上零样本，绕不过）→ 块 3 完成后；20X Browser 一单（Pro 合同冻结与 305/306 重开前提）→ 块 6。API 路线那单暂不排，块 2 先核对当前代码对 API 路线是否有改动，无改动则不花。其余验证一律用停在付款前的 rehearsal。
- **「可离开」第一条改为**：10 次 rehearsal 无人介入 + 上述两单真钱通过。
- **订单页 = 块 5，紧接块 3**，不再依赖真实单：块 3 完成后先读四页视觉体系与 D-345～D-351 否定原因，交 Lemon「订单页要回答的三个问题」（找单 / 跟进度 / 处理异常），拍板后做一版，验收数据用历史 78 单 + rehearsal 单。
- 块 1 已完成：token 07:04 UTC 保存，07:12 轮次同步成功，快照 $44.74（证据见 CURRENT_STATE 同行）。

## D-353（2026-09-23 16:20 UTC+8）块 2 收口：反馈台账裁定、默认路线切 Browser、API 路线标未验证

Lemon 逐条裁定（本窗口）：
- **默认路线切 BROWSER**（08:13 UTC 后台切换，审计 `10cb1079`）。理由见 CURRENT_STATE 同行。Lemon 判断 API 路线「应该可以」（很久前成功过很多单）；执行者不反驳，只标「未验证」，不花真钱验。
- **FB-01 / 02 / 03 / 05 / 06 / 08 关闭**：已发布项即验收（FB-02 由 Lemon 当天用后台保存 token 成功坐实）。
- **FB-04 保留、不急**：1657 / 3159 / 7402 三张卡 Lemon 尚未在卡台销，余额（$1～2）与生产一致；等 Lemon 销后再核实登记，不新增功能。
- **FB-07 并入块 6**（D-344 已裁定不修）。**FB-09 = 块 5**。
- 块 2 不排任何新后台功能。

下一块 = 块 3（运营不在场三硬伤），动手前先交文件白名单与验收方式给 Lemon。

## D-354（2026-09-23 16:55 UTC+8）已 Plus 账号不充；手动补钱后卡必须可用
...
## D-356（2026-09-23 23:35 UTC+8）订单页八问裁定（块 5 需求对齐，`tasks/2026-09-23-orders-page-three-questions.md`）

Lemon 逐条答，并「同意你建议的方向」（每码只显示最新一次尝试、历史就地展开；只有邮箱格可点开右侧抽屉；付款卡在订单里回答「用了哪张、扣了没、还能充几单」而非跳卡片弹窗）：

1. 默认时间窗 = **近 7 天**。
2. 筛选加**产品**与**路线**两个下拉（状态四选：处理中 / 已成功 / 未成功 / 需要我处理）。
3. 列表只放一个时间、另一个进详情。执行者定：**列表放提交时间，成功时间进详情**（Lemon 未指定，可改）。
4. API 路线进度三档（已收到 / 处理中 / 结果）**接受**。
5. 「手工充完标成功」做成按钮；Lemon 问「做在详情页是否有点深」→ 执行者定：**列表「需要我做什么」格直接放此刻唯一可做的动作按钮**（取消放卡 / 标成功 / 去核实），点击弹确认；详情抽屉顶部同样有。守卫同脚本（无付款痕迹才准）。
6. 付款不明判定只在订单抽屉做，诊断页只跳转 → **可以**。
7. 工作台「用/剩」= **(b) 剩 N 张 + 还能充 N 单**（按每卡上限减已用）。
8. D-350 版已在生产 → Lemon「你定」→ 执行者定：**留着，块 5 做完直接替换**。

下一步：可点 Demo（真实 80 单形态、标识匿名化）→ Lemon 点验 → `parity/orders-page.json` 契约 → 实现。

## D-357（2026-09-24 01:5x UTC+8）订单页 Demo v3 六轮后「过」；冻结原型、写契约、进入实现

Lemon 对 `step6-orders-v3.html`（`1b72003`）六轮反馈后回「过」。冻结：`prototypes/_frozen/orders-v3/README.md`（样式层与结构节奏说明；外壳/列表形态复用 `_frozen/cdk-a`）。契约：`parity/orders-page.json`（18 个探针，量控件几何不量列宽）。六轮裁定沉淀：卡尾号可点开本单抽屉并定位「卡与钱」（不跳 V1 卡片弹窗）；行内动作是与芯片同行的文字链接；抽屉里定位链接不弹窗；列表客户格只留邮箱，单号进抽屉头；历史尝试为邮箱后「N ▾」小标；客户+产品贴紧、路线组前 32px、进度前 22px、时间右对齐右留 28px。
全局令牌对比度（t3 / warn）**未改**，Lemon 未答，留待后议。
实现范围见 `tasks/2026-09-24-impl-orders-page.md`。D-350 版留在生产至本次替换。

## D-358（2026-09-24 02:1x UTC+8）订单页第八轮：时间列前移、进度列收尾；全站内容区封顶 1280；补 DESIGN.md 与 1920 视口验收

Lemon 在本地真实页面上指出「客户/产品/路线/进度都挤在左边、右边一大块干什么的」。量过：进度列吃掉全部余量，1440 下 559px、1920 下 1039px，内容最宽约 330px；实现与六轮定稿原型一致，是设计问题不是走样。根因是主内容区从没定过最大宽度，四页都随屏幕撑满。
裁定（Lemon「进度往右一些；其他的补」）：
1. 列序改为 客户 → 产品 → 路线 → **提交时间（左对齐，左留 22）→ 进度（左留 48，最后一列吃余量）**；第六轮「时间右对齐右留 28」撤销。
2. **全站内容区封顶 1280px**（admin.css `.main-area .topbar,.main-area .view`），依据 GitHub Primer `container-xl` 1280 / Linear ~1280 / Stripe ~1200（GitHub 原始文件核对）。工作台、CDK、卡片页在宽屏上一起收窄，这不是重做，是加一条外壳规则。
3. 补三样缺的能力：项目根 `DESIGN.md`（按 DESIGN.md 规范从代码提取，创意语言沿用客户页「候光」，概览措辞待 Lemon 核对）；真实页验收与几何契约加 1920 视口（契约视口改为 1920 并加「封顶」探针）；impeccable 设计评审（critique）对实现跑一次，结论入 reviews。
证据：`reviews/2026-09-24-orders-v3-impl.md` 第八轮段；原型 `step6-orders-v3.html` 覆盖为第八轮，冻结 README 同步。生产仍未发布。

## D-359（2026-09-24 03:2x UTC+8）critique 五项裁定；两个全局令牌调深；等 Session 单取消资格补齐

Lemon 对 `reviews/2026-09-24-orders-v3-critique.md` 的裁定：**1 做**（抽屉取消资格）、**2 不做**（桶内付款不明不改红）、**3 不改**（默认桶仍「全部 · 近 7 天」）、**4 同意**（等 Session 行内取消保留，芯片加「还剩 N 小时」，块 5 之后做）、**5 同意**（令牌调深）。
- 取消资格：抽屉只读投影补「等 Session 且已放卡」分支，条件逐字对应取消服务的无卡分支（无 ACTIVE/UNKNOWN/SETTLED attempt、无 create_direct 非确定失败调用、无外部单号/卡键）；`cardWillBeReleased` 无卡时为 false。此前 D-355 放卡后这类单在抽屉里永远没有「取消并放卡」，而列表和取消服务都放行。两条单测（可取消 / 有 UNKNOWN attempt 不可取消），第一版 mock 错位曾假阳性通过，按 getOrder 查询顺序改正后才算数。
...
## D-362（2026-09-24 09:0x UTC+8）欠账 15 落地：工作台按产品改为「剩 N 张 · 能充 N 单」（按 D-355 ⑦ 的既定裁定）

接班一屏曾把欠账 15 写成「要和 Lemon 讨论」，查 D-355 ⑦ 发现 Lemon 2026-09-23 已选 **(b) 剩 N 张 + 还能充 N 单**，不必再议，直接做。
- 「能充 N 单」= 库存口径（stockCountingCardSql）里每张卡（该产品每卡上限 − 已用次数）之和；上限走 D-361 的 `maxPaymentsSql`，已用与资格 SQL 同口径；SQL 新列 `remaining_orders`（`providerCardStockSql`），投影 `byProduct[].remainingOrders`；旧的 `used`（历史卡张数含已销）字段保留在接口、页面不再显示。
- 排版：数字才用等宽（汉字在等宽里字距发散），「自动补 / 需人工开」固定落第二行右侧，三格等高 47px，1280 与 1440 都不溢出。
- 本地隔离库实测：Plus 剩 2 张 · 能充 6 单（2 张 × 3）；5X / 20X 剩 0 · 能充 0。


## D-363（2026-09-24 09:2x UTC+8）新窗口接班：水位归 0 是 Lemon 操作；先做「同一故障只推一次」；块 4 与演练暂无条件

Lemon 答复新窗口三问：
1. **00:48:38 UTC 把 HNSKJ Plus 水位 1→0 的是 Lemon 本人**（后台 `admin`）。临时止响就是它，保持不动。
2. **四个问题的顺序同意**：先做⑤「同一故障只推一次」（调度器重试故障卡台时不先关缺卡告警，缺口真补上才关），再把①～④一批做；①排版先给原型再动。
3. **块 4 不花钱，且 Lemon 手里已没有 free 号。** 因此：块 4 真钱 Plus 一单不排；「可离开」第一条里的 10 次演练也做不了（演练要一个免费号的 Session）；块 6 PoC 本就因无账号挂起。**「可离开」第一条目前没有可执行路径**，怎么改由 Lemon 定，执行者先给选项，不自行改定义。

## D-364（2026-09-24 10:3x UTC+8）排版以 Lemon 的屏幕为准、格内保持一行；①～④本地实现待看

Lemon：「排版优先要适配我的电脑屏幕，而不是随便挤出两个宽度」「换两行视觉上很不好看，要保证一行」。据此：
- 首要验收视口 = 1440×730（MacBook 逻辑 1440×900，Chrome 窗口占满宽、约 817 高，后台站点无缩放；读自系统与 Chrome 配置）。写进 `DESIGN.md` Do 条。
- 工作台「卡与钱」每个产品格一行：1440 下格内可用 204px，一行只需约 172px（两位数约 187）；D-362 用 `width:100%` 把「自动补/需人工开」硬推下行是多余的。格子变窄时改显示「自动/人工」，不换行。
- ②③④ 按新窗口方案（Lemon「其他没问题」）：两台钱包统一「钱包 X USD · 查询于 时间 + 刷新余额」（数据 = `provider_balance_snapshots` 最新一行，两页共用 `latestProviderBalancesSql`；hnskj 后台刷新改为也落余额历史）；成功提示 4 秒消失、失败保留；「更新登录」先试一次钱包接口，卡台明确说 token 失效才带去贴 token；卡片页 highvcc 格直接显示上次余额，「查询于/刷新」放标题行右侧，原地刷新。
- 卡片页冻结原型（A 版）里那格还是「查余额」按钮，契约现报 5 处高度差（63→57 等）；按冻结规则，Lemon 看过新样子再重冻。

## D-365（2026-09-24 10:5x UTC+8）①～④发布；欠账 16/17 做；同一故障只推一条；时间例外同意

Lemon 答复：
1. **①～④ 发布**（先重冻卡片页原型那格）。另提：「剩 N 张 · 能充 N 单」字体总觉得和整页不协调，不知怎么调——执行者出几版让他挑。
2. **欠账 17 做**：转台开卡只替「正在等卡的订单」开，不替水位开。
3. **欠账 16 做**：调度器一台开不了就接着看下一台。
4. **同一次卡台故障只推一条**：「卡台故障」推过之后，同一台的「缺卡但开不出来」不再另推（只进后台）。
5. 「可离开」没懂——需要用大白话重讲，不改定义。
6. **钱包「查询于」当天只写时分**，同意（DESIGN.md 已记为受控例外）。

## D-366（2026-09-24 11:2x UTC+8）字体选 B；「可离开」第一条改为甲；D-365 三件发布
...
## D-370 更正（2026-09-25 01:3x UTC+8）5x 卡金额不用调；5x 真钱验收等客户单
...
## D-387（2026-09-26 UTC+8 凌晨）P0 止血；批 A 文案与规则定；今晚做批 A、批 B

Lemon：「1 现在做；2 统一写联系商家 / 暂时没法承诺 / 改 / 写 Pro 5X/20X，官方渠道、失败可退两个标签不改；3 今晚就可以做」。
1. **P0 止血（已做）**：LaunchAgent 加 `BROWSER_PAYMENT_VERIFICATION_WINDOW_MS=300000`；19:24:11 UTC 关付款 → 停 52721（code=0）→ bootout/bootstrap（supervisor 84622，`launchctl print` 环境含 300000）→ 19:24:52 开付款 → 池 85147 进程环境 300000、跑固定目录、心跳新鲜。付款后核实窗口回到核实器支持的 5 分钟（定不了转付款不明，由补核线再核）；D-269 ③「30 分钟」若要真正生效，须改代码并与运行租约一起设计。
2. **批 A 规则**：客户页所有「联系客服」→「联系商家」；不对客户承诺处理时限；失败屏 / 需换号屏标题写结论（**打破「标题永远是阶段名」**），进度环只在处理中显示；首页保留「Pro 5X / 20X」与「官方渠道」「失败可退」。
3. 批 A（v1 / 客户页，发布，不演练）与批 B（browser-mvp：P0 代码修 + 提速，一次演练 + 池发布）今晚做。

## D-388（2026-09-26 UTC+8 凌晨）客户页批 A 上线

release `20260925-customer-batchA-ac6e857`（`ac6e8578`，19:42 UTC switch，无迁移，回滚点 `20260925-block6-pro5x-f748bb6`）。内容见提交说明与报告 `reviews/2026-09-26-customer-ux-speed-audit/report.md`。核实：v1 1114：1049/0/65；8 条浏览器界面测试、前端 7 处变异全抓到；customer-sql-probe、ui-copy-check 通过；验卡预检对生产只读实跑（plus 可下单，pro_5x / pro_20x 路线未开）；截图亲看（成功屏风控提醒在 1440×730 首屏 486–531 px，原 774–819；失败 / 换号标题写结论、无百分比；手机长邮箱一行）；上线后公开状态接口查判未扣款关单 `PJV1-4U9x…` 由 VERIFYING 变 FAILED。
**按已定规则没做的**：成功屏「已关自动续费 / 有效期」（D-240「不向客户展示取消续费细节」，测试锁定）；复核改说「正在人工确认」（规则「不说已转人工」）；首页 Pro 字样与徽章（Lemon 定不改）；处理时限（Lemon：暂时没法承诺）。
**遗留**：①失败 / 换号屏仍显示停住的橙色环（去了数字），四态规格是「环只在处理中显示」——待 Lemon 定；②4 张判未扣款关单的卡密其实已退回可重兑，但 `canRetry` 按付款痕迹判「否」，页面叫客户联系商家（偏保守、不误导）——小修待排；③巡检补「执行器已失败、订单仍处理中」告警未做（P1 #4）。

## D-389（2026-09-26 UTC+8 早上）批 B（缩小版）+ 客户页两处小修

Lemon「以上全部同意你的建议」（批 B 按缩小范围今晚做；失败 / 换号屏去掉停住的环；已退回卡密显示可重新兑换）。
1. **客户页（服务器 release `20260926-d389-1ca9a42`，`1ca9a422`，23:20 UTC switch，无迁移，回滚点 `20260925-customer-batchA-ac6e857`）**：失败 / 需换号屏整个进度环收起（`.run.is-conclusive .ring{display:none}`，1440×730 与 390×844 截图亲看、无横向滚动）；失败单的卡密已退回（`cdks.status=AVAILABLE` 且 `order_id IS NULL`）直接 `canRetry=true`，不再按「点过付款」判否——生产 4 张判未扣款关单（9TN0 / CkFQ / G4OZ / 4U9x）卡密实查 AVAILABLE、未绑单、无有效期；上线后 `PJV1-4U9x…` 状态接口 `canRetry` false→true。`customer-sql-probe.sh` 原先手抄一份状态 SQL（漏了批 A 两处改动），改为从仓库导出的 `SELECT_ORDER` 实跑（按查询码 / 按卡密 / 真实订单三种）。
2. **批 B（browser-mvp，任务书 `tasks/2026-09-26-batch-b-pool-fix.md`，付款前三件未动）**：核实窗口上限只留核实器一份（`POST_PAYMENT_VERIFICATION_MAX_MS=300000`），池配置与单次配置都用它——填超了池启动即报错（新版本目录 `run-live-pool.sh check pay` 实测：300000 → READY；1800000 → `must be an integer between 30000 and 300000`）；`run-live-pool.sh` 默认 1800000→300000；跨模块测试把「值的三个来源（脚本默认 / LaunchAgent / fallback）」与「核实器能否构造」放在一起验。点击后计时（人机验证检测 / 结账页结果观察 / 账号套餐确认 / 付款结果）写本机 `pool/post-click-timing.jsonl`，只记录不改流程；演练跑不到点击后，观察函数抽成导出函数 `createPaymentOutcomeObserver`，单测直接测生产那一份。池空闲查新单 3 秒→1 秒（`BROWSER_POOL_POLL_INTERVAL_MS=1000`；心跳写库另有 5 秒节流）。测试：browser-mvp 347：338/0/9、v1 1116：1051/0/65；8 处故意改坏全部被抓。
3. 4 项有风险的提速（新标签不重复 goto、卡预检并行、库时间线异步写、入口等待）等有真单数据再做。
4. **演练与池发布**：23:21 UTC 关付款 → 停池 85147 → 关下单查心跳。第 1 张演练单 `PJV1-k3QP…` 用了企业邮箱号，定价弹窗默认 Business 栏，导航 `找到 0 个`（欠账 15，与本批无关；**导航失败证据包首次在真实页面上用上**：`evidence/20260925T234226Z-b6e75e6549a5` 截图直接给出原因）；锁到期后收口。第 2 张 `PJV1-Iynl…`（上次通过的普通邮箱号）用新版本目录代码演练 `PRE_SUBMIT_STOPPED`，PHP 982.14 / 税 0 / 付款点击 0，收口。00:03 UTC `pool-release.sh switch 20260926-d389-1ca9a42` → 开付款 → 开下单查心跳 → 池 56510 00:04:37 拉起，跑 current，进程环境 POLL 1000 / WINDOW 300000，心跳新鲜。池回滚点 `20260925-block6-pro5x-f748bb6`。生产暂停约 43 分钟（23:21～00:04 UTC）。
5. **发现（待 Lemon 定）**：演练用的单次工具（`production-live-worker.js:317`）组装时没开 `safeAbortOnFailure`（默认 false，`shared-live-composition.js:217`），失败后 run 挂在 RUNNING 直到 900 秒锁过期才能收口；常驻池开了（`production-live-pool-worker.js:313`），生产 `pool:lane-1` 43 次付款前失败都当场收口（开始到收口 20～369 秒）。客户路径不受影响。建议下次本来要动付款池、本来要演练时顺手改（白名单外，需批）。

## D-390（2026-09-26 UTC+8 上午）付款前卡住保持当场判失败；做欠账 17 / 16；欠账 18 不做

Lemon 答复：
1. **付款前卡住保持现状**：当场判失败、同一事务退卡密放卡、推 Bark `BROWSER_ORDER_FAILED`（「充值未完成：未付款；CDK已退回，客户可重新兑换」），客户页立即可重兑；已在结账页填好卡的（现场保留）先等 90 秒接手（D-210 / D-212 / D-213）。不改成「先通知、等 Lemon 看完再让客户重兑」。代码 `browser-execution-repository.js:893-913`；生产该告警推送 31 次 SENT，最近 `pcNK` 12:38:39 判失败、12:38:41 推到（UTC）。
2. **欠账 17（巡检补「付款前挂住」告警）与欠账 16（客户页四态剩余两项）现在做，一次发布。**
3. 「什么时候对外发新卡密」Lemon 答「同意」，含义待确认，不记为决定。
4. **欠账 18（演练工具失败不当场收口）不做**：「等后面如果我用着感觉还是不舒服的话，再调」。
5. **已上线（release `20260926-d390-ebb2b78`，`ebb2b787`，00:41 UTC switch，无迁移，回滚点 `20260926-d389-1ca9a42`）**：
   - 欠账 17：巡检加第三种卡住「停在付款前没人处理」（`stalled-order-queries.js` 的 `PRE_PAYMENT_STUCK_SQL`：订单处理中、有派发任务，且没有排队任务、没有有效租约的任务或 run、没有 HUMAN_REQUIRED、没有越过付款的 run，持续 3 分钟以上）→ `BROWSER_ORDER_STALLED` 推手机，文案说明钱没动、按 RUNBOOK §3 收单。排除规则在生产历史关单上实跑过：付款前停下的全选中，点过付款的 4 张（9TN0 / CkFQ / G4OZ / 4U9x，`PAYMENT_DECLINED`）全排除。**本机起不了一次性 MySQL（`mysqld` 缺 `libabsl_base` 动态库，修要重装软件，未做），没有在真实卡住的单上验过真阳性。**
   - 顺带：「客户卡住了」告警在订单结束后自动收掉（以前从不解除；上线后首轮把 09-12～09-18 的 3 条旧告警收掉，对应订单均 CLOSED，通知转 CANCELLED、未推手机）。
   - 欠账 16 之一：等卡说明改为「正在准备专用卡,可能需要几分钟」（D-250 定稿原文），后面只接「关掉本页也不影响…」，不重复「几分钟」。
   - 常驻池仍跑 `1ca9a422`：两版之间池子加载的代码只多了一个它不引用的新文件，不重启。
6. **冲突待 Lemon 定（欠账 16 之二）**：复核（REVIEWING）的客户文案。D-250（09-17，Lemon 五答 ③）定「归处理中，说明换成『正在人工确认，完成后这里会更新』」；09-12 客户页设计有测试锁定「出问题时说实话，不说『已转人工』」（`customer-page-houguang.test.js:56`），现文案为橙色「遇到点问题,我们已经收到通知在处理」。D-388 我按后者没做而没问，是按顺眼的一条下了结论——这次停下交 Lemon。

## D-391（2026-09-26 UTC+8 上午）复核文案按 D-250；何时对外发卡密由 Lemon 自己定

Lemon 答复：
1. **选 A**：客户页复核（REVIEWING）按 D-250 归「处理中」——绿色、标题仍是阶段名，说明写「正在人工确认，完成后这里会更新」，不承诺时间、不显示失败。**覆盖 09-12 客户页设计「不说已转人工」那条测试锁定**（`customer-page-houguang.test.js:56`）。
2. **什么时候对外发新卡密：Lemon 自己决定**，系统侧无动作。
3. **已上线**：release `20260926-d391-54dd294`（`54dd2947`，00:55 UTC switch，无迁移，回滚点 `20260926-d390-ebb2b78`）。复核屏：`tone: 'ok'`、标题仍是阶段名、环与百分比照常、保留复制订单号；「付款确认比平时久」那屏另行固定为橙色（原先借用复核的样式，不在本次范围）。v1 1124：1059/0/65；两处变异（复核改回橙 / 慢确认去掉橙）均被真浏览器测试抓到；截图 1440×730、390×844 亲看。欠账 16 清零。常驻池不动（两版之间池子加载的代码无变化）。

## D-392（2026-09-26 UTC+8 上午）付款前页面类失败自动重跑：先不做

讨论起因：Lemon 问「付款前卡住就立刻让客户重新兑换，对吗？能手动解决的问题会不会伤体验」。
查实：①「重新兑换」清空卡密与 Session、回第 1 步（`customer.js:782-798`）；②付款前失败类型（自用期，只看种类不看频率）：页面类（CHECKOUT_DRIFT / NAVIGATION / OBSERVATION）、号的问题（Session 失效 / 已是 Plus 走换号；企业邮箱 Business 栏走失败）、访问被挡；③页面类失败后同号同卡密成功 15/21，但间隔只有 4 张在一刻钟内（1 / 8 / 11 / 12 分钟），其余 1～3 小时、多在 09-13 改代码期间——**立刻重跑的成功率没有数据**；④停下等人会占住唯一 1 张可分配卡、运营不在时客户无期限等。
方案（修正版）：页面类失败按错误码自动重跑 1 次（共 2 次），访问被挡 / 号问题不重跑。代价：页面真变了时每单多等约半分钟～2 分钟、通知晚一轮；每次重跑可能在客户号上多建一张结账单（风控影响无证据）。
**Lemon 定：先不做**（他的担心：脚本识别不准、页面小变化、网络 / 隧道异常，自称无实证；若做，最多 2 次）。现状保持 D-390：当场判失败、退卡密、推手机。
仍待答：建议 2（重新兑换时卡密预填、说明按失败原因写）、建议 3（企业邮箱号改走「需要换一个账号」）。

## D-393（2026-09-26 UTC+8 上午）建议 2、3 放着；失败原因要有记录、方便统计

Lemon：「2 3 都放着。另外，我们后续的失败原因要有记录，方便我们统计。」
1. 重新兑换卡密预填 + 按原因说明、企业邮箱号改走换号：**都放着**（欠账 20 同条）。
2. 失败原因记录与统计：本条之后核查现状、补缺口（见下文实施记录）。
Lemon 定（同日）：**两件都现在做**——①付款前失败把具体原因（诊断原话 + 失败截图目录名）写进 `orders.failure_reason`（改付款池一处，需全量测试 + 演练 + 池重启）；②后台加「失败原因统计」（选日期、总单 / 成功 / 失败按原因分组、可点开看单）。**演练单自动排除**（按运行记录里的演练标记），页面默认最近 7 天、可改日期。
**更正与 Lemon 再定（同日）**：我曾说「具体原因没进数据库」，查的是 09-14 旧单——**错了**。导航失败自 09-25（D-375 / D-380）起在 `browser_run_events` 的 `fail-closed` 摘要里存 `navigationError`（清洗过的原话）/ `navigationActions` / `evidenceRef`；付款段非成功结局自 09-16（D-239 续）起存 `payment-outcome-diagnostic`（`diagnostic` 原话）。只有「账号被拦」等少数几类只有代码。据此 Lemon 定：①**付款池那件不做**；②统计放**诊断页新增一张统计表**（订单页冻结原型不动）：选日期（默认近 7 天）→ 总单 / 成功 / 未成功（写明排除演练 N 单）→ 按原因：单数、占比、最近一次、最近一条原话；点开看是哪几张单。演练单判定：有运行记录、且没有一条是常驻池（`worker_id LIKE 'pool:%'`）正式跑的（`BROWSER_REHEARSAL_STOPPED` 不算正式）；无运行记录的单算真实单。
**实施结果（同日）**：release `20260926-d393-738204c`（`738204c4`，03:07 UTC switch，无迁移，回滚点 `20260926-d391-54dd294`）。
- 诊断页新增「失败原因统计」（原型 `step6-diagnostics-interaction.html #fail-stats` 先改、实现照原型；契约数量 4→5 两边一致）：按提交时间（UTC+8 自然日）、默认近 7 天、「全部」不带日期；顶部写明「共 N 单 · 成功 · 未成功 · 还没结束 · 已排除演练 M 单」；每种原因：单数、占比（占未成功）、最近一单、最近一条原话（执行时记下的优先，没有则用订单说明并标「订单说明」，都没有写「只有代码，没有原话」）；点原因展开最近 20 张单（单号可点开抽屉，带失败截图目录名）。原话里连续 8 位以上数字遮掉。
- 结局口径：成功＝RECHARGE_SUCCESS 或关单前最后一步成功（与客户页同一份 `closedLastStatusSql`）；未成功＝失败 / 卡失败 / 其余关单；其余＝还没结束。
- 演练判定修正（第一版把退役一次性付款脚本 `worker_id` 为空的 5 张真付款单误判成演练）：正式运行＝常驻池（非演练停止）或 `worker_id` 为空或付款越过点击；生产被排除的 12 张逐张核对均为演练 / 测试。
- 验证：隔离 MySQL（`pojia-stage1-mysql` 内自建库，验完已删）集成测试 2/2，演练判定三条与「成功后关单」「读付款段诊断」五处变异全抓；同库顺带验「付款前挂住」告警真阳性（D-390 当时未验）：K1 租约过期没人管、K2 任务已结束订单未转均抓到，正常 / 排队 / 刚过期 / 付款不明 / 等人工 / 刚建 6 种不报。几何比对诊断页 @1440、订单页 @1920 与原型一致；四态（有数据 / 无数据 / 接口失败可重试 / 401 回登录）真页面亲验。
- 失败原因中文对照从 orders.js 挪到 admin.js 一份，订单页与诊断页共用；补「账号页认不准」（PROFILE_PAGE_AMBIGUOUS）。
- 现状：导航失败原话自 09-25、付款段自 09-16 记录；此后常驻池尚无真实失败单，所以生产上各原因目前都显示订单说明。「账号被拦」等少数几类仍只有代码。
**Lemon 核对「我理解的决定」四条：以上同意**（2026-09-26 UTC+8 上午）——付款前自动重跑先不做；卡密预填 / 企业邮箱改走换号放着，看统计数据再议；失败原因看诊断页统计表、付款池不改；统计默认近 7 天、演练单自动排除。

## D-394（2026-09-26 UTC+8 上午）21 张老码 Lemon 自己作废；做「放弃并放卡」按钮与真数据库测试

Lemon 答复：
1. **21 张自用期老码**（生产 AVAILABLE、无有效期、09-05～09-18 生成：Plus 19 / Pro 20X 2；D-313 定「CDK 页上线后用页面批量作废」未执行）：**Lemon 自己在 CDK 页批量作废**，顺带验收批量作废按钮。近几天演练用的都是 09-23 起的新码，不受影响。
2. **做后台「放弃并放卡」按钮**：付款前挂住的单，后台取消按钮因「跑过一次」拒绝（`order-cancellation-service.js:158-170`，SUBMISSION_RISK），现在只能 SSH 跑 `close-rehearsal-order.mjs`——Lemon 一个人处理不了，是「可离开」缺口。守卫与收单脚本同一套：任何付款痕迹即拒、run 租约未过期即拒。
3. **做真数据库测试**：平时 67 项跳过；规划页记 MySQL 集成在生产代码上 14/43 失败（D-258）。修好、以后发布前在本机测试容器里跑。
...
## D-395（2026-09-26 UTC+8 中午）「演练单」统一按运行方判定

Lemon 选 A：首页成功率（D-339 原只认收单脚本打的 `closeRehearsalOrder` 标记）与诊断页失败统计（D-393 按运行方）**统一为一份规则——按运行方判定**：有运行记录、且没有一条是正式运行（常驻池非演练停止 / worker_id 为空的退役一次性付款脚本 / 付款越过点击）即为演练单。生产上原标记判为演练的单新规则全部同判，新规则多排除 2 张（`zffo` 09-01 非付款测试、`pcNK` 09-25 误跑预检的演练单）；代价：首页近 7 天成功率样本会少算这类没用脚本收口的演练失败单。收单脚本仍写标记，只作审计信息，不再用于排除。
...
## D-397（2026-09-26 14:4x UTC+8）第一张卡密先发给能等的熟人、约 Lemon 在场的时间

新窗口接班（Lemon 问「怎么确保这个项目你能做好」）。接班核对：`state-check.sh` 20 项一致、`wrapup-check.sh` 全过、池心跳新鲜；可用卡密 7→8 的文档漂移已查清并更正（`22de7574`）。
...
## D-397 补记二（2026-09-26 16:xx UTC+8）Lemon：「做」——开不出卡时叫人的缺口已改，待发布

目标（交 Lemon 时的两条）：有客户在等卡、开不出来时每单响一次手机；补不了时不再推「正在补」。
实现（`862b6bb6`，只动服务器 v1，browser-mvp 不动、常驻池不用重启）：
- 自动开卡开着时，「客户在等卡」（ORDER_WAITING_FOR_CARD，每单一行）归调度器：本轮开不出（钱包低于底线 / 故障无处转台 / 日限 / 读不到钱包 / 有花过钱没核对的开卡任务）才打开，只叫可分配与在途都顶不上的那几单；分到卡、取消、或订单离开等卡时关，关不推手机。
- 分卡重试不再关这条（原先每 60 秒关一次，调度器再开就是一次关→开、每分钟响；这也是以前一条都不响的原因）。自动开卡关着时照旧。
- 库存偏低与等卡两类告警等本轮结论出来再写：「已安排 X 开一张 / 开卡任务在跑 / 开不出来：原因」。
- 与 D-365 的关系：「卡台故障只推一条」不变（管的是供卡层的「缺卡但开不出来」）；新这条是按客户订单的，故障期间每来一个等卡的客户仍响一次。
验证：v1 1143（1073/0/70）；真数据库 70/0/1，含新用例（真触发器上：推过一次后两轮分卡重试 + 两轮调度，incident_version 仍为 1；订单关闭后收掉）；8 个变异全被抓；生产只读：等卡单 0、待收的等卡告警 0、最新一轮（08:10 UTC）`BLOCKED` 且 waiting 0 → 上线后不会多推。发布待 Lemon 批。
**发布**（Lemon：「发布」）：release `20260926-d397-a44cb28`（HEAD `a44cb28`，v1 与 `862b6bb6` 一致），08:24 UTC switch，无迁移，回滚点 `20260926-d396-7a7998b`。独立核实：三服务 active 且 cwd 新目录、health 200、08:24 后 err 日志 0；首轮调度 08:24:43 UTC success / BLOCKED，hnskj 库存告警文案变为「开不出来：该台此刻不能开（FAULT:CARD_STOCK_PURCHASE_DISABLED），API 路线不转台。」、incident 未变；08:24 后新建 / 发出推送 0 条；等卡告警 OPEN 0。

## D-398（2026-09-26 16:5x UTC+8）ZovoCard 接进本系统，但不受本系统运营 / 执行规则约束（Lemon 定方向；边界待核对）

Lemon：「我希望能列入到我们系统，但是我不希望那套成熟的系统被我们自己的运营后台规则约束。」
**原则：一件事只有一套规则。** ZovoCard 那条路线上，执行相关的一律听它的；本系统只做入口、记录、通知与总开关。

- **本系统管**：卡密校验与客户页（Session 原样转交，不另加门槛）；总开关（停收新单、路线开关）；订单与事件记录、客户看到的进度；Bark 推送；对账展示（读它的资金流水 `balance_change`，不套本方卡账本）。
- **ZovoCard 管**：预检（Session 有效性、账号状态、报价）、选卡 / 开卡 / 注资、付款、拒付换卡、每卡次数、确认开通、取消续费、付款不明的处理、服务费。
- **本方规则在这条路线上不生效**：分卡任务与卡资格 / 每卡上限 / 最低卡余额 / 水位与开卡调度 / 消费账本占卡、本机执行器心跳、Session 30 分钟门槛、「付款前失败不重跑」（D-392）、Browser 的卡住判定。
- **只保留两件，都是它文档自己要求的调用方式，不是本方另加的限制**：本系统一张单只在它那边下一次（本方单号作它的 `client_request_id`）；结果不明时只查不重下。
- **实现方式**：入口分流（现状：所有订单一进来就建 `ASSIGN_CARD`、路线必须挂本方卡台，`order-intake-repository.js:110-120,326`）——新路线在入口走自己的分支，不进分卡与卡账本那条流程；不改 browser-mvp（D-254 不涉及）。
- **待 Lemon 核对**：①成功口径：D-240 要「确认 Plus + 取消续费」后才显示成功；按本原则改为它报 `completed` 即显示成功，续费没取消成功只进后台提醒（它自己会重试取消）。②走它哪条下单口：开放接口直充要本方指定 `card_id`；卡密兑换口是它自动选卡——按本原则倾向让它选卡，实测后定。
- **前提未满足**：Lemon 的 ZovoCard 账号余额 $0、未充值；直充接口需「已有入账充值 + 管理员开通或 SVIP」。字段与行为须对着真实响应（沙盒或真单）核实后才写代码（CLAUDE.md 惯犯规则 3）。
- CLAUDE.md「未来主执行链路是 Browser Worker」暂不改：这条路线实测通过、Lemon 定它当主路线时再改；Browser 路线保留作后备。

## D-399（2026-09-26 17:2x UTC+8）D-398 作废：ZovoCard 改为独立项目 `~/code/ZovoCard直充`

Lemon：「这个项目是不是更适合做独立的项目……交集太少，硬挤一起还不如独立开发」→ 讨论后「独立做吧，新建项目」。
新事实（相对 D-398）：ZovoCard 自带卡密发放 / 兑换 / 订单查询 / 回调，本项目能共用的卡密与客户页它也有；本项目最重的卡层与 Browser 付款在那条线上都用不上；接入要改本项目管真钱的入口与状态机（所有订单一进来就建 ASSIGN_CARD，`order-intake-repository.js:110-120,326`）。代价（Lemon 知悉）：两边不能自动互为后备、要看两个后台。
**结论**：本项目不接 ZovoCard，不改代码；D-398 的边界原则搬到新项目（其 D-002 独立、D-003 执行听 ZovoCard）。本项目继续按 D-352 / D-366 / D-397 等第一张真实客户单；ZovoCard 实测若明显更好，Browser 线后续投入另行讨论。
