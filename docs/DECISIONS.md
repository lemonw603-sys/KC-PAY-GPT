...
| D-119 | Browser 第一版容量采用 6 个常驻 BitBrowser Profile：单 Profile 同一时刻只处理一单，Profile 间并行；订单间必须清理客户页面、Session Cookie 与站点存储，清理失败的 Profile 不得回池。冷启动物理窗口必须顺序预热，全部就绪后才领取共享队列订单，避免 Local API 部分成功和后排订单租约过期；窗口连接后页面任务继续并行。最终 Checkout 税费/总额必须在填写卡片与账单地址后稳定，再绑定权威付款 permit 和 submit intent。 | 真实 1→3→6 访问/隔离和六 Profile 共享队列非付款闭环均已通过 | 2026-09-04 额度恢复后，全新隔离 MySQL 8.4 + migrations `001–044` + 六条合成 job 原样重跑；六条 job/run/attempt/funds 全部安全收敛，permit/submit/未释放租约全为 0。证据见 `docs/browser-research/BITBROWSER_SIX_PROFILE_ACCESS_AND_ISOLATION_VERIFICATION_2026-09-03.md`、`docs/browser-research/BITBROWSER_SIX_PROFILE_SHARED_QUEUE_NONPAYMENT_ATTEMPT_2026-09-03.md`。 |
...
| D-121 | Profile 客户隔离必须清除全部客户身份/未知 Cookie、站点 storage 和客户页面，但保留并恢复严格 allowlist 的 Profile 运行访问 Cookie：`cf_clearance`、`__cf_bm`、`__cflb`、`_cfuvid`、`oai-did`。不得保留 Session/Auth 或其他未知 Cookie，也不得因粗暴全清让每单重新触发 Cloudflare。 | 已实现并完成三 Profile 真实非付款验证，未部署 | 单测证明 Session/未知 Cookie 不恢复；真实三路 ChatGPT HTTP 200 与 Cookie/storage 隔离均 3/3。证据见 `docs/browser-research/BITBROWSER_THREE_PROFILE_ACCESS_AND_ISOLATION_ATTEMPT_2026-09-03.md`。 |
| D-122 | `PHP 982.14` 不是不可实现的历史价格，而是已被真实成功 API 订单和运营方近期多单确认可成交的目标总价；Browser 业务出口固定使用菲律宾，不把美国出口作为候选解法。当前 `PHP 1100.00` 只代表现有菲律宾 BitBrowser Profile + 当前 Checkout 信号组合产生 12% VAT；不得把它推广成所有菲律宾充值的固定成本。Browser 首笔真实付款前必须在菲律宾出口不变的前提下，用非付款 A/B 确认造成 `982.14/1100.00` 分流的账号、Checkout 创建路径、卡 BIN/支付信息和账单资料信号，并始终以地址填写后的最终稳定总额作为付款输入。 | 目标价格和菲律宾出口均已确认；Browser 路径机制待验证 | 历史成功 API 订单 `982.14 PHP` 与卡台 PURCHASE 已落盘；当前 Browser Delaware 非付款结果为 `1100.00 PHP`。研究与实验边界见 `docs/browser-research/PHILIPPINES_VAT_ROUTE_DIFFERENTIAL_2026-09-03.md`。 |
| D-123 | 美国出口可在单次明确允许下作非付款诊断对照，但不因此改变 Browser 生产出口保持菲律宾的决策。美国出口常规升级入口已实测切到 `US/USD`，Delaware 地址后为 `USD 20.00 + 0.00 tax`；该结果不是 `PHP 982.14` 复现。 | 常规路线对照已完成；旧显式探针结果已降级；生产决策未变 | 旧 `US + PH/PHP` 探针两次 HTTP 400 使用附件旧 token 裸调 API，绕过官方 Sentinel/request pipeline，不能作为账号、出口或地区组合被禁的有效证据；见 `docs/browser-research/US_EXIT_DELAWARE_TAX_AB_NONPAYMENT_2026-09-04.md`。 |
| D-124 | 禁止用附件旧 token 裸调 Checkout，也禁止在官方 Sentinel 生成后改写 Checkout POST body；两者都不是官方原生请求。地区/币种实验只能在官方前端构建请求之前改变已识别的输入，并在生成的 POST 不匹配合同时本地 abort。 | 已实现并现场验证 fail-closed | 官方 UI 请求确认 bearer/Sentinel/设备/目标路由头齐全；后改 POST 仍 HTTP 400；前置 PH config 后生成 `US/PHP`，本机拦截且 Checkout 上游请求为 0。 |
| D-125 | PH pricing config 同时存在 Plus `1100 inclusive` 与 `psp_override 982.14 exclusive`。后续 `982.14/1100` 分流主假设从“伪造 PH/PHP 地区”调整为“识别官方 PSP route/processor selection”；在未冻结官方选择条件前不得将 `982.14` 当作无税价或强制覆盖金额。 | 配置事实已现场验证；PSP 选择条件待查 | 美国出口下前置读取官方 PH config HTTP 200；官方前端仍生成 `US/PHP`，因此未向 Checkout 上游发送请求、未填卡、未付款。 |
| D-126 | 菲律宾官方 UI 下，当前 HNSKJ 测试卡与 `US/DE` 地区组合已确认稳定为 `982.14 + 117.86 = 1100.00 PHP`；官方 Checkout create 为 `PH/PHP` 且 `automatic_tax_enabled=true`，未观察到 ChatGPT checkout snapshot。不再重复同卡随机试验；下一个有效变量是不同 BIN/发卡路由的合规测试卡。 | 已完成非付款实测；第二卡类待具备 | `submitCalls=0`；证据见 `docs/browser-research/PH_OFFICIAL_UI_PSP_ROUTE_NONPAYMENT_2026-09-04.md`。 |

| D-127 | 首笔 Browser 真实付款仅在官方 Checkout 填卡与账单资料后，最终稳定总额严格为 `₱982.14` 且税费为 `₱0` 时允许；显示 `₱1,100`、含 VAT、金额不稳定或无法确认时，必须停在付款前。该授权不改变 Browser 付款默认关闭，仍需订单、卡片、Session 和单 Profile 现场复核。 | 用户已确认条件性授权；尚未执行 | 当前生产 Browser 付款权限仍为 false；既有 PH 实测为 `₱982.14 + ₱117.86 = ₱1,100`，不能视为已满足条件。 |
