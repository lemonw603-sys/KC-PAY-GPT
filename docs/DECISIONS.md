...
| D-123 | 美国出口可在单次明确允许下作非付款诊断对照，但不因此改变 Browser 生产出口保持菲律宾的决策。美国出口常规升级入口已实测切到 `US/USD`，Delaware 地址后为 `USD 20.00 + 0.00 tax`；该结果不是 `PHP 982.14` 复现。 | 常规路线对照已完成；显式 PH/PHP 创建被上游门禁拒绝；生产决策未变 | `US + PH/PHP` 已窄探测两次，均返回 HTTP 400 `unusual activity`，未进入 Checkout且已停止重试；见 `docs/browser-research/US_EXIT_DELAWARE_TAX_AB_NONPAYMENT_2026-09-04.md`。 |
