...
| Browser | 单 Profile 非付款税费闸门已通过；三 Profile 候选已准备但访问闸门未通过；付款仍未验收 | 原 BitBrowser Profile 已验证 Session/FREE/真实 Checkout/填卡/最终金额；六 Profile 池代码和隔离 fixture 已通过 | 2 个新 Profile 先各自取得 ChatGPT HTTP 200；之后恢复 3/6 Profile 同开、真实隔离/长期运行；生产部署与真实付款 |
...
- 容量方向已确认并完成代码实现：6 个常驻隔离 Profile，单 Profile 串行、Profile 间并行；macOS launcher 已支持 1–6 Profile。现场已新建 2 个无同步候选 Profile，但它们在无 Session 阶段访问 ChatGPT 即为 HTTP 403；因此当前仍只有原 Profile 通过真实访问闸门，三路同开不能验收（详见 `docs/browser-research/BITBROWSER_THREE_PROFILE_ACCESS_AND_ISOLATION_ATTEMPT_2026-09-03.md`）。
...
- Browser 下一步：先让至少一个新候选 Profile 通过其自身 headed 公开页访问或独立稳定出口建立 ChatGPT HTTP 200 基线；再恢复 3→6 Profile 同开、固定出口、并发隔离和吞吐验证。仍保持非付款，真实付款与生产部署需独立确认。
