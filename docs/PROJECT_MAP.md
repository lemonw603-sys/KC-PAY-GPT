...
> **2026-09-06 D4 生产对齐、自动开卡止损与发布机制修复（当前最高优先级）**：生产 current 仍为 `/opt/pojia/releases/20260905-browser-zero-tax-04e08e6`，但按 `04e08e6` 全量比对受控文件得到 `204 same / 27 changed / 0 missing / 2 extra`，证实该目录是旧 release 加局部 Browser 文件形成的多提交混装产物。混装携带的旧订单重试逻辑持续每分钟创建并领取新的自动开卡任务；停止前累计 969 条，故障窗口 `opened_count=0`、无新增卡、无 ACTIVE/UNKNOWN 资金。用户确认无需恢复该架构后，已将自动开卡 timer 停用、关闭数据库总开关并从部署候选删除 timer 单元，跨完整周期验证任务不再增长。migration 047→048 已在隔离 MySQL 8.4 用代表旧订单前滚成功（约 0.441 秒）。发布机制已提交 `8012da8`：只能从单一 commit 构建完整归档并全量校验，修改/缺失/额外文件均失败关闭；本地候选 825 个 tracked 文件通过。下一步为单独确认后的 D5 部署窗口，当前仍未部署。证据见 `docs/CARD_SOURCE_D4_PRODUCTION_ALIGNMENT_2026-09-06.md`。
