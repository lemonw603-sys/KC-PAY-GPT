...
未完成：Browser 自动填表与付款（0 次）；付款后凭证刷新路径（未验证）；20X 自动化；供给自动化（HNSKJ 维护中，自动开卡已停）；Worker 常驻；后台新版；账号风险数据（0）。
...
3. 按规格实现新流程 → 测试账号跑到付款前 → 一笔真实付款 → 删除旧编排。完成标准：旧实现已删除。**进度（09-07）**：现有 LIVE 链路已是页面点击创建结账 + 单遍填写；新增演练模式 `BROWSER_LIVE_STOP_BEFORE=SUBMIT`（停在点击前）与常驻身份不关窗；待用测试订单跑一次演练。
4. 并行：六个身份；供给自动化（按产品）；Browser Worker 搬到常开机器（BitBrowser Windows 版）；每单阶段时间线入库。
