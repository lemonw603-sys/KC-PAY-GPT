...
**当前推进点（2026-09-04）**：本地只读 env 已生成并通过配置加载器检查；一次 `ONCE` canary 已到生产 DB `READY`，但 BitBrowser Profile warmup 因厂商返回“网络不通已停止打开浏览器”安全退出。先修复/验证 Profile 代理连通性，再重跑 canary；不加载 launchd、不启动常驻生产 Browser Worker。
