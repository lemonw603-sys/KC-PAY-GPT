...
> **2026-09-05 多卡源策略方案冻结（已确认，未实现）**：用户确认未来可能有原卡台 + 2–3 个不同用途备用卡台，且切换频率较高。已落盘 `docs/CARD_SOURCE_SWITCHING_POLICY.md`：采用可注册卡源目录、能力声明、健康状态、优先级和策略版本；Browser 支持 AUTO / 指定来源优先 / 暂停来源三种最小模式；API 自动排除无 API 来源；切换只影响新订单，分配后冻结卡源，付款未知禁止换源/重付；后台集中控制并独立统计。下一批工作是实现策略表/审计、事务选择器、订单冻结字段、控制入口和集成回归；在此之前不部署、不改变生产路线。
