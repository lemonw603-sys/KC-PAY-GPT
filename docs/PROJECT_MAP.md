...
| 订单 | 成功 2（含 09-06 人工 20X）、失败 7、关闭 8、API 路线等卡 1、等 Session 2、**测试单 CARD_READY 1（演练后，卡 5501 仍分配，SUBMIT_RECHARGE 待 Worker）**；活动资金/run/队列/租约/许可全部 0 |
| 迁移 | 048 |
...
4. 并行：六个身份——**常驻多身份 Worker 已实现并冒烟**（`production-live-pool-worker.js`，`run-live-pool.sh check|run rehearsal|pay`，09-07 REHEARSAL 模式 Lane 3 空转 45 秒、心跳落库、窗口不关；出口隔离仍缺）；供给自动化（按产品）；Browser Worker 搬到常开机器（BitBrowser Windows 版）；每单阶段时间线入库。
