...
当前跳转事实：卡台规则、卡片库存、卡余额充值、Browser、对账、卡台路线已有映射；API 执行权限关闭只有文字提示，没有按钮。 默认充值方式切换按钮已经存在于“卡台路线”页面，可在 API/Browser 间切换，仅影响新订单；切换 Browser 前会检查 Browser dispatch、活动 profile 和心跳。这是待收敛缺口，不能再写成“所有阻断都有入口”。
