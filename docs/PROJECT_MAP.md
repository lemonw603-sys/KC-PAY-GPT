...
下一笔 Browser 真实订单前，必须按 `docs/2026-09-04_full-system-preflight-scope.md` 一次性核对客户充值页、运营后台（含 Browser）、订单/资金、库存/卡台/自动补给、后端共享核心及生产部署。该体检不创建订单、不调用 Provider 写接口、不付款；结果必须区分实时证据、代码证据、历史快照和未验证。
