...
/**
 * A minimal configured JIT address source. The address belongs in protected
 * runtime configuration, never in jobs, WAL, artifacts, or repository docs.
 */
export class ConfiguredBillingAddressSource {
  constructor({ address } = {}) {
    if (!address || typeof address !== 'object' || Array.isArray(address)) {
      throw new TypeError('address is required');
    }
    this.address = structuredClone(address);
  }
